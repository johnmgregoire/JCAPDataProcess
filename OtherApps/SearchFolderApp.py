...
class SearchFolderDialog(QDialog, Ui_SearchFolderDialog):
    def __init__(self, parent=None, title="", folderpath=None):
        ...
        self.setupUi(self)
        self.parent = parent
        self.FolderPushButton.pressed.connect(self.processopenfolder)
        self.PathlistPushButton.pressed.connect(self.processopenpathlist)
        self.SearchPushButton.pressed.connect(self.performsearch)
        self.ParentFolderPushButton.pressed.connect(self.selectfolder)
        self.openfolder = None
        self.openpathlist = None
        self.FoldersCheckBoxes = [
            self.CheckBox_0,
            self.CheckBox_1,
            self.CheckBox_2,
            self.CheckBox_3,
    ...
    def selectfolder(self):
        folder = mygetdir(
            parent=self, xpath=self.folder, markstr="select folder for search"
        )
        if len(folder) == 0:
            return
        else:
    ...
    def findrunfolder_searchstr(self, folder, searchstr):
        ...
        for dirpath, dirnames, filenames in os.walk(folder):
            ...
            runpathlist += [
                ...
                for fn in filenames
                if (searchstr in fn or searchstr in os.path.split(dirpath)[1])
                and ".zip" in fn
            ]
...
if __name__ == "__main__":
    ...
    class MainMenu(QMainWindow):
        def __init__(self, previousmm, execute=True, **kwargs):  # , TreeWidg):
            ...
            # self.setupUi(self)
            self.searchui = SearchFolderDialog(
                self, title="Select items to open", **kwargs
            )
