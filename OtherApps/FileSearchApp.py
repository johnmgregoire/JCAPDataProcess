...
class filesearchDialog(QDialog, Ui_filesearchDialog):
    def __init__(self, parent=None, title="", folderpath=None):
        ...
        self.setupUi(self)
        self.parent = parent
        self.treeWidget.itemDoubleClicked[QTreeWidgetItem, int].connect(self.openpath)
        button_fcn = [
    ...
    def openpath(self, item, column):
        ...
        ans = userinputcaller(
            self,
            inputs=[("file path", str, s)],
            title="Path available for copy",
            cancelallowed=True,
        )
    ...
    def findfolders(self):
        ...
        searchtups = []
        for i, (cb, lab, fold) in enumerate(
            zip(
                [
                    self.exp_k_checkBox,
                    self.exp_j_checkBox,
                ...
                ["EXP", "EXP", "ANA", "ANA"],
                [
                    tryprependpath(EXPFOLDERS_L, ""),
                    tryprependpath(EXPFOLDERS_J, ""),
                    tryprependpath(ANAFOLDERS_L, ""),
                    tryprependpath(ANAFOLDERS_J, ""),
                ],
            )
        ):
            if (
                len(fold) == 0 or not cb.isChecked()
            ):  # didn't find exp or ana folder but found other one
                self.toplevelitems += [None]
                continue
            mainitem = QTreeWidgetItem(
                ["%s~%s" % (lab, fold.rstrip(chr(47)).rstrip(chr(92)))], 0
            )
            ...
            for typefold in [
                ...
                for expanafold in [
                    fn
                    for fn in os.listdir(typefoldpath)
                    if self.foldersearchstr in fn
                    and os.path.isdir(os.path.join(typefoldpath, fn))
                ]:
    ...
    def nestedfill(self, fold, parentitem, fnendswith, level):
        ...
        if self.foldersearchstr in fold:
            fnstoadd = [
                ...
                if os.path.isfile(os.path.join(fold, fn)) and fn.endswith(fnendswith)
            ]
...
if __name__ == "__main__":
    ...
    class MainMenu(QMainWindow):
        def __init__(self, previousmm, execute=True, **kwargs):  # , TreeWidg):
            ...
            # self.setupUi(self)
            self.filesearchui = filesearchDialog(
                self, title="Search for exp/ana files", **kwargs
            )
