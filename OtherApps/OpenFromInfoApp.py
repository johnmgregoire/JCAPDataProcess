...
class openfrominfoDialog(QDialog, Ui_OpenFromInfoDialog):
    def __init__(self, parent, runtype='', exp=True, ana=True, run=False):
        ...
        QObject.connect(self.plateidLineEdit,SIGNAL("editingFinished()"),setfocus)
    ...
    def nestedfill(self, d, parentitem, prependstr='', skipkeys=[], expanaparent=False):
        ...
        dictkeys2=sorted([k for k in d.keys() if (self.exp and k.startswith('experiments')) or (self.ana and k.startswith('analyses'))])
        for k in dictkeys2:
            ...
            if expanaparent:#a k==run__X dict gets here
                ...
                if ('searchbool' in rd.keys()) or (('path' in rd.keys() and str(self.searchLineEdit.text()) in rd['path']) and ('type' in rd.keys() and str(self.typeLineEdit.text()) in rd['type'])):
                    item.setCheckState(0, Qt.Checked)
                else:
                    item.setCheckState(0, Qt.Unchecked)
                self.expanaditems+=[item]
            
            self.nestedfill(d[k], item, expanaparent=True)#first time through the "runs" key is found and then runparent True passed to the next level where run__ are found
            parentitem.addChild(item)
            item.setExpanded(True)#not working?
        
        dictkeys1=sorted([k for k, v in d.iteritems() if (not k in dictkeys2) and isinstance(v, dict)])
        for k in dictkeys1:
            item=QTreeWidgetItem([prependstr+k+':'], 0)
    ...
    def searchfcn(self, searchfolds, ana=True):
        ...
        if len(foldn)==0:
            testpaths=[os.path.join(rootn, fn) for rootn in searchfolds for fn in os.listdir(rootn)]
        else:
...
#        item.setText(column,''.join([ans, keepstr]))
    ...
    def ExitRoutine(self):
        selectkeys=[str(item.text(0)).strip().strip(':') for item in self.expanaditems if bool(item.checkState(0))]
        select_expanak_dlist=[(k.partition('__')[0], self.infofiled[k.partition('__')[0]][k]) for k in selectkeys]
        ...
        self.selectexppaths=[d['path'] for expanak, d in select_expanak_dlist if expanak=='experiments']
        self.selectanapaths=[d['path'] for expanak, d in select_expanak_dlist if expanak=='analyses']
