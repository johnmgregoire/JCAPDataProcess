...
class loadcsvDialog(QDialog, Ui_LoadCSVDialog):
    def __init__(self, parent, ellabels=['A', 'B', 'C', 'D'], platemappath=None, csvstartpath='', runk='run__1'):
        ...
        self.csvpath=mygetopenfile(parent=self, xpath=csvstartpath, markstr='Select .csv file')
        ...
        QObject.connect(self.sampleComboBox,SIGNAL("activated(QString)"), self.setplatemapoption)
        ...
        if self.ellabels!=['A', 'B', 'C', 'D']:#visualizer already has ellables so don't let user change them
    ...
    def opencsv(self):
        ...
        for k, cb in self.comboboxdict.items():
            ...
            for count, k in enumerate(self.csvd.keys()):
                cb.insertItem(count+1, k)#fom choices are not associated with particular indeces of the l_ structures
            cb.setCurrentIndex(0)
    ...
    def getinfousingplateid(self):
        ...
        if len(pmpath)==0:
            idialog=messageDialog(self, 'Error loading plate from plate_id %s' %plateidstr)
            idialog.exec_()
            return
    ...
    def setplatemapoption(self, settrue=False):
        if int(self.sampleComboBox.currentIndex())==0 or len(str(self.platemapLineEdit.text()))==0:#using indexes as fake sample_no so cannot use platemap, or platemap not available
            self.useplatemapCheckBox.setChecked(False)
    ...
    def createadhocplatemapdlist(self):
        ...
        return [dict([(k, arrd[k][i]) for k in arrd.keys()]) for i in range(len(arrd['sample_no']))]
    def ExitRoutine(self):
        if self.rund['parameters']['plate_id']==0:
            try:
                self.rund['parameters']['plate_id']=int(str(self.plateidLineEdit.text()))
            except:
                pass

        adhocbool=False
        if len(str(self.platemapLineEdit.text()))==0 or int(self.sampleComboBox.currentIndex())==0:#need to make dummy platemap if there is no platemap or using row number as sample_no
        ...
        self.rund['platemapsamples']=[d['sample_no'] for d in self.rund['platemapdlist']]
