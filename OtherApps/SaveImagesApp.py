...
class saveimagesDialog(QDialog, Ui_SaveImagesDialog):
    def __init__(
        ...
        fnl = [
            ...
            if fn.endswith(".ana") and not fn.startswith(".")
        ]
        if len(fnl) == 0:
            idialog = messageDialog(
                self, "Cannot save to ANA because no .ana in the folder"
            )
            idialog.exec_()
            self.reject()
            return
        ...
        if filenamesearchlist is None:
            ...
            searchchecker = lambda filen: True in [
                not (False in [searchstr in filen for searchstr in searchlist])
                for searchlist in filenamesearchlist
            ]
        self.widget_plow_dlist = []
        for widgk, val_dict_list in zip(
            self.widgetkeys[0:2], [self.plateid_dict_list, self.code_dict_list]
        ):
            ...
            for (k, d) in val_dict_list:
                ...
                if filenamesearchlist is None:
                    ...
                    item.setCheckState(
                        0, Qt.Checked if searchchecker(filen) else Qt.Unchecked
                    )
                mainitem.addChild(item)
                d["item"] = item
                self.widget_plow_dlist += [d]
        for widgk, plotw, lab in zip(
            self.widgetkeys[2:4], [xyplotw, histplow], [self.xyyname, self.fomname]
        ):
            if plotw is None:
                continue
            mainitem = self.widgetTopLevelItems[widgk]
        ...
        for widgk, browser, lab in zip(
            self.widgetkeys[4:5], [selectsamplebrowser], [self.selectsamplesname]
        ):
            if browser is None:
                continue
            mainitem = self.widgetTopLevelItems[widgk]
    ...
    def updateoptionsfrombatchidialog(self, batchidialog, lastbatchiteration=False):
        ...
        combinedprependstr = self.filterchars(
            prependstr + str(self.prependfilenameLineEdit.text())
        )
        ...
        if (
            lastbatchiteration
        ):  # only want to convert to done on last image being batch-saved
            self.doneCheckBox.setChecked(
                batchidialog.doneCheckBox.isChecked()
            )  # for batch save, images saved in place and then box check in the end if convert to .done chosen
    ...
    def ExitRoutine(self):
        ...
        anadict = readana(
            oldp, erroruifcn=None, stringvalues=True, returnzipclass=False
        )  # cannot be a .zip
        startingwithcopiedbool = "copied" in os.path.split(self.anafolder)[1]
        if (
            startingwithcopiedbool or self.doneCheckBox.isChecked()
        ):  # must convert to .done if starting with .copied. allows .done to be edited which is bad practice
            if not os.path.split(self.anafolder)[1].count(".") > 1:
                idialog = messageDialog(
                    self, "Cannot save because ANA folder has no extension"
                )
                idialog.exec_()
                return
            if (
                startingwithcopiedbool
            ):  # if modiyfing a .copied then need a new time stamp
                newanafn = timestampname() + ".ana"
                newanafolder = (
                    self.anafolder.rpartition(".")[0][:-15] + newanafn[:-4] + ".done"
                )
                movebool = False
            else:
                newanafolder = (
                    self.anafolder.rpartition(".")[0] + ".done"
                )  # this reapleces .run with .done but more generally .anything with .done
                movebool = True
                newanafn = self.anafn
            saveana_tempfolder(
                None,
                ...
                anadict=None,
                movebool=movebool,
                savefolder=newanafolder,
                saveanafile=False,
            )  # move files if necessary but don't create .ana or .exp yet. Do this first so image files get put only into new folder
        ...
        for d in self.widget_plow_dlist:
            ...
            if self.epsCheckBox.isChecked():
                fn_attr_list += [
                    (pngfn.replace("png", "eps"), pngattr.replace("png", "eps"))
                ]
        ...
        saveanafiles(
            newp, anadict=anadict, changeananame=True
        )  # need to overwrite the name because may be a new anafolder/timestamp
