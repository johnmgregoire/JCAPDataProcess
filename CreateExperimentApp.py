...
class expDialog(QDialog, Ui_CreateExpDialog):
    def __init__(self, parent=None, title="", folderpath=None):
        ...
        self.setupUi(self)
        self.parent = parent
        self.RunTreeWidgetFcns = treeclass_dlist(
            self.RunTreeWidget, key_toplevel="rcp_file", key_nestedfill="rcptuplist"
        )
        self.ExpTreeWidgetFcns = treeclass_dlist(
            self.ExpTreeWidget, key_toplevel="rcp_file", key_nestedfill="rcptuplist"
        )
        self.RunTreeWidget.itemDoubleClicked[QTreeWidgetItem, int].connect(
            self.editrunparams
        )
        self.ExpTreeWidget.itemDoubleClicked[QTreeWidgetItem, int].connect(
            self.editexpparams
        )
        ...
        for button, fcn in button_fcn:
            button.pressed.connect(fcn)
        self.batchprocesses = [
            self.batchuvissingleplate_norefdata,
            self.batchuvissingleplate,
            self.batchechedark,
            self.batchechewavelengths,
            self.batchxrfs,
        ]
        batchdesc = [
            "Filter uvis into data, ref_light, ref_dark",
            "Filter uvis into data including refs, ref_light, ref_dark",
            "Filter _DARK eche spectra as ref_dark",
            "Add eche runs by wavelength label",
            "Process xrfs",
        ]
    ...
    def updateuserfomd(self, clear=False, batchkeys=None):
        if clear:
            ...
            self.RunTypeLineEdit.setText(
                str(self.RunTypeLineEdit.text()).partition("__")[0]
            )
            return
    ...
    def batchuvissingleplate(
        self,
    ):  # uses cb filter for sample_no>0 but the only difference from other batch is to note use ref data, which has sample_no 0 so this is obselte until we see how to add sample_no that are not 0
    ...
    def batchxrfs(self):
        ...
        for i in range(mainitem.childCount()):
            mainitem.child(i).setCheckState(
                0,
                Qt.Unchecked
                if "binary" in str(mainitem.child(i).text(0))
                else Qt.Checked,
            )
        self.editexp_addmeasurement()
    ...
    def batchechedark(self):
        self.techtypetreefcns.checkbysearchstr(
            "spectrum", self.techtypetreefcns.typewidgetItem
        )
        self.RunTypeLineEdit.setText("ref_dark")
    ...
    def clearexp(self):
        for rcpd in self.rcpdlist:
            rcpd["filenamedlist"] = [
                dict(fd, previnexp=set([]), inexp=set([]))
                for fd in rcpd["filenamedlist"]
            ]
        self.expdlist_use = {}
    ...
    def importexpfile(self):
        ...
        self.LastActionLineEdit.setText(
            "Imported EXP including %d RUNs containing %d files"
            % (
                len(self.rcpdlist),
                numpy.array([len(d["filenamedlist"]) for d in self.rcpdlist]).sum(),
            )
        )
    ...
    def importexpparamsfile(self):
        ...
        self.expparamstuplist = [("exp_version:3", [])] + [
            tup for tup in importedexpparamstuplist if not "exp_version" in tup[0]
        ]
    ...
    def searchforruns(self):
        idialog = SearchFolderDialog(
            parent=self, folderpath=self.defaultrcppath, title="Search for RUN folders"
        )
        idialog.exec_()
    ...
    def importruns_info(self):
        idialog = runsfrominfoDialog(
            parent=self, runtype=str(self.ExpTypeLineEdit.text())
        )
        ...
        if "" in pathlist:
            ...
            if not idialog.exec_():
                return
        rcpdictadditions = [
            tupl for tupl, p in zip(idialog.rcpdictadditions, pathlist) if len(p) > 0
        ]
    ...
    def importruns_folder(self, folderp=None):
        if folderp is None:
            folderp = str(
                mygetdir(
                    parent=self,
                    xpath=self.defaultrcppath,
                    markstr="Folder containing .rcp or set of .zip",
                )
            )
            if len(folderp) == 0:
                return
        if folderp.endswith(".zip"):
            ...
            if len(rcpfns) == 1:
                ...
                pathlist = [
                    os.path.join(folderp, fn) for fn in fns if fn.endswith(".zip")
                ]
                if len(pathlist) == 0:
    ...
    def importruns(self, pathlist=None, startfolder=None, rcpdictadditions=None):
        if pathlist is None:
            ...
            pathlist = mygetopenfiles(
                parent=self, xpath=startfolder, markstr=".zip runs", filename=""
            )
            if not (
                isinstance(pathlist, list)
                and len(pathlist) > 0
                and not (False in [p.endswith(".zip") for p in pathlist])
            ):
                idialog = messageDialog(self, "Need to select only .zip")
                idialog.exec_()
                return
        techset, typeset, rcpdlist = readrcpfrommultipleruns(
            pathlist, rcpdictadditions=rcpdictadditions
        )
        ...
        self.processrunimport()
        self.LastActionLineEdit.setText(
            "Imported %d RUNs containing %d files"
            % (
                len(rcpdlist),
                numpy.array([len(d["filenamedlist"]) for d in rcpdlist]).sum(),
            )
        )
    ...
    def processrunimport(self):
        ...
        for d in self.rcpdlist:
            ...
            if self.getplatemapCheckBox.isChecked():
                d["platemapdlist"] = readsingleplatemaptxt(
                    getplatemappath_plateid(d["plateidstr"]),
        ...
        exp_type_list = [
            ...
            for d in self.rcpdlist
            for tup in d["rcptuplist"]
            if "experiment_type" in tup[0]
        ]
    ...
    def updaterunlist(self):
        ...
        self.techtypetreefcns.cleartree()
        for ind, (k, tl, treeitem) in enumerate(
            [
                ("tech", self.techlist, self.techtypetreefcns.techwidgetItem),
                ("type", self.typelist, self.techtypetreefcns.typewidgetItem),
            ]
        ):
            k_comments_tuplist = []
            for t in tl:
        ...
        self.RunPriorityLineEdit.setText(
            ",".join(["%d" % (i + 1) for i in range(len(self.rcpdlist))])
        )
    ...
    def editexp_addmeasurement(self):
        ...
        inexpfcndict_previnexp_filter[0] = (
            lambda filterresult: (filterresult == 1) and set.union or set.difference
        )
        ...
        self.editexp(
            inexpfcndict_previnexp_filter,
            user_run_foms=self.userfomd,
            run_foms_fcn=self.run_foms_fcn,
        )  # , rundesc=rundesc)
        self.updateuserfomd(clear=True)
    ...
    def editexp_filtercriteria(self):
        ...
        inexpfcndict_previnexp_filter[1] = (
            lambda filterresult: abs(filterresult) and set.union or set.difference
        )
        # inexpfcndict_previnexp_filter[2]=lambda filterresult: abs(filterresult)
        self.editexp(inexpfcndict_previnexp_filter)

    def editexp(
        self, inexpfcndict, user_run_foms={}, run_foms_fcn=None
    ):  # , rundesc=''):#for the user-specified data use, uses inexpfcndict to add or remove a file from the exp based on whether the fiel was not or was in the exp already. uses orderinds to define priority of evaluating the rcpdlist, the inexp and previnexp keys of the rcpd are the notable items being set here
        ...
        for ind in orderinds:
            ...
            rcpd["filenamedlist"] = [
                dict(fd, previnexp=fd["inexp"]) for fd in rcpd["filenamedlist"]
            ]
            if (
                selrun == 0 or (selrun - 1) == ind
            ):  # if considering all or this is the 1 we are considering then build an evaluation function
            ...
            rcpd["filenamedlist"] = [
                dict(
                    fd,
                    inexp=inexpfcndict[datause in fd["previnexp"]](
                        evalfiltresult * int(not fd["fn"] in expfilelist)
                    )(fd["previnexp"], dus),
                )
            ...
            # rcpd['fns_inexp']=
            fns_inexp = [
                fd["fn"] for fd in rcpd["filenamedlist"] if datause in fd["inexp"]
            ]
        ...
        s = str(le.text()).strip()
        s = filterchars(
            s, valid_chars="-_.; ()%s%s" % (string.ascii_letters, string.digits)
        )
        if s == dflt or len(s) == 0:
            le.setText(newdflt)
        self.expparamsdict_le_dflt["description"][1] = newdflt
        self.updateexp(
            datause, user_run_foms=user_run_foms, run_foms_fcn=run_foms_fcn
        )  # , rundesc=rundesc)
    ...
    def createFilterEvalFcn(self, rcpd):
        techlist = self.techtypetreefcns.strlist_checked(
            self.techtypetreefcns.techwidgetItem
        )
        techfcn = lambda fd: fd["tech"] in techlist
        typelist = self.techtypetreefcns.strlist_checked(
            self.techtypetreefcns.typewidgetItem
        )
        ...
        fcn = (
            lambda fd: techfcn(fd)
            and typefcn(fd)
            and filesearchfcn(fd)
            and platemapfcn(fd)
        )
        return fcn
    ...
    def createFileSearchfilterfcn(self):
        ...
        if len(s) == 0 and len(ss) == 0 and len(ns) == 0 and len(nss) == 0:
            ...
            return (
                lambda fd: (s in fd["fn"])
                and fd["fn"].startswith(ss)
                and (len(ns) == 0 or not ns in fd["fn"])
                and (len(nss) == 0 or not fd["fn"].startswith(nss))
            )
    ...
    def editparams(self, widget, item=None, column=0):
        ...
        if True in [kv in k for kv in echewarningkeys]:
            idialog = messageDialog(
                self,
                'THIS PARAM EFFECTS reference_vrhe and reference_e0\nBUT THEY WILL NOT BE AUTOMATICALY UPDATED.\nYOU SHOULD PROBABLY "Cancel"',
            )
            if not idialog.exec_():
                return
    ...
    def updateexp(self, datause, user_run_foms={}, run_foms_fcn=None):  # , rundesc=''):
        ...
        self.expdlist_use[datause] = [
            dict(
                rcpd,
                rcpdlistind=count,
                rcptuplist=self.RunTreeWidgetFcns.createtuplist_item(
                    self.RunTreeWidget.topLevelItem(count), filesbool=False
                )[1],
                filenamedlist=[
        ...
        for expd in self.expdlist_use[datause]:
            ...
            for i0v in i0vals:
                ...
                i1vals = sorted(
                    list(set([i1 for i0, i1, i2 in tuplistinds_inexp if i0 == i0v]))
                )
                l0n = []
                for i1v in i1vals:
                    k1, l1 = l0[i1v]
                    l1n = [
                        tup
                        for i2v, tup in enumerate(l1)
                        if (i0v, i1v, i2v) in tuplistinds_inexp
                    ]
                    if len(l1n) > 0:
                        l0n += [(k1, l1n)]
                        filecount += len(l1n)
        ...
        if "tri" in accesslist:
            ...
            access = "muri"
        elif len(accesslist) > 0 and not (
            False in [s == "public" for s in accesslist]
        ):  # access specified and all access is 'public'
            access = "public"
        else:
        ...
        if None in plateidstemp:
            idialog = messageDialog(
                self, "WARNING: plate_id missing for at least 1 run"
            )
            idialog.exec_()
    ...
    def saveexpgoana(self):
        ...
        for (
            ...
            rcpdl = [
                rcpd
                for rcpd in self.rcpdlist
                if rcpd["rcp_file"] == rcpfile and len(rcpd["platemapdlist"]) > 0
            ]
            if len(rcpdl) > 0:
    ...
    def saveexp(self, exptype=None, rundone=None):
        ...
        if exptype is None:
            ...
            if idialog.choice == "browse":
                savefolder = mygetdir(
                    parent=self,
                    xpath="%s" % os.getcwd(),
                    markstr="Select folder for saving EXP",
                )
        ...
        if rundone is None:
            if (
                savefolder is None
                and not self.prevsaveexppath is None
                and os.path.split(self.prevsaveexppath)[0].endswith("run")
            ):
        ...
        saveexpfiledict, exppath = saveexp_txt_dat(
            self.expfiledict,
            rundone=rundone,
            experiment_type=exptype,
            runtodonesavep=runtodonesavep,
            savefolder=savefolder,
            saverawdat=saverawdat,
            erroruifcn=lambda s: mygetsavefile(
                parent=self,
                xpath="%s" % os.getcwd(),
                markstr="Error: %s, select file for saving EXP",
                filename="%s.exp" % str(self.ExpNameLineEdit.text()),
            ),
        )
    ...
    def checkbysearchstr(self, searchstr, mainitem):
        for i in range(mainitem.childCount()):
            mainitem.child(i).setCheckState(
                0,
                Qt.Checked
                if searchstr in str(mainitem.child(i).text(0))
                else Qt.Unchecked,
            )

    def strlist_checked(self, mainitem):
        return [
            str(mainitem.child(i).text(0))
            for i in range(mainitem.childCount())
...
class treeclass_dlist:
    ...
    def createtuplist_item(self, item, filesbool=True):
        ...
        for i in range(item.childCount()):
            if filesbool or not str(item.child(i).text(0)).startswith(
                "files_technique__"
            ):
    ...
    def filltreeexp(self, expdlist_use, expparamstuplist):
        ...
        for k, dlist in expdlist_use.iteritems():
            for d in dlist:
                ...
                filecount = numpy.array(
                    [
                        len(tup2[1])
                        for tup in d[self.knf]
                        for tup2 in tup[1]
                ...
                techlist = list(
                    set(
                        [
                            tup[0].partition("files_technique__")[2].strip().strip(":")
                            for tup in d[self.knf]
                            if tup[0].startswith("files_technique__")
                        ]
                    )
                ...
                plateid = [
                    tup[0].partition("plate_id: ")[2].strip()
                    for tup in d[self.knf]
                ...
                if "run_foms" in d.keys() and len(d["run_foms"]) > 0:
                    self.nestedfill(
                        ...
                        mainitem,
                    )  # assumes run_foms values are strings
                self.nestedfill(
                    [
                        (
                            "parameters:",
                            [
                                tup
                                for tup in d[self.knf]
                                if not tup[0].startswith("files_technique__")
                            ],
                        )
                    ],
                    mainitem,
...
echeparamactiondict["reference_vrhe"][
    "message"
] = 'Press "OK" to also update reference_e0 \n "Cancel" will still update reference_vrhe'
echeparamactiondict["reference_vrhe"][
    "changefcn"
] = lambda item, oldstr, newstr: updateecheparamsfcn(
    item, oldstr, newstr, keystoalter=["reference_e0"]
)
echeparamactiondict["reference_e0"] = {}
echeparamactiondict["reference_e0"][
    "message"
] = 'Press "OK" to alos update reference_vrhe \n "Cancel" will still update reference_e0'
echeparamactiondict["reference_e0"][
    "changefcn"
] = lambda item, oldstr, newstr: updateecheparamsfcn(
    item, oldstr, newstr, keystoalter=["reference_vrhe"]
)
# *********************************************************************************************
