...
def mygetopenfiles(parent=None, xpath="%s" % os.getcwd(), markstr="", filename=""):
    if parent is None:
        xapp = QApplication(sys.argv)
        xparent = QWidget()
        returnfns = QFileDialog.getOpenFileNames(
            xparent,
            "".join(["Select file to open:", markstr]),
            os.path.join(xpath, filename).replace("\\", "/"),
        )[0]
...
def userinputcaller(
    parent, inputs=[("testnumber", int)], title="Enter values", cancelallowed=True
):
    problem = True
    while problem:
    ...
    def save(self):
        ...
        for dp in dpl:
            ...
            with open(dp, mode="r") as f:
                lines = f.readlines()
            templist = [
                (i, [l.startswith(k) for k in smpkeys].index(True))
            ...
            arr = readtxt_selectcolumns(
                dp,
                delim=delim,
                num_header_lines=headingslineind + 1,
                floatintstr=str,
                zipclass=False,
            )
            dropd = {}
        ...
        sp = mygetsavefile(
            parent=self,
            xpath=os.path.split(dpl[0])[0],
            markstr="savefile",
            filename="combinedfom.txt",
        )
        f = open(sp, mode="w")
        f.write(s)
        f.close()
    ...
    class MainMenu(QMainWindow):
        def __init__(self, previousmm, execute=True, **kwargs):  # , TreeWidg):
            ...
            self.combinefomui = combinefomDialog(
                self, title="Combine FOM from multiple files", **kwargs
            )
            if execute:
