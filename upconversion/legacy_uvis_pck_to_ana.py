...
genexpfilep=lambda expnam: os.path.join(os.path.join(r'K:\processes\experiment\uvis\todo',expname+'.todo'), expname+'.exp')
#genanafolderp=lambda ananame: os.path.join(r'K:\processes\analysis\uvis',ananame+'.todo')
genexpsavefolderp=lambda expname: os.path.join(r'D:\data\201512uvispcktoana\uvis\exp',expname+'.done')
genexpfolderforanafile=lambda expname: (r'/uvis/'+expname+'.done').replace(chr(92),chr(47))


genanafolderp=lambda ananame: os.path.join(r'D:\data\201512uvispcktoana\uvis\ana',ananame+'.done')
genresfolderp=lambda resultsfolder: os.path.join(r'K:\experiments\uvis\drop',resultsfolder)
for expname, ananame, resultsfolder in zip(l_expname, l_ananame, l_resultsfolder):#[-1:]
    print expname
...
#    expfilep=r'K:\processes\experiment\temp\manualchangesmplines.temp\20140312.123904.exp'
    ...
    if 0:#just print params from 1st file
        ...
        with open(os.path.join(resfolderp, resfns[0]+'.pck'), mode='r') as f:
            rd=pickle.load(f)
        print '\t'.join([rd['function_parameters']['darkcode'], rd['function_parameters']['refcode'], resultsfolder])
        #continue
    if anabool:
        anad={}
        anad['ana_version']='3'
    ...
    while len(runkl)>0:
        ...
        if 1:#
            zipsinexp=[os.path.split(v['run_path'])[1] for k, v in expfiled.items() if k.startswith('run__')]
        ...
        for techcount, tk in enumerate(['DR_UVVIS', 'T_UVVIS', 'R_UVVIS']):
            ...
            if techk in rund.keys() and 'spectrum_files' in rund[techk]:
                ...
                if 1 and techcount==1:#T_UVVIS
                    ...
                    if os.path.isfile(Rrunp_fullpath):
                        ...
                        if Rzipclass.fn_in_archive(rund['rcp_file']):
                            ...
                            expfiled[Rrunk]['run_path']=rund['run_path'].replace('T-UVVIS', 'R-UVVIS')
                            ...
                            expfiled[Rrunk]['files_technique__R_UVVIS']['spectrum_files']={}
                            Rfilesd=expfiled[Rrunk]['files_technique__R_UVVIS']['spectrum_files']
                            for l in Rlines:
                                l=l.strip()
                                k, garb, v=l.partition(': ')
                                if len(k)==0:
                ...
                for smpfn, filedstr in typed.items():
                    ...
                    if (techcount<2) and (smpfn[:-4] in resfns):
                        ...
                        try:
                            ...
                            continue
                        qualityfomkcheck=lambda fomk:'rescaled' in fomk or '2ndderiv' in fomk
                        fomd=dict([(fomk.replace('maxabsorp', 'max_abs').replace('-', '_'), fomv) for fomk, fomv in rd['fom'].iteritems() if not qualityfomkcheck(fomk)])
                        qfomd=dict([(fomk.replace('-', '_'), fomv) for fomk, fomv in rd['fom'].iteritems() if qualityfomkcheck(fomk)])
                        plateid=rd['measurement_info']['Plate ID']
                        if initanaparams:
                            rund['run_use']='data'
                            initanaparams=False
                            d['analysis_fcn_version']=rd['version']
                            d['description']='%s on %s; plate_id %s' %(','.join(sorted(fomd.keys())), tk, plateid)
                            ...
                            refdarkstr='0_%s_' %(rd['function_parameters']['darkcode'])
                            reflightstr='0_%s_' %(rd['function_parameters']['refcode'])
                            
                            plotparams=dict({}, plot__1={})
                            plotparams['plot__1']['x_axis']='t(s)'
                            plotparams['plot__1']['series__1']='abs' if 'abs' in rd['intermediate_arrays'].keys() else 'Signal_0'


                            csvheaderdict=dict({}, csv_version='1', plot_parameters={})
                            csvheaderdict['plot_parameters']['plot__1']=dict({}, fom_name='DA_bgcode_repr' if 'DA_bgcode_repr' in fomd.keys() else ('DA_bg_0' if 'DA_bg_0'in fomd.keys() else fomd.keys()[0]), colormap='jet_r', colormap_over_color='(0.,0.,0.)', colormap_under_color='(0.5,0.,0.)')
                        ...
                        if 1 and len(fomd)>0:#saveinter files
                            ...
                            if not os.path.isdir(anafolderp):
                                os.mkdir(anafolderp)
                            if samplestr:
                                fomdlist+=[dict(fomd, sample_no=int(samplestr), plate_id=int(plateid), runint=int(runk.partition('run__')[2]))]
                                if len(qfomd)>0:
                                    qfomdlist+=[dict(qfomd, sample_no=int(samplestr), plate_id=int(plateid), runint=int(runk.partition('run__')[2]))]#, run=runk
                            #write inter data if foms reagrdless of vald sample_no
                            for count, (fk, fdesc, fext) in enumerate([('inter_rawlen_files','uvis_inter_rawlen_file', 'rawlen'), ('inter_files','uvis_inter_interlen_file', 'interlen')]):
                                if count==0:
                                    interd=dict([(k, v) for k, v in rd['intermediate_arrays'].iteritems() if isinstance(v, numpy.ndarray) and len(v)==nrawpts])
                                else:
                                    if not 'rawselectinds' in rd['intermediate_arrays'].keys():
                                        continue
                                    ninterpts=len(rd['intermediate_arrays']['rawselectinds'])
                                    interd=dict([(k, v) for k, v in rd['intermediate_arrays'].iteritems() if isinstance(v, numpy.ndarray) and len(v)==ninterpts])
                                if len(interd)>0:
                                    fn='%s__%s_%s.txt' %(anak, os.path.splitext(smpfn)[0], fext)    
                                    ...
                                    if not fk in anarund.keys():
                                        anarund[fk]={}
                                    kl=saveinterdata(p, interd, savetxt=True)
                                    anarund[fk][fn]='%s;%s;%d;%d%s' %(fdesc, ','.join(kl), 1, len(interd[kl[0]]), samplestrwithcolon)
                ...
                if anabool and initanaparams:
                    ...
                    #raiseanerror
                    continue
                if not anabool:#use the refcode and darkcode from the pck if possible but if not use these as default
                    try:
                ...
                for refstr, runuse in [(refdarkstr, 'ref_dark'), (reflightstr, 'ref_light')]:
                    refrunk=''
                    for smpfn, filedstr in typed.items():
                        if smpfn.startswith(refstr):
                            ...
                            expfiled[refrunk][techk]['spectrum_files'][smpfn]=typed[smpfn]
    ...
    if 1 and anabool and len(fomdlist)>0:#saveana
        ...
        for count, (dlist, fk, fdesc) in enumerate([(fomdlist, 'fom_files', 'csv_fom_file'), (qfomdlist, 'misc_files', 'csv_fom_file')]):
            if len(dlist)==0:
                continue
            fomnames=list(set([k for fd in dlist for k in fd.keys()]).difference(set(['sample_no','runint','plate_id'])))
            csvfilstr=createcsvfilstr(dlist, fomnames, intfomkeys=['runint','plate_id'])#, strfomkeys=strkeys+strkeys_fomdlist)#, fn=fnf)
            ...
            anamultirund[fk][fn]='%s;%s;%d;%d' %(fdesc, ','.join(allfomnames), totnumheadlines, len(fomdlist))
        
        if 1:#copy misc files
            miscfns=[fn for fn in os.listdir(resfolderp) if fn.endswith('.png') or (fn.endswith('.pck') and not fn[0].isdigit()) or fn.endswith('.txt')]
            if len(miscfns)>0:
                anamultirund['misc_files']={}
                for fn in miscfns:
                    shutil.copy(os.path.join(resfolderp, fn), os.path.join(anafolderp, fn))
