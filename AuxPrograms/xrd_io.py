...
bstuplist = [
    ...
    (
        "user",
        ".//XmlFieldConnections/FieldRestrictionConnection",
        "FieldName",
        "User",
        ".//Data",
        ["Value"],
    ...
    (
        "collimator",
        ".//BeamPathContainerAbc/BankPositions/BankPosition",
        "BankPosition",
        "4",
        ".//MountedComponent",
        ["VisibleName"],
    ...
    (
        "frames_per_sample",
        ".//DataEntityContainer/MeasurementPoints",
        ["Value", "Unit"],
    ),
    (
        "detector_serial",
        ".//Detectors/DetectorList/DetectorData/SerialNumber",
        ["Value"],
    ),
    (
        "detector_image_size_x",
        ".//Detectors/DetectorList/DetectorData/ImageSizeX",
        ["Value", "Unit"],
    ),
    (
        "detector_image_size_y",
        ".//Detectors/DetectorList/DetectorData/ImageSizeX",
        ["Value", "Unit"],
    ),
    (
        "face_to_detection_layer",
        ".//Detectors/DetectorList/DetectorData/FaceToDetectionLayer",
        ["Value", "Unit"],
    ),
...
fmtbrukertimestamp = (
    lambda s: s.partition(".")[0].replace("-", "").replace(":", "").replace("T", ".")
)


def get_bmsl_dict(p):
    paramd = {}
    updatefcn = lambda paramd, k, v: paramd.update(
        [
            (
                k.replace("2", "two_"),
                v.replace("JCAP-XRDS-01", "HTE-XRDS-01").replace(
                    "jcap-xrds-01", "hte-xrds-01"
                ),
            )
        ]
        if len(v) > 0
        ...
        if len(tup) == 3:  # xpath to node is unique
            ...
            for node in tree.iterfind(tpath):
                strval = (
                    node.attrib[tattr[0]]
                    if len(tattr) == 1
                    else " ".join([node.attrib[attr] for attr in tattr])
                    .encode("utf-8")
                    .replace("\xb5", "u")
                    .replace("\xb0", "deg")
                    .replace("\xc2", "")
            ...
            for pnode in tree.iterfind(ppath):
                if pnode.attrib[pattr] == pval:
                    if tpath == "":  # match part of multi-attribute node
                        strval = (
                            pnode.attrib[tattr[0]]
                            if len(tattr) == 1
                            else " ".join([pnode.attrib[attr] for attr in tattr])
                            .encode("utf-8")
                            .replace("\xb5", "u")
                            .replace("\xb0", "deg")
                            .replace("\xc2", "")
                ...
                for pnode in gpnode.iterfind(".//ScanParameterAbc"):
                    if pnode.attrib["VisibleName"] == tup[0]:
                        for node in pnode.iterfind(".//Value"):
                            strval = (
                                " ".join([node.attrib[attr] for attr in tup[1]])
                                .encode("utf-8")
                                .replace("\xb5", "u")
                                .replace("\xb0", "deg")
                                .replace("\xc2", "")
                            ...
                            keyname = "_".join(
                                [gpnode.attrib["VisibleName"], tup[0]]
                            ).lower()
                            updatefcn(paramd, keyname, strval)
...
def xy_file_dict_into_rcpdlist(dp, fn, rcpdlist):
    ...
    if len(fdlist) == 0:
        ...
        numcharsincommin = lambda p1, p2: numpy.array(
            [c1 == c2 for c1, c2 in zip(os.path.normpath(p1), os.path.normpath(p2))]
        ).sum(dtype="int32")
        numchars, fd = sorted(
            [(numcharsincommin(dp, fd["folderpath"]), fd)] for fd in fdlist
        )[
            -1
    ...
    an_name = (
        ...
        if "original" in dp
        else "Analysis__XRDS_Bruker_Process"
    )
    keysstr = (
        "two_theta.deg,intensity.counts"
        if "original" in dp
        else "two_theta.deg_processed,intensity.counts_processed"
    )
    afd = {
        "folderpath": dp,
        "fn": fn,
...
def get_externalimportdatad_xrds_folder(p, parent=None):
    ...
    for rcpind, (bfold, bfn) in enumerate(b_tups):
        ...
        popinds = [
            count
            for count, (dp, fn) in enumerate(g_tups)
        ...
        g__smpind_frameind_fold_fn = sorted(
            [createtupfcn(g_tups.pop(i)) for i in popinds[::-1]]
        )  # sorted by sample ind then frame ind
        bsmld = get_bmsl_dict(os.path.join(bfold, bfn))
        rcpd["file_dlist"] += [
            {
                "tech": "files_technique__XRDS",
                "type": "bsml_files",
                "fn": bfn,
                "fval": "xrds_bruker_bsml_file;",
                "folderpath": bfold,
            }
        ]
        for count, (xstr, ystr) in enumerate(bsmld["xystr_list"]):
            ...
            for (
                ...
                g__smpind_frameind_fold_fn
            ):  # possibly not all gfrm got use but they were part of the bsml so they won't get treated as separate runs
                if smpind == count:
        ...
        if 'machine_name' in rcpd.keys():
            rcpd['machine_name'] = rcpd['machine_name'].lower().replace("desktop-hgk6u1o", "hte-xrds-01")
        rcpdlist+=[rcpd]
    ...
    for gfold, gfn in g_tups:
        ...
        rcpd[
            ...
            {}
        )  # other default paremetrs could be added here but at least need dictionary defined so plate_id can be added
        rcpdlist += [rcpd]
    for dp, fn in e_tups:
        find_gfrm_error_bool = xy_file_dict_into_rcpdlist(dp, fn, rcpdlist)
    multirun_ana_files = {}
    multirun_ana_files["Analysis__Bruker_Eva"] = [
        {
            "folderpath": dp,
            "fn": fn,
            "type": "misc_files",
            "fval": "xrds_bruker_eva_file;",
        }
        for dp, fn in m_tups
    ]
    multirun_ana_files["Analysis__User_Notes"] = [
        {
            "folderpath": dp,
            "fn": fn,
            "type": "misc_files",
            "fval": "xrds_user_txt_file;",
        }
        for dp, fn in t_tups
    ]
    return {
        "rcpdlist": rcpdlist,
        "multirun_ana_files": multirun_ana_files,
    }  # the fns in rcpdlist
