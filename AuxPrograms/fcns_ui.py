...
def autotickformat(ax, x=False, y=False, ndec=3):
    for bl, xax, lims in zip(
        [x, y], [ax.xaxis, ax.yaxis], [ax.get_xlim(), ax.get_ylim()]
    ):
        if bl:
            try:
...
def userinputcaller(
    parent,
    inputs=[("testnumber", int)],
    title="Enter values",
    cancelallowed=True,
    returnchangedbool=False,
):
    problem = True
    while problem:
...
def userselectcaller(
    parent, options=["none"], title="Select values", cancelallowed=True
):
    problem = True
    while problem:
