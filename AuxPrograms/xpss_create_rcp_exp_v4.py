...
class setup_rcp_and_exp_xpss():
    def __init__(self, import_path, rcpext='.done', expext='.done',
                 ...
                 pmidstr=None, sample_no_from_position_index=None,
                 testmode=False):
        """
        ...
        Note for usage:
        If a custom sample list is given then it needs to be wrapped into a lambda function i.e.
        sample_no = [i+1 for i in range(1972)]+[i+1 for i in range(34)]
        lambda i: sample_no[i]
        ...
        self.rcpext = rcpext
        self.expext = expext
        iserror = self.setup_folders(overwrite_runs, self.testmode)
        if iserror:
            return

        self.setup_file_dicts()
        self.add_all_files(self.testmode)
        self.save_rcp_exp(self.testmode)
    ...
    def parse_spec_files(self):
        ...
        self.data_acquisition_timestamp = '20' + parse(self.pre_parsed[0][' 151 Date Acquired']).strftime(
            '%d%y%m.%H%M%S')
    ...
    def setup_file_dicts(self):
        ...
        self.datatype, self.plate_idstr, ','.join(self.technique_names))
        ...
        rcpdict[
            'technique_name'] = self.datatype  # don't pay attention to this "technique_name" it is an artifact of previous data and does not have the same meaning as e.g. XPSSURVEY
        ...
        self.datatype, compname.lower(), self.rcpmainfoldname, rcpdict['name'] + self.rcpext)
        ...
        # messy fix for : : bug
        self.add_kratos_file = lambda fn: [d.update({fn: filed_createflatfiledesc({'file_type': 'xpss_kratos_file'})})
                                           for d in xpsstechd]
        self.pattern_file_keys = ['BE(eV)', 'Intensity']
        self.add_pattern_file = lambda tech, fn, nrows, sample_no: [d.update({fn: filed_createflatfiledesc(
            {'file_type': 'xpss_spectrum_csv', 'keys': self.pattern_file_keys, 'num_header_lines': 1,
             'num_data_rows': nrows, 'sample_no': sample_no})}) for d in techdlist[self.technique_names.index(tech)]]
...
positional_keys = [val for val in xpsimportclass.spec.keys() if ('stage' and 'osition') in val and 'Descriptor' not in val]
pos = {key:[] for key in positional_keys}
for spec in xpsimportclass.pre_parsed[1::]:
 for key in positional_keys:
    pos[key].append(spec[key])
'''
