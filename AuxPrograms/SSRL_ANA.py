...
add_integr_file = lambda fn,sample_no: [
    d.update({fn: filed_createflatfiledesc({'file_type': 'ssrl_csv_pattern_file',
                                            'sample_no': sample_no,
                                            'num_header_lines': 1,
...
add_proc_file = lambda fn, sample_no: [
    d.update({fn: filed_createflatfiledesc({'file_type' : 'ssrl_qchi_npy_file',
                                            'sample_no' : sample_no})})
    for d in procd]
...
expdict['description']= 'Analysis__SSRL_Qchi,Analysis__SSRL_Integrate on plate_id {}'.format(expdict['plate_ids'])
expdict['analysis_type']= 'ssrl'
expdict['access']= access
expdict['experiment_path']= os.path.join(os.path.split(anapath)[1],expdict['experiment_name'])
