...
class setup_rcp_and_exp_xpss():
    def __init__(self, import_path, rcpext='.done', expext='.done', overwrite_runs=False, plate_idstr=None, access='hte', pmidstr=None, sample_no_from_position_index=lambda i:(1+i), testmode=False):
        """
    ...
    def parse_spec_files(self):
        ...
        if self.plate_idstr is None:#ideally plate_id is read from the filename or spec file because it was entered by user when starting data acquisition, but if it was passed in the class init then ignore
        ...
        self.run_params_dict={}#flat dictionary where the values can be numeric or strings where illegal characters will be removed. The parameters are probably from the spec file since these params apply to all data being imported
        self.run_params_dict['technique_names']=self.technique_names #if there are parameters like starting binding energy that shoudl be saved for each technique, make the dict value a list of the param values assumed to be in the same order as technique_names
    ...
    def setup_file_dicts(self):
        ...
        for k, v in self.run_params_dict.iteritems():
            v=strrep_generic_file_dict_value(v).strip('[').rstrip(']')#make lists comma delimited but without the brackets
...
xpsimportclass=setup_rcp_and_exp_xpss('', rcpext='.run', expext='.run', overwrite_runs=True, plate_idstr=None, access='tri', pmidstr=None, sample_no_from_position_index=lambda i:(1+i), testmode=True)
