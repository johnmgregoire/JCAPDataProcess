...
def removeoutliers_meanstd(
    arr, nptsoneside, nsig, gapptsoneside=0
):  # avrages maximum of 2*nptoneside points and usees distance from mean scaled by std compared to nsig to determine if the value should be replaced by the mean. if gapptsoneside>0, will do this leaving a gap around the point in question and using nptsoneside-gaps points for the mean and std
    if nptsoneside == 1 and gapptsoneside == 0:
        return removesinglepixoutliers(arr, critratiotoneighbors=nsig)
    nsig = max(nsig, 1.0)
    nptsoneside = max(nptsoneside, 2.0)
    gapptsoneside = min(gapptsoneside, nptsoneside - 2.0)
    for gap in range(gapptsoneside + 1):
        starti = numpy.uint32(
            [max(i - (nptsoneside - gap), 0) for i in range(len(arr))]
        )
        stopi = numpy.uint32(
            [min(i + (nptsoneside - gap) + 1, len(arr)) for i in range(len(arr))]
        )
        ...
        arr = numpy.array(
            [
                (
                    ((numpy.append(arr[i0:i], arr[i + 1 : i1]).mean() - arr[i])) ** 2
...
def CalcArrSS(x, WeightExp=1.0, TestPts=10):
    p = WeightExp
    i = TestPts
    s0 = x[:i].std() / i ** p + 1
...
def xx(x, WeightExp=1.0, TestPts=10):
    p = WeightExp
    i = TestPts
    s0 = x[:i].std() / i ** p + 1
    while x[:i].std() / i ** p < s0 and i < len(x):
...
def concat_extrap_ends(x, npts, polyorder=1, lowside=True, highside=True):
    ...
    if lowside:
        ans = scipy.polyfit(-1 * (i + 1.0), x[:npts], polyorder)
        x = numpy.concatenate([scipy.polyval(list(ans), i[::-1]), x])
    if highside:
        ans = scipy.polyfit(-1 * (i[::-1] - 1.0), x[-1 * npts :], polyorder)
...
def lininterpbetweenregularpoints(existy, interval):
    ...
    return numpy.concatenate(
        [arr + start for arr, start in zip(o, existy[:-1])] + [existy[-1:]]
    )


def interpwithinarr(
    existind, existy, order=3, interpplotax=None, interpcols=["k", "r"]
):
    if order == 1:
...
def savgolsmooth(
    x, nptsoneside=7, order=4, dx=1.0, deriv=0, binprior=0
):  # based on scipy cookbook. x is 1-d array, window is the number of points used to smooth the data, order is the order of the smoothing polynomial, will return the smoothed "deriv"th derivative of x
    if nptsoneside <= 1:
        return x
    if binprior > 1:
        origlen = len(x)
        x = numpy.array(
            [
                x[i * binprior : (i + 1) * binprior].mean()
                for i in range(origlen // binprior)
            ]
        )
        dx *= binprior
    side = numpy.uint16(max(nptsoneside, numpy.ceil(order / 2.0)))
    s = numpy.r_[2 * x[0] - x[side:0:-1], x, 2 * x[-1] - x[-2 : -1 * side - 2 : -1]]
    # a second order polynomal has 3 coefficients
    b = numpy.mat(
        [[k ** i for i in range(order + 1)] for k in range(-1 * side, side + 1)]
    )
    m = numpy.linalg.pinv(b).A[
        deriv
    ...
    if binprior > 1:
        ...
        xr = numpy.concatenate(
            [ia * (b - a) + a for a, b in zip(smooth_data[:-1], smooth_data[1:])]
        )
        xr = numpy.concatenate(
            [
                (smooth_data[1] - smooth_data[0]) * ia[: binprior // 2]
                + smooth_data[0],
                xr,
                (smooth_data[-1] - smooth_data[-2]) * ia[: binprior // 2]
                + smooth_data[-1],
            ]
        )
        smooth_data = numpy.concatenate(
            [
                xr,
                (smooth_data[-1] - smooth_data[-2]) * ia[: origlen - len(xr)]
                + smooth_data[-1],
            ]
        )
    return smooth_data
...
class fitfcns:  # datatuples are x1,x2,...,y
    ...
    def genfit(
        ...
        for (
            ...
            datatuple
        ):  # if the numerical data is given as a list or tuple then convert to arrays. regardless convert to float64 because leastsq REQUIRES THIS
            datatuple = (
                datatuple[0:i] + tuple([numpy.float64(arr)]) + datatuple[i + 1 :]
            )
            i = i + 1
        while self.performfit:
            ...
            if not optimizerfcn is None:
                ...
                if not fitout[4] in [1, 2]:
                    ...
                    try:
                        self.sigmas = scipy.array(
                            [self.covmat[i, i] for i in range(len(self.sigmas))]
                        )
                    except:
                        pass
...
def Lorentzian(
    pars, x
):  # defined in nontraditional way so that pars[2] is the peak height
    return pars[2] / (1 + ((x - pars[0]) / pars[1]) ** 2)


def GaussLorentz(pars, x):
    gw = min(max(pars[3], 0.0), 1.0)
    return gw * Gaussian(pars, x) + (1.0 - gw) * Lorentzian(pars, x)


def GaussHalfLorentz(pars, x):
    return 0.5 * Gaussian(pars, x) + 0.5 * Lorentzian(pars, x)


PeakFcnLibrary = {
    "Gaussian": Gaussian,
    "Lorentzian": Lorentzian,
    "GaussHalfLorentz": GaussHalfLorentz,
}


def fitpeakset(
    X, Y, initpars, peakfcn, negpeaks=True, optimizerfcn=None, nsigcut=3.0
):  # peak function must be a function that accepts a list of 3 parameters (the reshape 3 needs to be changed if num params differs)
    numgauss = len(initpars)
    if numgauss == 0:
        return (numpy.float32([]), numpy.float32([]), 0.0)
    if nsigcut is None:
        imin = 0
        imax = len(X)
    else:
    ...
    while repeatwithpkremoved:
        ...
        finalparamsshaped = numpy.reshape(
            finalparams, (len(finalparams) // initpars.shape[1], initpars.shape[1])
        )
        if negpeaks:
    ...
    sigmashaped = numpy.reshape(
        sigmas, (len(finalparams) // initpars.shape[1], initpars.shape[1])
    )
...
def arrayzeroind1d(arr, postoneg=False, negtopos=True):
    ...
    return (1.0 * zeroind * arr[(zeroind + 1,)] - (zeroind + 1) * arr[(zeroind,)]) / (
        arr[(zeroind + 1,)] - arr[(zeroind,)]
    )  # returns array of the floating point "index" linear interpolation between 2 indeces


def clustercoordsbymax1d(
    arr, pkind, critsepind
):  # results will be sorted. wherever there are peak indeces too close together. the peak index next to the peak index with highest arr value gets removed
    pkind.sort()
    ...
    while indindslow.size > 0:
        ...
        if (
            arr[pkind[indindslow[maxindindindlow]]]
            > arr[pkind[indindshigh[maxindindindhigh]]]
        ):
...
def peaksearch1dSG(
    ...
    ifirstder = savgolsmooth(
        x, nptsoneside=firstdernpts, order=firstderorder, dx=dx, deriv=1
    )
    ...
    if negpeaks:
        ...
        temp = numpy.where(
            x[(numpy.uint32(numpy.round(zeroind)),)] < (-1 * critpeakheight)
        )
        fullpkind = numpy.append(fullpkind, zeroind[temp])
    if fullpkind.size == 0:
        return fullpkind
    ...
    if critcurve is not None:
        isecder = savgolsmooth(
            x, nptsoneside=secdernpts, order=secderorder, dx=dx, deriv=2
        )
        temp = numpy.where(
            numpy.abs(isecder[(numpy.uint32(numpy.round(pkind)),)]) > (critcurve)
        )
...
def removesinglepixoutliers(arr, critratiotoneighbors=1.5):
    c = numpy.where(
        (arr[1:-1] > (critratiotoneighbors * arr[:-2]))
        * (arr[1:-1] > (critratiotoneighbors * arr[2:]))
    )
    c0 = c[0] + 1
...
def findlinearsegs(
        ...
        [
            (
                ...
                or (
                    numpy.all(
                        numpy.abs(dy[i:] - dy[i])
                        < max(numpy.abs(dy[i] * dydev_frac), dydev_abs)
                    )
                    and (len(dy) - i,)
                    or (
                        numpy.where(
            ...
            for i in range(len(dy) - dydev_nout)
        ]
    )
    ...
    if len(lenconstdy) == 0:
        ...
        lendn = savgolsmooth(
            numpy.float32(lenconstdy),
            nptsoneside=npts_SGconstdy,
            order=1,
            dx=1.0,
            deriv=1,
            binprior=0,
        ...
        if numpy.any(
            lenconstdy[: npts_SGconstdy + 1] >= lenconstdy[npts_SGconstdy + 1]
        ):
        ...
        istart_segs[(istart_segs > len(y) - 1 - npts_SGconstdy * 2)] = (
            len(y) - 1 - npts_SGconstdy * 2
        )
        istart_segs += (
            numpy.array(
                [
                    numpy.argmax(
                        lenconstdy[i - npts_SGconstdy * 2 : i + npts_SGconstdy * 2]
                    )
                    for i in istart_segs
                ]
            )
            - npts_SGconstdy * 2
        )
        istart_segs = numpy.array(
            clustercoordsbymax1d(lenconstdy, istart_segs, dn_segstart)
        )
        istart_segs = istart_segs[
            lenconstdy[istart_segs] >= dydev_nout / maxfracoutliers
        ]
        #
    ...
    if not critdy_fracmaxdy is None:
        istart_constsegs, len_constsegs, garb, garb = findzerosegs(
            dy,
            critdy_fracmaxdy,
            dydev_nout,
            dn_segstart,
            ...
            plotbool=plotbool,
            dx=1.0,
            maxfracoutliers=maxfracoutliers,
        )
        #        if len(istart_constsegs)>0:
        #            print istart_constsegs, len_constsegs
        temp = [
            [i, l]
            for i, l in zip(istart_constsegs, len_constsegs)
            if numpy.min((istart_segs - i) ** 2) > dn_segstart ** 2
        ]
        if len(temp) > 0:
    ...
    fitdy_segs, fitinterc_segs = numpy.array(
        [
            ...
            for i, l in zip(istart_segs, len_segs)
        ]
    ).T
    if plotbool:
        pylab.figure()
        pylab.plot(y)
        cols = ["g", "c", "y", "m"]
        for count, (i, l, fd, fy0) in enumerate(
            zip(istart_segs, len_segs, fitdy_segs, fitinterc_segs)
        ):
            if l == max(len_segs):
                c = "r"
            else:
    ...
    return (
        ...
        fitinterc_segs,
        dy,
    )  # fit intercept is wrt the beginning of the array, index=0 not x=0


def findmatchinglinearsegs(
    y,
    dydev_frac,
    dydev_nout,
    dn_segstart,
    ...
    istart_segs += (
        numpy.array(
            [
                numpy.argmax(lenconstdy[i - nptstemp * 2 : i + nptstemp * 2])
                for i in istart_segs
            ]
        )
        - nptstemp * 2
...
def findzerosegs(
    ...
    lenzeroy = numpy.array(
        [
            (
                numpy.all(numpy.abs(y[i:]) <= yzero_maxfrac)
                and (len(y) - i,)
                or (numpy.where(numpy.abs(y[i:]) > yzero_maxfrac)[0][:ydev_nout][-1],)
            )[0]
            for i in range(len(y) - ydev_nout)
        ]
    )
    nptstemp = 2
    lendn = savgolsmooth(
        numpy.float32(lenzeroy),
        nptsoneside=nptstemp,
        order=1,
        dx=1.0,
        deriv=1,
    ...
    istart_segs += (
        numpy.array(
            [
                numpy.argmax(lenzeroy[i - nptstemp * 2 : i + nptstemp * 2])
                for i in istart_segs
            ]
        )
        - nptstemp * 2
...
def calcsegind_dlist(dlist, SGpts=8):
    ...
    for d in dlist:
        d["Ewe(V)_dtSG"] = savgolsmooth(
            d["Ewe(V)"], nptsoneside=SGpts, order=1, dx=d["dt"], deriv=1, binprior=0
        )
        rising = d["Ewe(V)_dtSG"] >= 0
        d["segind"] = numpy.empty(len(rising), dtype="uint16")
        endptcorr = int(
            1.5 * SGpts
        )  # remove possibiliies of segment breaks within 1.5SGpts of the edges
        ...
        for si, i0 in zip(range(d["segind"].max() + 1), inds[:-1]):
            ...
            d["segprops_dlist"][-1]["dEdt"] = d["Ewe(V)_dtSG"][inds2][
                SGpts:-SGpts
            ].mean()
...
def manualsegind_dlist(dlist, istart=[0], SGpts=8):
    ...
    for d in dlist:
        ...
        for count, (i0, i1) in enumerate(zip(istart, iend)):
            ...
            d["segprops_dlist"][-1]["rising"] = d["segprops_dlist"][-1]["dEdt"] > 0.0
...
def SegSG_dlist(dlist, SGpts=10, order=1, k="I(A)"):
    ...
    for d in dlist:
        ...
        for segd in d["segprops_dlist"]:
            inds = segd["inds"]
            d[kSG][inds] = numpy.float32(
                savgolsmooth(
                    d[k][inds], nptsoneside=SGpts, order=order, deriv=0, binprior=0
                )
            )
...
def calcdiff_stepill(
    d,
    interd,
    ikey="Illum",
    ykeys=["Ewe(V)"],
    xkeys=["t(s)", "I(A)"],
    illfracrange=(0.4, 0.95),
    darkfracrange=(0.4, 0.95),
):
    if isinstance(ikey, list) or isinstance(ikey, numpy.ndarray):
        ...
        illum = d[ikey] != 0
    istart_len_calc = lambda startind, endind, fracrange: (
        startind + numpy.floor(fracrange[0] * (endind - startind)),
        numpy.ceil((fracrange[1] - fracrange[0]) * (endind - startind)),
    )
    ...
    interd["rawselectinds"] = numpy.int32(
        numpy.round(getdarkvals(numpy.arange(len(d[ikey]))))
    )
    for k in (
        xkeys + ykeys
    ):  # these are len 1 arrays that will get saved as itnermediate data
...
def calcdiff_choppedill(
    ...
    dark_istart, dark_len = istart_len_calc(
        numpy.concatenate([[darkstart], fallinds]),
        numpy.concatenate([riseinds, [darkend]]),
        darkfracrange,
    )
    ...
    for k in xkeys + ykeys:
        ...
        interd[k + "_dark"] = getdarkvals(d[k])
    for k in ykeys:
        interd[k + "_illdiff"] = interd[k + "_ill"] - 0.5 * (
            interd[k + "_dark"] + numpy.append(interd[k + "_dark"], numpy.nan)[1:]
        )
...
def calcdiff_ill_caller(d, interd, ikey="Illum", thresh=0, **kwargs):
    ...
    try:
        if (
            len(riseinds) == 0
            or len(fallinds) == 0
            or (len(riseinds) == 1 and len(fallinds) == 1 and riseinds[0] > fallinds[0])
        ):
...
def calc_comps_multi_element_inks(
    platemapdlist,
    cels_set_ordered,
    conc_el_chan,
    key_append_conc=".Loading",
    key_append_atfrac=".AtFrac",
    tot_conc_label=None,
):
    ...
    inkchannelsused = ["A", "B", "C", "D", "E", "F", "G", "H"][
        : conc_el_chan.shape[1]
    ]  # grab the channels used
    ...
    if key_append_atfrac is None:
        ...
        concarr = numpy.array(
            [[d[k] for k in cels_set_ordered_conc] for d in platemapdlist]
        )
        if not tot_conc_label is None:
            [
                d.update([(tot_conc_label, carr.sum())])
                for d, carr in zip(platemapdlist, concarr)
            ]
...
def ternary_comp_to_cart(a, b, c):
    return 1.0 - a - b / 2.0, numpy.sqrt(3) * b / 2.0


def quaternary_comp_to_cart(a, b, c, d):
    return (
        1.0 - a - b / 2.0 - d / 2.0,
        b / 2.0 * (3.0 ** 0.5) + d / 2.0 / (3.0 ** 0.5),
        d * (2.0 ** 0.5) / (3.0 ** 0.5),
    )


def filterbydistancefromline(
    arr_of_xyz,
    xyz1,
    xyz2,
    critdist,
...
):  # if N points in D dimmensions arr_ is NxD, xyz1 and 2 are D, critdist is D-dimmension Euclidean, lineparameter is 0 to 1 from xyz 1 to xyz2 and can be out of that range
    # see http://mathworld.wolfram.com/Point-LineDistance3-Dimensional.html
    if (
        is_composition
    ):  # probably cheaper to not do coordinate transform and just multiply by the distance ratio for the ndim, but this allows quat. to be done with 3D math
    ...
    distfromlin = numpy.array(
        [
            numpy.linalg.norm(numpy.cross(xyz2 - xyz1, xyz1 - xyz))
            / numpy.linalg.norm((xyz2 - xyz1))
            for xyz in arr_of_xyz
        ]
    )
    if betweenpoints:
        lineparameter = numpy.array(
            [
                -numpy.inner(xyz2 - xyz1, xyz1 - xyz)
                / numpy.linalg.norm((xyz2 - xyz1)) ** 2
                for xyz in arr_of_xyz
            ]
        )
        if invlogic:
            ...
            inds = numpy.where(
                (distfromlin <= critdist) & (lineparameter >= 0) & (lineparameter <= 1)
            )[0]
    else:
        lineparameter = numpy.array(
            [
    ...
    if returnonlyinds:
        ...
        if numnansamples > 0:
            temp = (
                numpy.ones(len(notnoninds) + numnansamples, dtype="float64") * numpy.nan
            )
        ...
        return {
            "select_inds": inds,
            "dist_from_line": distfromlin,
            "norm_dist_along_line": lineparameter,
        }


def twotheta_q(
    q, wl=0.15418, units="deg"
):  # q is scattering vector in 1/nm.  wl is wavelength in nm. return float32 2Theta value in degrees unless units='rad'
    if units == "rad":
        return numpy.float32(2.0 * numpy.arcsin(wl * q / (4.0 * numpy.pi)))
    else:
        return numpy.float32(
            2.0 * numpy.arcsin(wl * q / (4.0 * numpy.pi)) * 180.0 / numpy.pi
        )
...
def q_twotheta(
    twotheta, wl=0.15418, units="deg"
):  # units are those of twotheta, wl in nm, q in 1/nm
    if units == "deg":
        twotheta *= numpy.pi / 180.0
    return 4 * numpy.pi * numpy.sin(twotheta / 2.0) / wl


def integrate_spectrum_with_piecewise_weights(
    spectrumx,
    spectrumy,
    weightsx,
    weightsy,
    below_val=0,
    above_val=0,
    return_completed_weights=False,
):
...
def linear_2d_extrapolation(
    xy, z, xy_to_extrapolate, num_pts_in_lin_fit=6
):  # returns array of 2d interp values using linear interp of num_pts_in_lin_fit (up to num_pts_in_lin_fit x 2 if need be). Arguments mimic griddata
    x, y = xy.T
    new_z = []
    for xi, yi in xy_to_extrapolate:
        dist_tups = sorted(
            [
                ((xv - xi) ** 2 + (yv - yi) ** 2, count)
                for count, (xv, yv, zv) in enumerate(zip(x, y, z))
                if not np.isnan(zv)
            ]
        )
        for extra_pts in range(
            num_pts_in_lin_fit + 1
        ):  # try up to twice as many points if necessary to get unique x and y values
            fitinds = [tup[1] for tup in dist_tups[: num_pts_in_lin_fit + extra_pts]]
            xf = x[fitinds]
            yf = y[fitinds]
            zf = z[fitinds]
            if (
                (xf[0] == xf).prod() + (yf[0] == yf).prod()
        ...
        new_z += [(fitcoefs * np.array([xi, yi, 1.0])).sum()]
...
def linear_1d_extrapolation(x, z, x_to_extrapolate, num_pts_in_lin_fit=6):
    ...
    for xi in x_to_extrapolate:
        dist_tups = sorted(
            [
                ((xv - xi) ** 2, count)
                for count, (xv, zv) in enumerate(zip(x, z))
                if not np.isnan(zv)
            ]
        )
        for extra_pts in range(
