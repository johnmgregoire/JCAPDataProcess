...
class ssrl_integrator:
    def __init__(
        ...
        self.calib_file = os.path.join(
            self.import_path, *[fn for fn in fns if "calib" in fn]
        )
    ...
    def parse_calib(self):
        ...
        with open(self.calib_file) as fl:
            for line in fl.readlines():
                if "=" in line:
                    try:
                        self.params[line.split("=")[0]] = self.conv(
                            line.split("=")[1].strip("\n")
                        )
                    except ValueError:
                        print (
    ...
    def perform_integration(self):
        for fn in self.tif_fns:
            ...
            if not self.testmode:
                ...
                fn_1d = os.path.join(
                    self.import_path, "ana_1_" + fn.strip(".tif") + "_processed.csv"
                )
                with open(fn_1d, "wb") as f1d:
                    np.savetxt(
                        f1d,
                        np.array([Qchi["Qv"] * 10e10, Qchi["Intens"]]).T,
                        delimiter=",",
                        header="q.nm,intensity.counts",
                        comments="",
                        fmt="%.5f,%.5f",
                    )
        ...
        with open(picklepfn, "wb") as qchi2binary:
            # save the qchi using numpy
            d = {
                "q_invA": Qchi["Qv"] * 10e10,
                "q_invnm": Qchi["Qv"] * 10e9,
                "chi_deg": Qchi["chi"] + 90,
                "chi_rad": (Qchi["chi"] + 90) / 180.0 * np.pi,
            }
    ...
    def read_and_integrate_tiff(self, filen):
        ...
        model.setFit2D(
            d,
            self.params["bcenter_x"],
            self.params["bcenter_y"],
            tilt,
            rot,
            self.params["pxs"],
            self.params["pxs"],
