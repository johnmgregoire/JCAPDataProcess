...
class setup_rcp_and_exp_ssrl:
    ...
    def read_ssrl_calib(
        self, line_inds=None, delim="="
    ):  # add in the calib values to run parameters, although this calib may need to be manually overridden during the
        ...
        for l in lines:
            ...
            self.run_params_dict[
                ...
                v.strip()
            )  # calib params are string vaslues even for the numbers so convert before sending to functions
    ...
    def setup_file_dicts(self):
        ...
        self.expdict["description"] = "%s run on plate_id %s" % (
            self.datatype,
            self.plate_idstr,
        )
        ...
        # messy fix for : : bug
        self.add_calib_file = lambda fn: [
            d.update({fn: filed_createflatfiledesc({"file_type": "ssrl_calib_file"})})
            for d in calibtechd
        ]
        self.add_speccsv_file = lambda fn: [
            d.update(
                {fn: filed_createflatfiledesc({"file_type": "ssrl_spec_csv_file"})}
            )
            for d in csvtechd
        ]
            ...
            d.update(
                {
                    fn: filed_createflatfiledesc(
                        {"file_type": "ssrl_meta_csv_file", "sample_no": sample_no}
                    )
                }
            )
            for d in metatechd
        ...
        self.add_tif_file = lambda fn, sample_no: [
            d.update(
                {
                    fn: filed_createflatfiledesc(
                        {"file_type": "ssrl_mar_tiff_file", "sample_no": sample_no}
                    )
                }
            )
            for d in tiftechd
    ...
    def add_all_files(self, testmode):
        ...
        self.speccsv_fns, self.tif_fns = (
            [fn for fn in fns if ".txt" in fn],
            [fn for fn in fns if ".tif" in fn],
        )
        ID_spec = np.array(
            [j.split("_")[1].strip(".txt") for j in self.speccsv_fns]
        ).astype(np.int)
        ID_tif = np.array([j.split("_")[1].strip(".tif") for j in self.tif_fns]).astype(
            np.int
        )
    ...
    def save_rcp_exp(self, testmode):
        ...
        saveexp_txt_dat(
            ...
            runtodonesavep=self.runfolderpath,
            savefolder=self.runfolderpath,
        )
        # print 'exp file saved to ', dsavep
        if not testmode:
            shutil.copy(
                self.calib_path, os.path.join(self.runfolderpath, self.calib_fn)
            )
            shutil.copy(
                os.path.join(self.import_path, self.shellfn),
                os.path.join(self.runfolderpath, self.shellfn),
            )
            for fn in self.speccsvfns:
                shutil.copy(
                    os.path.join(self.import_path, fn),
            ...
            for fntif, fncsv in zip(self.tif_fns, self.speccsv_fns):
                shutil.copy(
                    os.path.join(img_path, fntif),
                    os.path.join(self.runfolderpath, fntif),
                )
                shutil.copy(
                    os.path.join(img_path, fncsv),
                    os.path.join(self.runfolderpath, fncsv),
                )
...
importclass = setup_rcp_and_exp_ssrl(
    import_path,
    calib_path,
    rcpext=".done",
    expext=".done",
