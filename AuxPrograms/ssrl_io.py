...
def get_externalimportdatad_ssrl_batchresults(
    p, p_processed=None, askforprocessed=True, parent=None
):  # assume folder is for single .rcp
    if p_processed is None and askforprocessed:
        p_processed = str(
            mygetdir(
                parent=parent,
                xpath=p,
                markstr='Folder containing processed data, i.e. parent folder of "pck2d"',
            )
        )
    ...
    if len(csvfns) == 1:  # should only be 1 file that is the summary of spec info
        ...
        rcpd["file_dlist"] += [
            {
                "tech": "files_technique__SSRL",
                "type": "csv_summary_files",
                "fn": fn,
                "fval": "ssrl_spec_csv_file;",
                "folderpath": p,
            }
        ]
        ...
        multirun_ana_files[an_name] = [
            {
                "type": "fom_files",
                "fn": xrfcsvfn,
                "fval": "csv_fom_file;%s;1;%d" % (headline, len(gr["compositions"])),
                "roi_keys_to_copy": roi_keys_to_copy,
                "headline": headline,
            }
        ]
    ...
    if os.path.isdir(imdir):
        ...
        ana_files_generator = (
            lambda fn: {
                "Analysis__SSRL_Integrate": [
                    {
                        "fn": fn.replace(".tif", "_integrated.csv"),
                        "type": "pattern_files",
                        "fval": "ssrl_csv_pattern_file;q.nm,intensity.counts;1;%d;"
                        % npts,
                        "h5arrind": count,
                        "h5dataset": "qcounts",
                    }
            ...
            if q_subbcknd is None
                ...
                "Analysis__SSRL_Process": [
                    {
                        "fn": fn.replace(".tif", "_processed.csv"),
                        "type": "pattern_files",
                        "fval": "ssrl_csv_pattern_file;q.nm_processed,intensity.counts_processed;1;%d;"
                        % npts_subbcknd,
                        "h5arrind": count,
                        "h5dataset": "qcounts_subbcknd",
                    }
                ],
            }
        )
        ...
        rcpd["file_dlist"] += [
            {
                ...
                "type": "pck2d_files",
                "fn": pck2dvalsfn,
                "fval": "ssrl_vals_pck_file;",
                "folderpath": os.path.split(pck2dvalsp)[0],
            }
        ]
        pck2d_fns = sorted(
            [fn for fn in os.listdir(pck2dfolder) if fn.endswith("_chiq.pck")]
        )  # assuem this sorting gives the same ordering as the xys
        rcpd["file_dlist"] += [
            {
                "xyarr": xyarr,
                "tech": "files_technique__SSRL",
                ...
                "fn": fn,
                "fval": "ssrl_pck_file;",
                "folderpath": pck2dfolder,
            }
            for fn, xyarr in zip(pck2d_fns, xy_images)
        ]
    rcpdlist = [rcpd]
