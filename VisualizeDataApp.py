...
class visdataDialog(QDialog, Ui_VisDataDialog):
    def __init__(self, parent=None, title="", folderpath=None, GUIMODE=GUIMODE):
        ...
        self.AnaExpFomTreeWidgetFcns = treeclass_anaexpfom(
            self.AnaExpFomTreeWidget, self.SummaryTextBrowser
        )
        ...
        button_fcn = [
            ...
            (
                self.FilterFomPlotBySelectionPushButton,
                self.replot_with_sample_selection_filter,
            ),
        ...
        self.AnaExpFomTreeWidget.itemClicked[QTreeWidgetItem, int].connect(
            self.processclick_selecttreeitem
        )
        ...
        self.compPlotMarkSelectionsCheckBox.released.connect(self.plotfom)
        ...
        self.inkjetconcentrationadjustment = (
            False if not self.GUIMODE else "ask_if_appropriate"
        )  #'ask_if_appropriate' for dialog, or False/True
        self.plotwsetup()
        self.clearall()
    ...
    def createfilenamefilter(self, filterstr=None):
        if filterstr is None:
            ans = userinputcaller(
                self,
                inputs=[("filename search string", str, "")],
        ...
        if self.SelectTreeFileFilterTopLevelItem is None:
            self.SelectTreeFileFilterTopLevelItem = QTreeWidgetItem(
                ["required filename str"], 0
            )
    ...
    def importfrominfo(self):
        if (
            not self.GUIMODE
        ):  # this is for users to find exp/ana so not valid for non gui mode
            return
    ...
    def importana(
        ...
        summlines += [
            ...
            for anak in self.sorted_ana_exp_keys()
        ]
        ...
        readandformat_anafomfiles(
            ...
            platemap4keys_default=self.platemap4keys_default,
        )
    ...
    def process_multielementink(
        ...
        if errorbool:
            ...
            ):  # don't display error messgage if set to True for batch purposes
        ...
        calc_comps_multi_element_inks(
            rund["platemapdlist"],
            cels_set_ordered,
            conc_el_chan,
            key_append_conc="",
            key_append_atfrac=None,
        )
    ...
    def importexp(
        ...
        for runk, rund in self.expfiledict.iteritems():
            ...
            if not "platemapdlist" in rund.keys():
                if True in [
                    ...
                    rund["platemapdlist"] = copy.copy(
                        [
                            tup[1]
                            for tup in platemap_plateid_dlist_list
                            if tup[0] == rund["parameters"]["plate_id"]
                        ][0]
                    )
                else:
                    selectpmfcn = lambda s, xpath: mygetopenfile(
                        parent=self,
                        xpath=xpath,
                    ...
                    if self.defaultplatemapCheckBox.isChecked():
                        pmpath = getplatemappath_plateid(
                            str(rund["parameters"]["plate_id"]), erroruifcn=selectpmfcn
                        )
                    else:
                    ...
                    if len(pmpath) == 0:
                        ...
                        platemap_plateid_dlist_list += [
                            (
                                rund["parameters"]["plate_id"],
                                readsingleplatemaptxt("", lines=pmlines),
                            )
                        ]  # path not used here because passing lines
                        rund["platemapdlist"] = copy.copy(
                            platemap_plateid_dlist_list[-1][1]
                        )
                        ps = os.path.normpath(pmpath)
                        if not ps in platemapfilenamestr:
                            platemapfilenamestr = ",".join(
                                [platemapfilenamestr, ps]
                            ).strip(",")
            rund["platemapsamples"] = [d["sample_no"] for d in rund["platemapdlist"]]
            multielementink_concentrationinfo_bool = bool(
                self.inkjetconcentrationadjustment
            )
            ans = getelements_plateidstr(
                str(rund["parameters"]["plate_id"]),
                multielementink_concentrationinfo_bool=multielementink_concentrationinfo_bool,
            )
            if multielementink_concentrationinfo_bool and not ans is None:
                ...
                if not tup_multielementink is None:
                    ...
                    cels_set_ordered = self.process_multielementink(
                        rund,
                        errorbool,
                        tupormessage,
                        inkjetconcentrationadjustment=self.inkjetconcentrationadjustment,
            ...
            if not els is None and len(set(els)) < len(
                els
            ...
            if masterels is None:
                ...
                if self.GUIMODE:
                    idialog = messageDialog(
                        self,
                        "Would you like to modify platemap so %s permuted to match previous plate with elements %s?"
                        % (",".join(els), ",".join(masterels)),
                    )
                    doit = idialog.exec_()
                else:
                    doit = True
                if doit:
                    ...
                    for d in rund["platemapdlist"]:
                        for let, el in zip(channelsused, masterels):
                            d[let] = d[
                                "orig" + channelsused[els.index(el)]
                            ]  # this allows plates with different permutations of the same set fo elements (as the first plate to be read here) to have their platemaps permuted to match.
                else:
                    masterels = [
                        "A",
                        "B",
                        "C",
                        "D",
                    ]  # this will keep any subsequent .exp from matching the masterels and when plots are made they are just vs the platemap channels not the printed elements
            else:  # this patholigcal case of having different sets of elements in the same exp/ana is not handled for >4 element prints
                if self.GUIMODE:
                    idialog = messageDialog(
                        self,
                        "WARNING: %s has elements %s but elements %s were already loaded"
                        % (runk, ",".join(els), ",".join(masterels)),
                    )
                    idialog.exec_()
                masterels = ["A", "B", "C", "D"]
        if masterels is None or masterels == ["A", "B", "C", "D"]:
            ...
            isinstance(self.inkjetconcentrationadjustment, bool)
            and self.inkjetconcentrationadjustment
            ...
            #                self.platemap4keys_default=self.ellabels[:4]
                ...
                if ellabelsforquatplots == "ask":
                    if self.GUIMODE:
                        ans = userinputcaller(
                            ...
                            title="Enter element labels for quaternary plots or close to use platemap",
                            cancelallowed=True,
                        )
                    else:
                        ans = None
                ...
                if not ans is None:
                    ...
                    self.platemap4keys_default = (
                        self.ellabels
                    )  # the calculated comps for these 4 elements will now be considered as 4 platemap channels
                    self.ellabelsLineEdit.setText(",".join(self.ellabels))
        else:
            self.remap_platemaplabels(
                newellabels=masterels
            )  # self.ellabels assigned in here
            self.platemap4keys_default = self.ellabels[:4]
            if len(self.ellabels) > 4:
                ...
                ):  # either exp or no ana__ has specified which 4 to use so ask or allow to be specified
                    if ellabelsforquatplots == "ask":
                        ans = userinputcaller(
                            self,
                            inputs=[
                            ...
                            title="Enter 4 elements to use for plots or close to use platemap",
                            cancelallowed=True,
                        )
                    elif isinstance(ellabelsforquatplots, list):
                        ans = ellabelsforquatplots
        ...
        if fromana:
            ...
            nonabcdsummlines = [
                "-".join([anak, get4elementsetstr(self.anafiledict[anak])])
                for anak in self.sorted_ana_exp_keys()
            ]
            nonabcdsummlines = [
                l for l in nonabcdsummlines if not l.endswith("-")
            ]  # remove the regular ABCD ones
            if len(nonabcdsummlines) > 0:
                summlines = (
                    ["4-plot elements (if not ABCD):"] + nonabcdsummlines + summlines
                )
        else:
            self.clearvisuals()
            summlines = []
            ...
            self.AnaExpFomTreeWidgetFcns.appendFom(
                self.l_fomnames[-1], self.l_csvheaderdict[-1]
            )
            self.setupfilterchoices()
            self.updatefomplotchoices()
            self.fillxyoptions(clear=True)
    ...
    ):  # should work for up to 8 elements, only the length of ellables will be used
        if not newellabels is None:
            if len(newellabels) < 4:
                self.ellabels = (
                    newellabels + ["A", "B", "C", "D"][len(newellabels) :]
    ...
    def openontheflyfolder(
        self, folderpath=None, platemappath=None, plateidstr=None
    ):  # assume on -the-fly will never involve a .zip
        ...
        if platemappath is None:
            if plateidstr is None:
                plateidstr = os.path.split(os.path.split(folderpath)[0])[1].rpartition(
                    "_"
                )[2][:-1]
            ...
            while els is None:
                ans = userinputcaller(
                    self,
                    inputs=[("plate_id", str, "")],
                    title="Enter plate_id for the on-the-fly folder",
                    cancelallowed=True,
                )
                if ans is None:
                    plateidstr = None
        ...
        self.expfiledict["run__1"]["platemapdlist"] = readsingleplatemaptxt(
            "", lines=pmlines
        )
        self.expfiledict["run__1"]["platemapsamples"] = [
            d["sample_no"] for d in self.expfiledict["run__1"]["platemapdlist"]
        ]
        ...
        self.expfiledict["run__1"]["parameters"]["plate_id"] = (
            0 if plateidstr is None else int(plateidstr)
        )
        self.platemapfilenameLineEdit.setText(os.path.normpath(platemappath))
        self.remap_platemaplabels(
            newellabels=els
        )  # if els weren't found the argument will be None so ellables will still be the default from earlier in this function and the remap wont' do anything to a standard platemap
        self.anafolder = ""
        self.anafiledict = {}
        self.AnaExpFomTreeWidgetFcns.initfilltree(self.expfiledict, self.anafiledict)
    ...
    def updateontheflydata(self):
        ...
        try:
            ...
            return
        self.lastmodtime, d_appended = createontheflyrundict(
            self.expfiledict, self.expfolder, lastmodtime=self.lastmodtime
        )
        self.setupfilterchoices()
    ...
    def updatefomdlist_plateruncode(self, inds=None, createnewfromexp=False):
        if createnewfromexp:
            ...
            self.l_fomdlist = [
                [
                    dict({}, anaint=0, runint=runint, plate_id=pl, sample_no=smp)
                    for runint, pl, smp in runint_pl_smp
                ]
            ]
        ...
        for ind in inds:
            ...
            for d in fomdlist:
                ...
                if not (
                    ...
                    try:
                        pmd = rund["platemapdlist"][
                            rund["platemapsamples"].index(d["sample_no"])
                        ]
                        for k in (
                            self.ellabels
                            + ["code", "x", "y"]
    ...
    def fillxyoptions(self, clear=False):
        ...
        if (
            not clear
        ):  # arr options and then fom options except if appending then don't change existing indeces and add the new stuff, which presumably will be iether fom or arr opts
    ...
    def setupfilterchoices(self):
        ...
        sets_pl_ru_co = [
            set([d[k] for fomdlist in self.l_fomdlist for d in fomdlist])
        ...
        sets_pl_ru_te_ty_co += [
            sets_pl_ru_co[2]
        ]  # code stays as int for now and is convereted below
                ...
                for typek, typed in techd.iteritems():
                    ...
                    for filek in sorted(typed.keys()):
                        ...
                        if numpy.isnan(smp) or not smp in rund["platemapsamples"]:
                            ...
                            codes += [
                                rund["platemapdlist"][
                                    rund["platemapsamples"].index(smp)
                                ]["code"]
                            ]
                    if (
                        len(codes) == 0
                    ):  # the exp/rcp don't have sample-specific information so just allow all codes through
                        codes = sorted(
                            list(set([smpd["code"] for smpd in rund["platemapdlist"]]))
                        )
        ...
        for mainitem in self.widgetItems_pl_ru_te_ty_co:
            ...
            for item in items:
                del item
        for i, (mainitem, setfromfoms) in enumerate(
            zip(self.widgetItems_pl_ru_te_ty_co[:-1], sets_pl_ru_te_ty_co)
        ):
            vals = sorted(
                list(set(map(operator.itemgetter(i), flattups)).union(setfromfoms))
            )
            for s in vals:
    ...
    def processclick_selecttreeitem(self, item, column):
        ...
        filed = copy.copy(
            filed
        )  # to avoid zipclass being incorproated into exp or anafiledict
        filed["path"] = p
    ...
    def updatefiltereddata(self):
        ...
        if self.SelectTreeFileFilterTopLevelItem is None:
            ...
            searchstrs = [
                str(self.SelectTreeFileFilterTopLevelItem.child(i).text(0)).strip()
                for i in range(self.SelectTreeFileFilterTopLevelItem.childCount())
                if bool(self.SelectTreeFileFilterTopLevelItem.child(i).checkState(0))
            ]
        self.filteredexpfilekeys = [
            list(l_keytup) + [filek]
            for pl, expkeytup in l_keytup
            for co, filek in zip(
                self.exp_keys_codearr_dict[(pl, expkeytup)],
                sorted(d_nestedkeys(self.expfiledict, expkeytup).keys()),
            )
            if (co in allowedvals and not (False in [s in filek for s in searchstrs]))
        ]
    ...
    def performontheflyfom(
        self,
    ):  # onthefly fom calc could be on existing (not on thefly) .exp so expfolder could be .zip
        self.analysisclass = AnalysisClasses[self.OnFlyAnaClassComboBox.currentIndex()]
        mainitem = self.widgetItems_pl_ru_te_ty_co[2]
        temp = [
            str(mainitem.child(i).text(0)).strip().partition("__")[2]
            for i in range(mainitem.childCount())
            if bool(mainitem.child(i).checkState(0))
        ]
        if len(temp) == 0:
        ...
        self.analysisclass.getapplicablefilenames(
            self.expfiledict, "", te, ty
        )  # empty "data usek" string will not filter and then filter by the first checked technique and type
        if self.SelectTreeFileFilterTopLevelItem is None:
            searchstrs = []
        else:
            searchstrs = [
        ...
        self.analysisclass.filedlist = [
            d
            for d in self.analysisclass.filedlist
            if not (False in [s in d["fn"] for s in searchstrs])
        ]
        checkbool, checkmsg = self.analysisclass.check_input(critfracapplicable=0.001)
        ...
        self.analysisclass.perform(
            None,
            expdatfolder=self.expfolder,
            anak="",
            zipclass=self.expzipclass,
            expfiledict=self.expfiledict,
        )
        self.l_fomdlist += [
            self.analysisclass.fomdlist
        ...
        self.l_csvheaderdict += [
            self.analysisclass.csvheaderdict
        ]  # this contains default plot info
    ...
    def filterandplotfomdata(self, plotbool=True):
        ...
        plateidallowedvals = [
            int(str(mainitem.child(i).text(0)).strip())
            for i in range(mainitem.childCount())
            if bool(mainitem.child(i).checkState(0))
        ]
        mainitem = self.widgetItems_pl_ru_te_ty_co[1]
        runintallowedvals = [
            int(str(mainitem.child(i).text(0)).lstrip("run__"))
            for i in range(mainitem.childCount())
            if bool(mainitem.child(i).checkState(0))
        ]
        mainitem = self.widgetItems_pl_ru_te_ty_co[-1]
        ...
        plotdinfo = []
        for fomdlist_index0, (usebool, fomdlist, fomnames, platemapkeys) in enumerate(
            zip(l_usefombool, self.l_fomdlist, self.l_fomnames, self.l_platemap4keys)
        ):  # the pmkeys here provides custom comps calcualtion without reassigning any pm channels
            ...
            if self.compCalcFromFOMCheckBox.isChecked():
                ...
                calc_comps__starts_contains_tups = [
                    (ellab, srchstr) for ellab in ellabs
                ]
            else:
            ...
            ]  # and not numpy.isnan(d[fomname]), don't do this so can do fomname swap without loss of samples
    ...
    def replot_with_sample_selection_filter(self, plotbool=True):
        filterinds = [
            ...
            for i, tup in enumerate(
                zip(self.fomplotd["fomdlist_index0"], self.fomplotd["fomdlist_index1"])
            )
    ...
    def updatefomplotchoices(self):
        ...
        self.numStdPlots = 0
        self.fomselectnames = sorted(
            list(set([nam for fomnames in self.l_fomnames for nam in fomnames]))
        )
        ...
        for count, s in enumerate(self.fomselectnames):
            self.fomplotchoiceComboBox.insertItem(
                count + 1, s
        ...
        keys = [
            ...
            % (
                count + 1,
                csvheaderdict["plot_parameters"][k]["fom_name"],
                csvheaderdict["anak"],
                k,
            )
    ...
    def plot_preparestandardplot(self, plotbool=True, loadstyleoptions=True):
        ...
        for mainitem, vals in zip(
            ...
            for i in range(mainitem.childCount()):
                ...
                item.setCheckState(
                    0, Qt.Checked if str(item.text(0)) in vals else Qt.Unchecked
                )
        ...
        fomnames = [
            self.fomplotchoiceComboBox.itemText(i)
            for i in range(self.fomplotchoiceComboBox.count())
        ]
    ...
    def fomstats(self):
        ...
        if len(fom) == 0:
            ...
            return
        tempfmt = (
            lambda x: ("%.2e" if x > 999.0 else ("%.4f" if x > 0.009 else "%.2e")) % x
        )
        strarr = []
        for fcn in [
            numpy.mean,
            ...
            if isinstance(fcn, float):
                strarr += [
                    [
                        ("%d" % (fcn * 100)) + "%",
                        tempfmt(numpy.percentile(fom, fcn * 100)),
                    ]
                ]
            else:
    ...
    def extractxy_ana(self, arrkeys, anaint, runint, smp):
        ...
        for count, k in enumerate(arrkeys):
            if k == "None":
                continue
            fn_filed_keyind_rawselind = [
                (
                    fn,
                    filed,
                    filed["keys"].index(k),
                    None
                    if not "rawselectinds" in filed["keys"]
                    else filed["keys"].index("rawselectinds"),
                )
                for fn, filed in fn_filed_tosearch
                if k in filed["keys"]
            ]
            if (
                len(fn_filed_keyind_rawselind) > 0
            ):  # ideally this is length 1 because if onlger that means foudn the same array multiple places and choosing the 1 found first
                ...
                arr2d = getarrs_filed(
                    os.path.join(self.anafolder, fn),
                    filed,
    ...
    def extractxy_exp(self, arrkeys, runint, smp):
        xyysubset = [None] * len(
            arrkeys
        )  # this is a subset of x, y and yr if any of those found in ana
        runk = "run__%d" % runint
        ...
        for count, k in enumerate(arrkeys):
            ...
            if (
                ...
                p, zipclass = ans
                arr2d = getarrs_filed(
                    p, filed, selcolinds=selcolinds, zipclass=zipclass
                )
    ...
    def extractxydata(self, arrkeys, filed=None):
        ...
        if not filed is None:
            keyinds = [
                filed["keys"].index(arrk) if arrk in filed["keys"] else None
                for arrk in arrkeys
            ]
            if keyinds[0] is None or (
                keyinds[1] is None and keyinds[2] is None
            ):  # no pari of x,y to plot
                return None
            selcolinds = [keyind for keyind in keyinds if not keyind is None]
            arr2d = getarrs_filed(
                filed["path"], filed, selcolinds=selcolinds, zipclass=filed["zipclass"]
            )
            plotdata = [[[], []], [[], []]]
        ...
        anaint, runint, smp = [
            self.l_fomdlist[i0][i1][k] for k in ["anaint", "runint", "sample_no"]
        ]
        ...
        while (
            look_in_upstream_ana
        ):  # keep looking in ana__ if they are linked through the select_ana parameter
            xyydlist1 = self.extractxy_ana(arrkeys, anaint, runint, smp)
            xyydlist2 = self.extractxy_ana(arrkeys, anaint, None, smp)
            for count, (d1, d2) in enumerate(
                zip(xyydlist1, xyydlist2)
            ):  # use the data from runint=None, which means files_multi_run, only if data from the runint,smp wasn't found
                if d1 is None and not d2 is None:
                    xyydlist1[count] = xyydlist2[count]
            for count, (d0, d1) in enumerate(
                zip(xyydlist, xyydlist1)
            ):  # use the data from runint=None, which means files_multi_run, only if data from the runint,smp wasn't found
                if (
                    d0 is None and not d1 is None
                ):  # only update the master xyydlist if something new found in this round of extractxy_ana. This allows differnet ana__ to contribute to x and y
                    xyydlist[count] = xyydlist1[count]
            if None in [
                v for v, arrk in zip(xyydlist, arrkeys) if arrk != "None"
            ]:  # still things to find that are not in run,fom or the ana looked at so far
                anak = "ana__%d" % anaint
                ...
                look_in_upstream_ana = (
                    False  # if already found things to plot go with that
                )
        if runint > 0:
            lefttogetinds = [
                count
                for count, (d, arrk) in enumerate(zip(xyydlist, arrkeys))
                if d is None and not arrk == "None"
            ]
            if len(lefttogetinds) > 0:
                xyysubset = self.extractxy_exp(
                    [arrkeys[i] for i in lefttogetinds], runint, smp
                )
                for i, d in zip(
                    lefttogetinds, xyysubset
                ):  # d might stil be None at this points
                    xyydlist[i] = d
        plotdata = [[[], []], [[], []]]
        for count, (xd, yd) in enumerate(
            [(xyydlist[0], xyydlist[1]), (xyydlist[0], xyydlist[2])]
        ):
        ...
        lab = self.customlegendfcn(
            getval("sample_no"),
            self.getellabels_pm4keys(self.l_platemap4keys[i0]),
            getval("comps"),
            getval("code"),
            getval("fom"),
            getval("xy"),
    ...
    def getxydata_smplist(
        self, smplist, pltlist
    ):  # only xy, the right-y box is ignored. This can take any sample list and find available x,y data the same way that would happen by clicking on the sample. this is motivated by udiscoverit export
        cbl = [
            self.xplotchoiceComboBox,
            self.yplotchoiceComboBox,
        ]
        ...
        fominds_smps = [
            numpy.where(
                (self.fomplotd["sample_no"] == smp) & (self.fomplotd["plate_id"] == plt)
            )[0]
            for smp, plt in zip(smplist, pltlist)
        ]
        xyinfodlist = []
        for smp, fominds in zip(smplist, fominds_smps):
            ...
            for fomind in fominds:
                ...
                anaint, runint, smp = [
                    self.l_fomdlist[i0][i1][k]
                    for k in ["anaint", "runint", "sample_no"]
                ]
    ...
    def get_xy_plate_info_browsersamples(
        self, saveudibool=True, ternary_el_inds_for_udi_export=None, savep=None
    ):
        ...
        if not ("plate_id" in keys and "sample_no" in keys):
            messageDialog(
                self, "aborting: browser must have plate_id and sample_no columns"
            ).exec_()
            return None
        ...
        if xyinfodlist is None or not (False in [d is None for d in xyinfodlist]):
            ...
            return None
        xyinfodlist = [
            dict(d, sample_no=smp, plate_id=plt)
            for d, smp, plt in zip(xyinfodlist, smplist, pltlist)
            if not d is None
        ]
        if not saveudibool:
            return xyinfodlist
            ...
            if count == 0:
                ...
                if max(distarr) > 0:
                    if not messageDialog(
                        self, "Not all x arrays are identical. Continue and use mean?"
                    ).exec_():
                        return
        ...
        for elinds in l_elinds:
            ...
            if savep is None:
                savep = mygetsavefile(
                    parent=self,
                    markstr="select start path and <els>.udi will be appended"
                    if ternary_el_inds_for_udi_export == "ALL"
                    else "Select path for udi file",
                )
    ...
    def getellabels_pm4keys(self, pmkeys):
        if not (
            False in [pmk in self.ellabels for pmk in pmkeys]
        ):  # pmkeys has a libreal interpreation and could already be ellabels in which case do nothing here
            return pmkeys
        ellabelinds = [
            ["A", "B", "C", "D", "E", "F", "G", "H"].index(pmk) for pmk in pmkeys
        ]
        return [
            self.ellabels[i] if i < len(self.ellabels) else "X" for i in ellabelinds
        ]  # pmkeys should refer to element label indeces that exist but if not fill the X to softly notify the user that soemthig is wrong, i.e. not all the element labels were read from the database
    ...
    def plotxy(
        ...
        for count, (ax, (xarr, yarr), (sxarr, syarr), xl, yl) in enumerate(
            zip(
                [self.plotw_xy.axes, self.plotw_xy.twaxes],
                self.plotdata,
                self.selectpointdata,
                [arrkeys[0], arrkeys[0]],
                [arrkeys[1], arrkeys[2]],
            )
        ):
    ...
    def fillcomppermutations(self):
        if (
            self.ellabels == ["A", "B", "C", "D"] and self.GUIMODE
        ):  # default to only using 4 elements , i.e. if more than that then they should have been ready correctly from database and are not enter-able here
            ans = userinputcaller(
                self,
                inputs=[
                    ("A", str, "A"),
        ...
        if len(self.ellabels) == 4:
            ...
            els = [
                ...
                "2",
                "3",
            ]  # for when there are more than 4 elements from which 4-el selections will be made from the anadict['platemap_comp4plot_keylist'] and this permuation permutes that selection
    ...
    def plotfom(self):
        ...
        if self.tabs__plateids != newplateids:
            self.tabs__plateids = self.setup_TabWidget(
                self.tabs__plateids, newplateids, compbool=False
            )
        newcodes = sorted(list(set(self.fomplotd["code"])))
        if (
            len(newcodes) > 1
        ):  # if more than 1 code make -1 an additional tab that plots all codes together
            newcodes += [-1]
        if self.tabs__codes != newcodes:
            self.tabs__codes = self.setup_TabWidget(
                self.tabs__codes, newcodes, compbool=True
            )
        ...
        idtupsarr = numpy.array(
            [
                self.fomplotd["fomdlist_index0"][inds],
                self.fomplotd["fomdlist_index1"][inds],
            ]
        ).T
        if len(inds) == 0:
            if self.GUIMODE:
                idialog = messageDialog(
                    self,
                    "Nothing to plot, all FOMs probably NaN. Existing plots to remain unchanged.",
                )
                idialog.exec_()
            return
            ...
            if "," in vstr:
                ...
                try:
                    ...
                    for count, (fcn, le) in enumerate(
                        zip(
                            [self.cmap.set_under, self.cmap.set_over],
        ...
        pmkeys = self.l_platemap4keys[
            idtupsarr[0][0]
        ]  # get the first pmkeys and then check if the others are the same. this is just ABCD unless changed by platemap_comp4plot_keylist
        pmkeys_consistent_bools = [
            self.l_platemap4keys[idtup[0]] == pmkeys for idtup in idtupsarr
        ]
        if False in pmkeys_consistent_bools:
            if self.GUIMODE:
                idialog = messageDialog(
                    self,
                    "Trying to plot FOMs from ana__ with different 4-element lists. Continue?",
                )
                if not idialog.exec_():
                    return
            els = ["X", "X", "X", "X"]
        ...
        for val, plotw in zip(self.tabs__plateids, self.tabs__plotw_plate):
            ...
            for xvyv, tupind in [
                ((xv, yv), self.select_idtups.index(tuple(tupa)))
                for xv, yv, tupa in zip(x[inds], y[inds], idtupsarr[inds])
                if tuple(tupa) in self.select_idtups
            ]:
                ...
                if self.compPlotMarkSelectionsCheckBox.isChecked():
                    c = numpy.float64(
                        [
                            ...
                            for tupa in idtupsarr[inds]
                        ]
                    )
                    sortinds_inds = numpy.argsort(
                        c.sum(axis=1)
                    )  # this sorting puts the "brightest" colors on top so any duplicate black compositions are plotted underneath
        ...
        if len(idtupsarr) == 0:
            ...
            self.repr_anaint_plots = numpy.median(
                [self.l_fomdlist[i0][i1]["anaint"] for i0, i1 in idtupsarr]
            )  # not sure if there is a better way to deicde whcih ana saved figures becomes associated with
    ...
    def compplot(self, plotw, comps, cols, sm):
        ...
        if nintervals == 0:
            nintervals = None
        self.quatcompclass.loadplotdata(
            comps[compsinds][:, self.comppermuteinds],
            cols[compsinds],
    ...
    def setup_TabWidget(self, oldopts, newopts, compbool=False):
        ...
        if len(newopts) < len(oldopts):
            ...
            for count in range(len(newopts) - len(l)):
                ...
                l += [plotw]
                tabw.addTab(plotw, "")
                plotw.genericclickonplot.connect(fcn)
    ...
    def plateclickprocess(self, coords_button_ax):
        ...
        inds = numpy.where(
            (plate == val) & numpy.logical_not(numpy.isnan(self.fomplotd["fom"]))
        )[0]
        x, y = self.fomplotd["xy"][inds].T
        if len(x) == 0:
            return
    ...
    def compclickprocess(self, coords_button_ax):
        ...
        inds = numpy.where(
            ((val < 0) | (code == val))
            & numpy.logical_not(numpy.isnan(self.fomplotd["fom"]))
        )[0]
        x, y = self.fomplotd["xy"][inds].T
        comps = self.fomplotd["comps"][inds]
        if len(x) == 0:
        ...
        if compclick is None:
            return
        permcomp = comps[
            :, self.comppermuteinds
        ]  # this comps may be a custom selection of 4 platemap channels according to platemap_comp4plot_keylist, in which case the string permutation selection will not match this
        dist = numpy.array([(((c - compclick) ** 2).sum()) ** 0.5 for c in permcomp])
        if min(dist) > critdist:
            return
    ...
    def clearvisuals(self):
        ...
        for plotw in self.tabs__plotw_plate + self.tabs__plotw_comp:
            ...
            plotw.fig.canvas.draw()
        self.tabs__plateids = self.setup_TabWidget(
            self.tabs__plateids, [-1], compbool=False
        )
    ...
    def plotwsetup(self):
        ...
        for b, w in [
            ...
            b.hide()
        self.plotw_xy.fig.subplots_adjust(left=0.22, bottom=0.17)
        self.quatcompclass = quatcompplotoptions(
            None,
            self.CompPlotTypeComboBox,
            plotw3d=None,
            include3doption=True,
            plotwcbaxrect=[0.88, 0.1, 0.04, 0.8],
        )

    def updateinfo(self):
        self.compLineEdit.setText(
            ",".join(["%.2f" % n for n in self.fomplotd["comps"][self.selectind]])
        )
        self.xyLineEdit.setText(
            ",".join(["%.2f" % n for n in self.fomplotd["xy"][self.selectind]])
        )
    ...
    def addrem_select_fomplotdinds(
        self,
        remove=False,
        fomplotdind=None,
        smplist=None,
        xy=None,
        comp=None,
        select_fomplotd_inds=None,
    ):  # only 1 of index, smplist, xy, comp should be not None
        selectinds = []
        if not select_fomplotd_inds is None:
            ...
            selectinds = [
                count
                for count, smp in enumerate(self.fomplotd["sample_no"])
                if smp in smplist
            ]
            ...
            xy = numpy.array(xy)
            dist = numpy.array(
                [((numpy.array(xyv) - arr) ** 2).sum() for xyv in self.fomplotd[k]]
            )
        ...
        if remove:
            ...
            indstoremove = sorted(
                [self.select_idtups.index(tup) for tup in tupstoremove]
            )[::-1]
    ...
    def drawcirc_idtup(self, idtup):
        idtupsarr = numpy.array(
            [self.fomplotd["fomdlist_index0"], self.fomplotd["fomdlist_index1"]]
        ).T
        ...
        circ = pylab.Circle(
            self.fomplotd["xy"][i_fomplotd], radius=1, edgecolor="r", facecolor="none"
        )
        plotw.axes.add_patch(circ)
        plotw.fig.canvas.draw()
        return (circ, plotw)
    ...
    def writeselectbrowsertext(self):
        sorthelp = (
            ["plate_id", "sample_no", "runint", "anaint", "code"]
            + self.ellabels
            + ["x", "y"]
        )
        ...
        sortind_k = sorted(
            list(
                set(
                    [
                        (gensortind(k), k)
                        for i0, i1 in self.select_idtups
                        for k in self.l_fomnames[i0]
                    ]
                )
            )
        )
        ...
        strfmt = (
            lambda x: (
                "%d"
                if isinstance(x, int)
                else ("%.3f" if (x == 0.0 or (x < 1000.0 and x > 0.04)) else "%.4e")
            )
            % x
        )
    ...
    def savefigs(
        self,
        save_all_std_bool=False,
        batchidialog=None,
        lastbatchiteration=False,
        filenamesearchlist=None,
        justreturndialog=False,
        prependstr="",
    ):
        cbl = [
            self.xplotchoiceComboBox,
                ...
                str(val),
                {
                    ...
                    "checked": (
                        ...
                        in [
                            ...
                            for i in range(mainitem.childCount())
                            if str(val) == str(mainitem.child(i).text(0)).strip()
                        ]
                    ),
                },
            )
        ...
        idialog = saveimagesDialog(
            ...
            plateid_dict_list=plateid_dict_list,
            code_dict_list=code_dict_list,
            histplow=self.plotw_fomhist,
            xyplotw=xyplotw,
            x_y_righty=x_y_righty,
            repr_anaint_plots=self.repr_anaint_plots,
            selectsamplebrowser=self.browser,
            filenamesearchlist=filenamesearchlist,
        )
        anaklist_activeplots = [
            self.l_csvheaderdict[i0]["anak"] for i0 in self.fomplotd["fomdlist_index0"]
        ]
        if (
            len(set(anaklist_activeplots)) == 1
        ):  # if all the FOMs in active plot are from the same ana__ then use that as prepend string ni filenames of images
        ...
        if justreturndialog:
            ...
            if not batchidialog is None:
                idialog.updateoptionsfrombatchidialog(
                    batchidialog, lastbatchiteration=lastbatchiteration
                )
            idialog.ExitRoutine()
    ...
    def save_all_std_plots(self, batchidialog=None):
        ...
        if batchidialog is None:
            ...
            for i in range(1, self.numStdPlots + 1):
                ...
                comboind_strlist += [
                    (i, str(self.stdcsvplotchoiceComboBox.currentText()))
                ]
        ...
        for i in cbinds:
            ...
            )  # or logic for ,-delim and and logic within each or block with &-delim
            filenamesearchlist = str(batchidialog.filenamesearchLineEdit.text()).split(
                ","
            )
            filenamesearchlist = [
                s.strip()
                for s in filenamesearchlist
            ...
            if len(filenamesearchlist) == 0:
                ...
                filenamesearchlist = [
                    [sv.strip() for sv in s.split("&") if len(sv.strip()) > 0]
                    for s in filenamesearchlist
                    if len(s.strip()) > 0
                ]
            self.savefigs(
                save_all_std_bool=True,
                batchidialog=batchidialog,
                filenamesearchlist=filenamesearchlist,
                lastbatchiteration=(i == cbinds[-1]),
            )  # for std plots all foms will be from same ana__  and prepend str will be filled in automatically
...
if __name__ == "__main__":
    ...
    class MainMenu(QMainWindow):
        def __init__(self, previousmm, execute=True, **kwargs):
            ...
            self.visui = visdataDialog(
                self, title="Visualize ANA, EXP, RUN data", **kwargs
            )
            # test udi creation
