...
class runsfrominfoDialog(QDialog, Ui_RunsFromInfoDialog):
    ...
    def nestedfill(self, d, parentitem, firststartswith='run', prependstr='', skipkeys=[], runparent=False):
        ...
        dictkeys2=sorted([k for k in d.keys() if k.startswith(firststartswith) and not k in skipkeys])
        for k in dictkeys2:
            ...
            self.nestedfill(d[k], item, firststartswith='runs__', runparent=True)#first time through the "runs" key is found and then runparent True passed to the next level where run__ are found
            parentitem.addChild(item)
            item.setExpanded(True)#not working?
        
        dictkeys1=sorted([k for k, v in d.iteritems() if not k.startswith(firststartswith) and isinstance(v, dict) and not k in skipkeys])
        for k in dictkeys1:
            item=QTreeWidgetItem([prependstr+k+':'], 0)
