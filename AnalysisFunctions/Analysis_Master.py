...
def stdgetapplicablefilenames(
    ...
    requiredparams = [
        (rp + techk) if rp.endswith("__") else rp for rp in requiredparams
    ]
    ...
    num_files_considered = numpy.int32(
        [
            len(expfiledict[runk]["files_technique__" + techk][typek])
            for runk in runklist
        ]
    ).sum()
    filedlist = [
        dict(
            ...
            expkeys=[runk, "files_technique__" + techk, typek, fnk],
            run=runk,
            fn=fnk,
            sample_no=v["sample_no"],
            nkeys=len(v["keys"]),
            num_header_lines=v["num_header_lines"],
            reqkeyinds=[
                v["keys"].index(reqk) for reqk in requiredkeys if reqk in v["keys"]
            ],
            optkeyinds=[
                (optk in v["keys"] and (v["keys"].index(optk),) or (None,))[0]
                for optk in optionalkeys
            ],
        )
        for runk in runklist
    ...
    filedlist = [
        dict(d, keyinds=d["reqkeyinds"] + [k for k in d["optkeyinds"] if not k is None])
        for d in filedlist
    ]
    filedlist = [
        dict(
        ...
        for d in filedlist
    ]  # has to be here because only place with access to expfiledict
    filedlist = [
        dict(
            d,
...
def getapplicablefilenames_byuse_tech_type(
    expfiledict, usek, techk, typek, runklist=None, requiredparams=[]
):
    requiredparams = [
        (rp + techk) if rp.endswith("__") else rp for rp in requiredparams
    ]
...
def stdcheckoutput(fomdlist, fomnames, filedlist):
    nancount = [
        (not k in d) or (isinstance(d[k], float) and numpy.isnan(d[k]))
        for d in fomdlist
        for k in fomnames
    ].count(True)
    nancount += len(fomnames) * (
        len(filedlist) - len(fomdlist)
    )  # any missing fomd  (there is a filed but not fomd) counts as len(fomnames) wirth fo NaN
...
class Analysis_Master_nointer:
    ...
    def check_input(self, critfracapplicable=0.9):
        ...
        return (
            ...
            % (self.num_files_considered - len(self.filedlist), 1.0 - fracapplicable),
        )
    ...
    def check_output(self, critfracnan=0.9):
        ...
        return (
            fracnan <= critfracnan,
            "%d FOMs, %.2f of attempted calculations, are NaN or missing"
            % (numnan, fracnan),
        )
    ...
    def initfiledicts(self, runfilekeys=[], runklist=None):
        ...
        if len(runfilekeys) > 0:
            ...
            self.runfiledict = dict(
                [(runk, dict([(fk, {}) for fk in runfilekeys])) for runk in runklist]
            )
        else:
            self.runfiledict = {}

    def prepare_filedlist(
        self,
        filedlist,
        expfiledict,
        expdatfolder=None,
        expfolderzipclass=None,
        fnk="fn",
    ):
        ...
        closeziplist = []
        runk_or_none = None
        if (not expfolderzipclass) or (
            not expdatfolder is None
        ):  # and not os.path.isdir(expdatfolder)):#if zipclass is an open .zip then don't need the path, but if not and expdat folder provided but is not absolute path, make it an absolute path
            expdatfolder = prepend_root_exp_path(expdatfolder)
        if (
            expfolderzipclass is None and expdatfolder is None
        ):  # read all from runfolders
            ...
            if bool(expfolderzipclass):
                closeziplist += [
                    expfolderzipclass
                ]  # close this at the end of perform only if it was created here
        if runk_or_none is None:
            ...
            if not bool(expfolderzipclass):
                ...
                runk_or_none = [
                    d.update(
                        [
                            ("readfcn", readbinary_selinds),
                            (
                                "readfcn_args",
                                (
                                    os.path.join(expdatfolder, d[fnk] + ".dat"),
                                    d["nkeys"],
                                ),
                            ),
                            (
                                "readfcn_kwargs",
                                {"keyinds": d["keyinds"], "zipclass": False},
                            ),
                        ]
                    )
                    if (d[fnk] + ".dat") in expfns
                    else d["run"]
                    for d in filedlist
                ]
            else:
                    ...
                    d.update(
                        [
                            ...
                            (
                                ...
                                {
                                    "keyinds": d["keyinds"],
                                    "zipclass": expfolderzipclass,
                                },
                            ),
                        ]
                    )
                    if expfolderzipclass.fn_in_archive(d[fnk] + ".dat")
                    else d["run"]
                    for d in filedlist
                ]
        ...
        for runk in runstoopen:
            ...
            if bool(runzipclass):
                ...
                closeziplist += [
                    runzipclass
                ]  # don't leave runzipclass open after perform finished
            else:
                pathjoinfcn = lambda fn: os.path.join(runp, fn)
            [
                d.update(
                    [
                        ("readfcn", readtxt_selectcolumns),
                        ("readfcn_args", (pathjoinfcn(d[fnk]),)),
                        (
                            "readfcn_kwargs",
                            {
                                "num_header_lines": d["num_header_lines"],
                                "selcolinds": d["keyinds"],
                                "zipclass": runzipclass,
                                "delim": None,
                            },
                        ),
                    ]
                )
                for rk, d in zip(runk_or_none, filedlist)
                if rk == runk
            ]
        return closeziplist
    ...
    def readdata(self, p, numkeys, keyinds, num_header_lines=0, zipclass=None):
        if not (
            os.path.isdir(os.path.split(p)[0])
            or os.path.isdir(os.path.split(os.path.split(p)[0])[0])
        ):
        ...
        dataarr = readtxt_selectcolumns(
            p,
            selcolinds=keyinds,
            delim=None,
            num_header_lines=num_header_lines,
            zipclass=zipclass,
        )  # this could read ascii version in .ana for anlaysis that builds on analysis (e.g. CV_photo), or hypothetically in .exp but nothing as of 201509 that write ascii to .exp - this may be needed for on-the-fly
        return dataarr
    ...
    def genuserfomdlist(self, anauserfomd, appendtofomdlist=True):
        ...
        if appendtofomdlist:
            self.fomdlist = [
                dict(d, **userd) for d, userd in zip(self.fomdlist, userfomdlist)
            ]  # adds user foms to fomdlist dicts but the corresponding keys are NOT in self.fomnames
    ...
    def perform(
        ...
        expfiledict=None,
        anauserfomd={},
    ):  # zipclass intended to be the class with open zip archive if expdatfolder is a .zip so that the archive is not repeatedly opened
        self.initfiledicts()
    ...
    def writefom(
        ...
        fn=None,
        strkeys_fomdlist=[],
        createdummyfomdlist=False,
        num_intfoms_at_start_of_fomdlist=0,
    ):  # self.fomnames assumed to be float, createdummyfomdlist is for writing all NaN and aborting analysis
        ...
        self.multirunfiledict["fom_files"][fn] = self.writefom_bare(
            destfolder,
            fn,
            strkeys=strkeys + strkeys_fomdlist,
            floatkeys=floatkeys + self.fomnames[num_intfoms_at_start_of_fomdlist:],
            intfomkeys=["runint", "plate_id"]
            + self.fomnames[:num_intfoms_at_start_of_fomdlist],
        )
    ...
    def writefom_bare(
        ...
        self.csvfilstr = createcsvfilstr(
            self.fomdlist, floatkeys, intfomkeys=intfomkeys, strfomkeys=strkeys
        )  # , fn=fnf)self.fomnames
        if destfolder is None:
            return
        allfomnames = (
            ["sample_no"] + intfomkeys + strkeys + floatkeys
        )  # this is the order in createcsvfilstr and doesn't allow int userfoms
        p = os.path.join(destfolder, fn)
        totnumheadlines = writecsv_smpfomd(
            p, self.csvfilstr, headerdict=self.csvheaderdict
        )
...
def calcfom_analyzedata_calcfomdialogclass(
    self, checkinputbool=True
):  # this argument is called self sothsi function appears the same as it would as part of the calcfomdialogclass
    ...
    for runk, typek, b in runk_typek_b:
        ...
        if b:
            self.activeana[frunk][typek] = copy.deepcopy(
                self.analysisclass.multirunfiledict[typek]
            )
        else:
            self.activeana[frunk][typek] = copy.deepcopy(
                self.analysisclass.runfiledict[runk][typek]
            )
    ...
    if (
        active_int > 1
        and self.analysisclass.analysis_name == "Analysis__ECMS_Calibration"
    ):
        self.activeana["plate_ids"] = self.anadict["ana__%i" % (active_int - 1)][
            "plate_ids"
        ]
        plateidsliststr = self.activeana["plate_ids"]
    else:
        plateidsliststr = ",".join(
            "%d" % i
            for i in sorted(
                list(set([d["plate_id"] for d in self.analysisclass.fomdlist]))
            )
        )
        self.activeana["plate_ids"] = (
            "''" if len(plateidsliststr) == 0 else plateidsliststr
        )
    ...
    desc += "; run " + ",".join(
        "%d" % i
        for i in sorted(list(set([d["runint"] for d in self.analysisclass.fomdlist])))
    )
    ...
    if "process_fom" in gentype:
        if "from_file" in gentype:
            self.activeana["process_fom_from_file_paths"] = ",".join(
                sorted(
                    list(
                        set(
                            [
