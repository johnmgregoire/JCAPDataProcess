...
from FOM_process_basics import (
    ...
    Analysis_Master_FOM_Process,
)  # , stdgetapplicablefomfiles
...
class Analysis__FOM_Merge_Aux_Ana(Analysis_Master_FOM_Process):
    def __init__(self):
        ...
        self.dfltparams = {
            ...
            "select_fom_keys": "ALL",
            "select_aux_keys": "ALL",
            "remove_samples_not_in_aux": 1,
            "aux_ana_path": "self",
            "aux_ana_ints": "ALL",
            "match_plate_id": 1,
            "key_append_all_aux": "none",
        }
        ...
        self.requiredparams = []
        self.fomnames = []
        self.plotparams = dict(
            {}, plot__1={}
        )  # copied in the default getapplicablefomfiles
        self.csvheaderdict = dict(
            {}, csv_version="1", plot_parameters={}
        )  # get for each csv during .perform()
        self.auxfiledlist = []
        self.auxpath = ""
    ...
    def getapplicablefilenames(
        ...
        if (
            self.params["aux_ana_path"] == "self"
            and len(calcFOMDialogclass.aux_ana_dlist) == 0
            ...
            if not (
                True
                in [
                    self.params["aux_ana_path"] in auxd["auxexpanapath_relative"]
                    for auxd in calcFOMDialogclass.aux_ana_dlist
                ]
            ):
                self.params["aux_ana_path"] = auxd[
                    "auxexpanapath_relative"
                ]  # if aux_ana_path not available or 'None', use the msot recently added one, which will be auxd due to the above iterator#20180329removed os.path.split
        ...
        self.processnewparams(
            calcFOMDialogclass=calcFOMDialogclass, recalc_filedlist=True
        )  # not hasattr(self, 'filedlist'))
        return self.filedlist

    def processnewparams(self, calcFOMDialogclass=None, recalc_filedlist=True):
        self.processnewparams_merge(
            calcFOMDialogclass=calcFOMDialogclass, recalc_filedlist=recalc_filedlist
        )

    def processnewparams_merge(
        self,
        calcFOMDialogclass=None,
        recalc_filedlist=True,
        additionl_required_params_aux=[],
    ):
        self.fomnames = []
        if (
            recalc_filedlist
        ):  # the parmas can change what fom files are in filedlist whcih determines which fom names must not be duplicated so need to reevaluate this every time params change
        ...
        if self.params["aux_ana_path"] == "self":
            ...
            try:
                if (
                    self.previous_params is None
                    or (not "custom" in self.auxd)
                    or (
                        False
                        in [
                    ...
                    p = mygetopenfile(
                        parent=(
                            None if (calcFOMDialogclass is None) else calcFOMDialogclass
                        ),
                        markstr="Select .csv with 1 header line inlcuding sample_no",
                    )
                    if p is None or len(p) == 0:
                        raise ValueError()
                    self.auxpath, fnk = os.path.split(p)
                    fileattrd = dict(
                        {}, fn=fnk, num_header_lines=1
            ...
            if not (
                ...
                self.params = copy.copy(
                    self.dfltparams
                )  # no aux_ana meet search so return to default, will also happen if there are no aux_ana open
                self.filedlist = []
                return
            for auxd in calcFOMDialogclass.aux_ana_dlist:
                if (
                    self.params["aux_ana_path"] in auxd["auxexpanapath_relative"]
                ):  # can only use 1 aux ana so use the first one found
        ...
        if custom_load_csv_bool:
            ...
            if self.params["aux_ana_ints"] == "ALL":
                ...
                anak_list = [
                    k
                    for k in sort_dict_keys_by_counter(self.auxd, keystartswith="ana__")
                    if k.partition("ana__")[2] in anaintstrlist
                ]
            anak_list = [
                k
                for k in anak_list
        ...
        keysfcn = lambda filed, aux_key_append, notallowedkeys: [
            k + aux_key_append
            for k in filed["keys"]
            if (not k in FOMKEYSREQUIREDBUTNEVERUSEDINPROCESSING)
            and (not k + aux_key_append in FOMKEYSREQUIREDBUTNEVERUSEDINPROCESSING)
            and (not k + aux_key_append in notallowedkeys)
        ...
        keystestfcn = lambda filed: len(
            set(filed["keys"]).intersection(reqdkeysset)
        ) == len(reqdkeysset)
        existkeys = [
            k
        ...
        for anak in anak_list:
            for fnk, filed in self.auxd[anak]["files_multi_run"][
                ...
                if (
                    ...
                    self.auxfiledlist += [
                        {
                            "anakeys": [anak, "files_multi_run", "fom_files", fnk],
                            "ana": anak,
                            "fn": fnk,
                            "keys": filed["keys"],
                            "num_header_lines": filed["num_header_lines"],
                            "process_keys": keysfcn(filed, aux_key_append, existkeys),
                        }
                    ]
                    existkeys += [
                        self.auxfiledlist[-1]["process_keys"]
                    ]  # aux keys can be from multipleana__ blocks but only 1 ana_file
        auxkeylist = [
            k if len(aux_key_append) == 0 else k[: -len(aux_key_append)]
            for filed in self.auxfiledlist
            for k in filed["process_keys"]
        ]
    ...
    def perform(
        ...
        anauserfomd={},
        expfiledict=None,
    ):  # must have same arguments as regular AnaylsisClass
        self.initfiledicts()
        matchplateidbool = bool(self.params["match_plate_id"])
        if matchplateidbool:
            matchplateidfcn = lambda tempfomd: tempfomd["plate_id"]
        else:
            matchplateidfcn = lambda tempfomd: [""] * len(
                tempfomd["sample_no"]
            )  # use emtpy strings so that matching tups look the same but plate_id is always empty string so it always matches
        for filed in self.filedlist:
            fn = filed["fn"]
            try:
                # if 1:
                fomd, self.csvheaderdict = readcsvdict(
                ...
                if self.params["select_fom_keys"] == "ALL":
                    ...
                    keysearchlist = [
                        s.strip() for s in keysearchlist if len(s.strip()) > 0
                    ]
                process_keys = [
                    k
                    for k in filed["process_keys"]
                ...
                auxfomd_list = [
                    readcsvdict(
                        os.path.join(self.auxpath, auxfiled["fn"]),
                        auxfiled,
                ...
                if self.params["remove_samples_not_in_aux"]:
                    ...
                    for k in (
                        list(FOMKEYSREQUIREDBUTNEVERUSEDINPROCESSING) + process_keys
                    ):
                ...
                if plateidinanyaux and not matchplateidbool:
                    ...
                    newfomd["aux_plate_id"] = numpy.array(
                        [""] * len(newfomd["sample_no"]), dtype="|S8"
                    )  # plate_id presumably no more than 8 characters long
                for auxfomd, auxfiled in zip(auxfomd_list, self.auxfiledlist):
                    auxfomdinds, fomdinds = numpy.array(
                        [
                            [count, plt_smp_list.index(tup)]
                            for count, tup in enumerate(
                                zip(matchplateidfcn(auxfomd), auxfomd["sample_no"])
                            )
                            if tup in plt_smp_list
                        ]
                    ).T
                    ...
                    if (
                        len(auxfomdinds) == 0
                    ):  # if no matching plate,sample then skip this column merge
                        continue
                    self.fomnames += auxfiled["process_keys"]
                    for k in auxfiled["process_keys"]:
                        newfomd[k] = (
                            numpy.ones(len(newfomd["sample_no"]), dtype="float64")
                            * numpy.nan
                        )
                        newfomd[k][fomdinds] = auxfomd[
                            k[: -len(self.params["key_append_all_aux"])]
                            if self.params["key_append_all_aux"] != "none"
                            else k
                        ][auxfomdinds]
                    if (
                ...
                allkeys = (
                    list(FOMKEYSREQUIREDBUTNEVERUSEDINPROCESSING)
                    + self.fomnames
                ...
                if self.params["aux_ana_path"] == "custom":
                    ...
                    self.multirunfiledict["misc_files"][newfn] = "%s;%s;%d;%d" % (
                        "csv_fom_file",
                        ",".join(auxfiled["keys"]),
                        auxfiled["num_header_lines"],
                        auxfiled["num_data_rows"]
                        if "num_data_rows" in auxfiled
                        else len(auxfomd_list[0]["sample_no"]),
                    )
                    if not newfn in os.listdir(
                        destfolder
                    ):  # assume aux custom fn is unique from the ana-generated files in this folder. If the custom file copied already doesn't need to be copied again
                        shutil.copy(
                            os.path.join(self.auxpath, auxfiled["fn"]),
                            os.path.join(destfolder, newfn),
                        )
            except:
                if self.debugmode:
                    raiseTEMP
            ...
            )  # sample_no, plate_id and runint are explicitly required in csv selection above and are assume to be present here
...
class Analysis__FOM_Merge_PlatemapComps(Analysis_Master_FOM_Process):
    def __init__(self):
        ...
        self.dfltparams = {
            "select_ana": "ana__1",
            "select_fom_keys": "ALL",
            "key_append_conc": ".PM.Loading",
            "key_append_atfrac": ".PM.AtFrac",
            "tot_conc_label": "Tot.PM.Loading",
            "value_when_missing": "nan",
        }
        self.params = copy.copy(self.dfltparams)
    ...
    def processnewparams(self, calcFOMDialogclass=None):
        self.fomnames = []
        try:
            self.value_when_missing = (
                numpy.nan
                if self.params["value_when_missing"] == "NaN"
                else float(self.params["value_when_missing"])
            )
        except:
        ...
        for filed in self.filedlist:
            ...
            try:
                ...
                for pid in allplateids:
                    ...
                    els, tup_multielementink = getelements_plateidstr(
                        str(pid),
                        multielementink_concentrationinfo_bool=True,
                        return_defaults_if_none=True,
                    )
                    if not tup_multielementink is None:
                        ...
                        conc_el_chan = numpy.zeros(
                            (len(els), len(els)), dtype="float64"
                        )
                        numpy.fill_diagonal(conc_el_chan, 1.0)
                    tot_conc_label = (
                        None
                        if len(self.params["tot_conc_label"]) == 0
                        else self.params["tot_conc_label"]
                    )
                    calc_comps_multi_element_inks(
                        platemapdlist,
                        cels_set_ordered,
                        conc_el_chan,
                        key_append_conc=self.params["key_append_conc"],
                        key_append_atfrac=self.params["key_append_atfrac"],
                        tot_conc_label=tot_conc_label,
                    )
                    newfomnames = (
                        [el + self.params["key_append_conc"] for el in cels_set_ordered]
                        + [
                            el + self.params["key_append_atfrac"]
                    ...
                    newfomnames = [
                        lab
                        for lab in newfomnames
                        if not (
                            lab in process_keys
                            or lab in FOMKEYSREQUIREDBUTNEVERUSEDINPROCESSING
                        )
                    ]
                    fomdinds_plate = numpy.where(
                        (fomd["plate_id"] == pid) & (fomd["sample_no"] > 0)
                    )[0]
                    ...
                    for k in newfomnames:
                        if not k in self.fomnames:
                            fomd[k] = (
                                numpy.ones(len(fomd["sample_no"]), dtype="float64")
                                * self.value_when_missing
                            )
                            self.fomnames += [k]
                        fomd[k][fomdinds_plate] = numpy.array(
                            [platemapdlist[pmind][k] for pmind in pminds]
                        )
                self.strkeys_fomdlist = []
...
class Analysis__Filter_Linear_Projection(Analysis_Master_FOM_Process):
    def __init__(self):
        ...
        self.dfltparams = {
            "select_ana": "ana__1",
            "select_fom_keys": "ALL",
            "max_dist_from_line": 1.0,
            "keys_param_space": "x,y",
            "line_start": "0.,0.",
            "line_stop": "100.,150.",
            "allow_beyond_endpts": 0,
            "is_comp_space": 0,
        }
        self.params = copy.copy(self.dfltparams)
    ...
    def processnewparams(self, calcFOMDialogclass=None):
        self.fomnames = []
        l_ndims = [
            self.params[k].count(",") + 1
            for k in ["keys_param_space", "line_start", "line_stop"]
        ]
        nd = l_ndims[0]
        ...
        if nd > (4 if self.params["is_comp_space"] else 3):
            ...
            return
        try:
            self.endpoints = numpy.float64(
                [
                    [float(v.strip()) for v in self.params[k].split(",")]
                    for k in ["line_start", "line_stop"]
                ]
            )
        except:
        ...
        if (
            None in list_pmkeys
        ):  # require that all runs in exp have platemap loaded, but once loaded the keys can come from any platemap (union of platemap keys)
            self.platemapkeys = []
        else:
            self.platemapkeys = list(set([k for l in list_pmkeys for k in l]))
        filed_k_tups = [
            (filed, k) for filed in self.filedlist for k in filed["keys"]
        ]  # allow process_keys and things like sample_no
        ...
        for k in self.params["keys_param_space"].split(","):
            ...
            matchkeys = [
                (count, kv) for count, kv in enumerate(self.platemapkeys) if k in kv
            ]
            if len(matchkeys) > 0:
        ...
        self.params["keys_param_space"] = ",".join(
            [k for filedbool, k in self.fomfromfilebool_key__params_space]
        )
    ...
    def perform(
        ...
        for filed in self.filedlist:
            ...
            try:
                ...
                pmkeys = [
                    axisk
                    for fomfromfilebool, axisk in self.fomfromfilebool_key__params_space
                    if not fomfromfilebool
                ]
                pmaxes_pmkeys = []
                if len(pmkeys) > 0:
                    pmaxes_pmkeys = []
                    for k in pmkeys:
                        ...
                        axisvals = (
                            numpy.ones(len(fomd["runint"]), dtype="float64") * numpy.nan
                        )
                        for runint in runints:
                            runinds = numpy.where(fomd["runint"] == runint)[0]
                            platemapdlist = expfiledict["run__%d" % runint][
                                "platemapdlist"
                            ]
                ...
                arr_of_xyz = numpy.array(
                    [
                        fomd[axisk] if fomfromfilebool else pmaxes_pmkeys.pop(0)
                        for fomfromfilebool, axisk in self.fomfromfilebool_key__params_space
                    ]
                ).T
                ...
                filterlined = filterbydistancefromline(
                    arr_of_xyz,
                    self.endpoints[0],
                    self.endpoints[1],
                    self.params["max_dist_from_line"],
                    betweenpoints=not bool(self.params["allow_beyond_endpts"]),
                    invlogic=False,
                    returnonlyinds=False,
                    is_composition=bool(self.params["is_comp_space"]),
                )
...
class Analysis__FOM_Interp_Merge_Ana(Analysis__FOM_Merge_Aux_Ana):
    def __init__(self):
        ...
        self.dfltparams = {
            ...
            "aux_ana_path": "self",
            "aux_ana_ints": "ALL",
            "interp_keys": "platemap_xy",
            "fill_value": "extrapolate",
            "kind": "linear",
            "interp_is_phasemap": 0,
            "interp_is_comp": 0,
            "num_pts_in_extrapolation": 6,
            "plate_ids": "ALL",
        }
    ...
    def processnewparams(self, calcFOMDialogclass=None, recalc_filedlist=True):
        ...
        self.processnewparams_merge(
            calcFOMDialogclass=calcFOMDialogclass,
            recalc_filedlist=recalc_filedlist,
            additionl_required_params_aux=self.interp_type
            if isinstance(self.interp_type, list)
            else [],
        )
        for filed in self.filedlist:
            filed["process_keys"] = [
                k
                for k in filed["process_keys"]
                if not k in self.params["select_aux_keys"].split(",")
            ]  # these keys will be interpolated. process_keys are those coming along for the ride
            if isinstance(
                self.interp_type, list
            ):  # if interp keys are used they better be in all destination filed. note this check is for process_keys assumintg things like sample_no and runint will not be used for interpolation
        ...
        if (
            self.interp_type == "xy"
            or (isinstance(self.interp_type, list) and len(self.interp_type) == 2)
        ) and self.params["fill_value"] == "extrapolate":
            ...
            "," in self.params["fill_value"]
        ):  # this not supported in scipy 0.16 so will cause error
            ...
            self.params["fill_value"] == "extrapolate"
        ):  # this case only needed for 1d extrapolation since standard scipy is 0.16 and it doesn't support extrapolation
            self.need_to_extrapolate_separately = True
            self.interpkwargs = {"fill_value": numpy.nan}
            self.interpkwargs["kind"] = self.params["kind"]
            self.interpkwargs["bounds_error"] = False
    ...
    def perform(
        ...
        for filed in self.filedlist:
            ...
            try:
                ...
                if (
                    self.params["interp_is_phasemap"] == 1
                ):  # need to sort so that phase con calculation so it can be used for mask in shift calculation
                    sorttup = lambda k: (
                        0 if k.startswith("phase_concentration.") else 1,
                        int(k.partition(".")[2])
                        if k.partition(".")[2].isdigit()
                        else 99999,
                        k,
                    )
                    keys_to_interp = [
                        tup[2] for tup in sorted([sorttup(k) for k in keys_to_interp])
                    ]
                ...
                if self.interp_type in ["xy", "xy_line"]:
                    ...
                    if self.interp_type == "xy":
                        ...
                        for auxfomd, auxfiled in zip(auxfomd_list, self.auxfiledlist):
                            ...
                            toappend = [
                                ...
                                for tup in zip(
                                    xaux,
                                    yaux,
                                    *[auxfomd[k] for k in allkeys[num_interpdim:]]
                                )
                                if (not numpy.nan in tup)
                                and not True
                                in [
                                    newval in l
                                    for newval, l in zip(
                                        tup, self.src_x_then_y[:num_interpdim]
                                    )
                                ]
                            ]  # num pts by num data arrays, don't allow the interp coords to be duplicated
                            for i in range(len(allkeys)):
                                self.src_x_then_y[i] += [tup[i] for tup in toappend]
                        dest_x = numpy.float64(
                            [
                                [xl[smps.index(smp)], yl[smps.index(smp)]]
                                for smp in fomd["sample_no"]
                            ]
                        )
                        ...
                        m, b = numpy.polyfit(
                            xaux_all, yaux_all, 1
                        )  # fit set of x,y to line to get slope
                        rot = numpy.arctan(
                            m
                        )  # find rotation angle and rotation transform to remove the slope, i.e. turn into a horizontal line where only x coordinate is needed
                        c, s = numpy.cos(-rot), numpy.sin(-rot)
                        linparam = lambda x, y: x * c - y * s
                        for auxfomd, auxfiled in zip(
                            auxfomd_list, self.auxfiledlist
                        ):  # ****
                            xaux = numpy.array(
                                [xl[smps.index(smp)] for smp in auxfomd["sample_no"]]
                            )
                            yaux = numpy.array(
                                [yl[smps.index(smp)] for smp in auxfomd["sample_no"]]
                            )
                            toappend = [
                                tup
                                for tup in zip(
                        ...
                        dest_x = numpy.float64(
                            [
                                linparam(xl[smps.index(smp)], yl[smps.index(smp)])
                                for smp in fomd["sample_no"]
                            ]
                        )
                    ...
                    if len(self.interp_type) == 2:
                        ...
                        num_interpdim = 2
                        dest_x = numpy.float64(
                            [fomd[self.interp_type[0]], fomd[self.interp_type[1]]]
                        ).T
                    else:
                        fcn = interpolate.interp1d
                        num_interpdim = 1
                    ...
                    for auxfomd, auxfiled in zip(auxfomd_list, self.auxfiledlist):
                        toappend = [
                            tup
                            for tup in zip(*[auxfomd[k] for k in allkeys])
                            if (not numpy.nan in tup)
                            and not True
                            in [
                                newval in l
                ...
                for k, dataind in zip(
                    ...
                    if self.params["interp_is_phasemap"] == 1 and k.startswith(
                        ...
                        if maskkey in keys_to_interp:
                            ...
                            inds = numpy.where(
                                numpy.array(self.src_x_then_y[maskdataind]) > 0.0
                            )[
                                0
                            ]  # phase map only has meaningful shift values when phase conc is nonzero
                            destinds = numpy.where(newfomd[maskkey] > 0.0)[
                                0
                            ]  # not only mask but only interp where necessary, phase_conc already interpolated so can set shift to 0 where phase_conc is already 0.
                            if (
                                len(inds) <= 1
                            ):  # can't interp with 1 point. may need to increase this threshold and use nearest interp
                            ...
                            newfomd[k] = (
                                numpe.ones(len(dest_x), dtype="float64") * nump.nan
                            )
                            continue
                    else:
                    ...
                    if num_interpdim == 1:
                        ...
                        if self.need_to_extrapolate_separately:
                            ...
                            if len(naninds) > 0:
                                ans[naninds] = linear_1d_extrapolation(
                                    xvals,
                                    numpy.array(zvals),
                                    d_x[naninds],
                                    num_pts_in_lin_fit=self.params[
                                        "num_pts_in_extrapolation"
                                    ],
                                )
                    else:
                        ...
                        if self.need_to_extrapolate_separately:
                            naninds = numpy.where(numpy.isnan(ans))[0]
                            if len(naninds) > 0:
                                ans[naninds] = linear_2d_extrapolation(
                                    xvals,
                                    numpy.array(zvals),
                                    d_x[naninds],
                                    num_pts_in_lin_fit=self.params[
                ...
                if self.params["interp_is_comp"]:
                    ...
                    keystonormalize = [
                        ...
                        if k.startswith("phase_concentration.")
                    ]
                else:
                    keystonormalize = []
                if len(keystonormalize) > 0:
                    keys_to_interp_orig = [
                        k.replace("AtFrac", "InterpRaw")
                        if "AtFrac" in k
                        else (k + "InterpRaw")
                    ...
                    for k, kraw in zip(keystonormalize, keys_to_interp_orig):
                        ...
                        newfomd[k][newfomd[k] < 0] = 0.0
                    normarr = numpy.float64([newfomd[k] for k in keystonormalize]).sum(
                        axis=0
                    )
                    for k in keystonormalize:
                        newfomd[k] /= normarr
