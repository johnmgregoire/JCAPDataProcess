...
loss_fcn_dict = {
    "L2_5x_positive": lambda resid: np.where(resid > 0.0, 5.0 * resid ** 2, resid ** 2),
    "L2": lambda resid: resid ** 2,
    "L1": lambda resid: np.abs(resid),
    "LogCosh_1E9": lambda resid: np.log(np.cosh(resid * 1.0e9)),
    "L2_5x_first_half": lambda resid: np.where(
        np.arange(len(resid)) < (len(resid) // 2), 5.0 * resid ** 2, resid ** 2
    ),
    "L2_3x_first_half_3x_positive": lambda resid: np.where(
        np.arange(len(resid)) < (len(resid) // 2),
        3.0 * np.where(resid > 0.0, 3.0 * resid ** 2, resid ** 2),
        np.where(resid > 0.0, 3.0 * resid ** 2, resid ** 2),
    ),
    "L2_3x_first_half_5x_positive": lambda resid: np.where(
        np.arange(len(resid)) < (len(resid) // 2),
        3.0 * np.where(resid > 0.0, 5.0 * resid ** 2, resid ** 2),
        np.where(resid > 0.0, 5.0 * resid ** 2, resid ** 2),
    ),
    "L2_inv_prop_to_time": lambda resid: (np.arange(len(resid)) + 1.0)[::-1]
    * resid ** 2,
}
...
def get_colors_fom_cmap(
    fomarr, vmin, vmax, cmap
):  # if need over/under colors those should have been already set via cmap.set_under, cmap.set_over
...
def read_ecms_simulation_data(ms_simulation_model, species_list):
    ...
    for k, v in d["metadata"].iteritems():
        if not isinstance(v, str):
            continue
        d["metadata"][k] = (
            [attemptnumericconversion_tryintfloat(v2) for v2 in v.split(",")]
            if "," in v
...
def read_hiden_csv(p):
    ...
    ms_start_time = time.strptime(
        " ".join(date_time_str.split(",")[1::2]), "%m/%d/%Y %I:%M:%S %p"
    )
...
def eche_split_current_by_sweep_and_generate_intended_voltage(
    tech, sim_lib_dict, echem_t, echem_i, smooth_paramd=None, time_delta=0.5
):
    if tech.startswith("CV"):
        ...
        segment_index_arr = []
        for count, (t0, t1, v0, v1) in enumerate(
            zip(
                profd["sweep_endpoint_times_s"],
                profd["sweep_endpoint_times_s"][1:],
                profd["sweep_endpoint_potentials_vrhe"],
                profd["sweep_endpoint_potentials_vrhe"][1:],
            )
        ):
            ...
            segment_index_arr += [count] * len(
                inds
            )  # TODO. nothing here makes this be the correct total length because echem_t may be longer (go beyond bounds of the profile)
        for nanind in np.where(np.isnan(sim_basis_v))[0]:
            goodinds = np.where(np.logical_not(np.isnan(sim_basis_v)))[0]
            replace_ind = goodinds[
                np.argmin((echem_t[goodinds] - echem_t[nanind]) ** 2)
            ]
            if np.abs(echem_t[replace_ind] - echem_t[nanind]) < time_delta:
                sim_basis_v[nanind] = sim_basis_v[replace_ind]
        if (
            smooth_paramd is None
            or smooth_paramd["SGfilter_order"] == 0
            or smooth_paramd["SGfilter_nptsoneside"] < 3
        ):
            return (
                sweep_currents,
                sim_basis_v,
                np.hstack(sweep_currents),
                np.array(segment_index_arr),
            )
        smooth_sweep_currents = [
            signal.savgol_filter(
                arr,
                2 * smooth_paramd["SGfilter_nptsoneside"] + 1,
                smooth_paramd["SGfilter_order"],
            )
            for arr in sweep_currents
        ]
        echem_i_smooth = np.hstack(smooth_sweep_currents)
        echem_i_smooth[: int(smooth_paramd["SGfilter_nptsoneside"] * 1.5)] = echem_i[
            : int(smooth_paramd["SGfilter_nptsoneside"] * 1.5)
...
if (
    0
):  # in spyder open new concolse, run __file__ and then up to and include MainMenu definition in CalcFomApp. then run __file__= and code up to here, then run rest of thiss sectino, then run specific load to prepare running perform line by line
    mainapp = QApplication(sys.argv)
    form = MainMenu(None, execute=False, modifyanainplace=False, guimode=False)
    calcFOMDialogclass = form.calcui
if 0:
    calcFOMDialogclass.importexp(
        exppath=r"L:\processes\experiment\temp\20190403.132935.done\20190403.132935.exp"
    )
    cb = calcFOMDialogclass.AnalysisNamesComboBox
    selind = [
        i
        for i in range(int(cb.count()))
        if str(cb.itemText(i)).startswith("Analysis__ECMS_Time_Join")
    ]
    ...
    (
        ...
        calcFOMDialogclass.tempanafolder,
        calcFOMDialogclass.expfolder,
        True,
        "ana__99",
        calcFOMDialogclass.expzipclass,
        {},
        calcFOMDialogclass.expfiledict,
    )
    self = calcFOMDialogclass.analysisclass
elif 0:
    calcFOMDialogclass.clearanalysis()
    calcFOMDialogclass.importana(
        p=r"L:\processes\analysis\temp\20190403.135546.run\20190403.135546.ana"
    )
    for i in range(1, int(calcFOMDialogclass.FOMProcessNamesComboBox.count())):
        if (
            str(calcFOMDialogclass.FOMProcessNamesComboBox.itemText(i)).partition("(")[
                0
            ]
...
class Analysis__ECMS_Time_Join(Analysis_Master_inter):
    def __init__(self):
        ...
        self.dfltparams = {
            "eche_techniques": "CV3,CV6,CV9",
            "masses": "2,15,26",
            "mass_spec_lag_time_s": 5.0,
            "post_eche_time_for_MS_settle_s": 60.0,
        }
    ...
    def processnewparams(self, calcFOMDialogclass=None):
        ...
        try:
            self.eche_techniques = self.params["eche_techniques"].split(
                ","
            )  # ultimatley get this from CV* in params
            self.masses = self.params["masses"].split(",")
        except:
            self.filedlist = []
        (
            self.num_files_considered,
            self.filedlist_ms,
        ) = getapplicablefilenames_byuse_tech_type(
            expfiledict, "data", "HidenMID", "hiden_single_mass_files"
        )
        self.filedlist = [
            d
            for tech in self.eche_techniques
            for d in getapplicablefilenames_byuse_tech_type(
                expfiledict,
                "data",
                tech,
                "pstat_files",
        ...
        self.description = "%s on %s" % (
            ",".join(self.fomnames),
            ",".join(self.eche_techniques),
        )
    ...
    def perform(
        ...
        self.fomnames += [
            "eche_technique",
            "fn_eche",
        ]  # not supposed to put the string foms here but usine writefombare so it's ok
        self.fomnames += [
            "fn_mass__%s" % mass for mass in self.masses
        ]  # here fns are per sample,plate,technique
        for filed in self.filedlist_ms:
            ...
            filed["mass"] = mass
            filed["start_time"], filed["ms_time_ms"], filed["ms_torr"] = read_hiden_csv(
                p
            )
            filed["start_time"] = time.mktime(filed["start_time"])
            filed["end_time"] = filed["start_time"] + filed["ms_time_ms"][-1] / 1000.0
            filed["epoch(s)"] = (
                filed["ms_time_ms"] / 1000.0
                + filed["start_time"]
        ...
        for filed in self.filedlist:
            if numpy.isnan(filed["sample_no"]):
                ...
                continue
            fomd = dict(
                [(k, "") for k in self.fomnames[1:]]
            )  # fill empty string for all file paths and tech so that if not found it still exists
            fn = filed["fn"]
            ...
            for msfiled in self.filedlist_ms:
                if (
                    msfiled["start_time"] <= filed["start_time"]
                    and msfiled["end_time"] >= filed["end_time"]
                ):
                    num_ms_found += 1
                    msinds = np.where(
                        (msfiled["epoch(s)"] >= filed["start_time"])
                        & (
                            msfiled["epoch(s)"]
                            <= (
                                filed["end_time"]
                                + self.params["post_eche_time_for_MS_settle_s"]
                            )
                        )
                    )[0]
                    inds = [
                        -1
                        if ms_epoch > datadict["epoch(s)"][-1]
                        else np.argmin((datadict["epoch(s)"] - ms_epoch) ** 2)
                        for ms_epoch in msfiled["epoch(s)"][msinds]
                    ]
                    interlend = {}
                    interlend["rawselectinds"] = inds
                    interlend[
                        "ms_adjusted_epoch(s)_mass__%s" % msfiled["mass"]
                    ] = msfiled["epoch(s)"][msinds]
                    interlend["MS(torr)_mass__%s" % msfiled["mass"]] = msfiled[
                        "ms_torr"
                    ][msinds]
                    ...
                    for k in datadict.keys():
                        ...
                        if (
                            "Ewe" in k
                        ):  # for MS beyond the end of eche time, assume open circuit, NaN=unknown potential and 0 current
                            interlend[k + "_MS"][indsarr == -1] = np.nan
                        elif "I" in k:
                            interlend[k + "_MS"][indsarr == -1] = 0.0
                    ...
                    interlend["t(s)_MS"] = (
                        msfiled["epoch(s)"][msinds] - datadict["epoch(s)"][0]
                    )
                    fni = "%s__mass__%s_%s_interlen.txt" % (
                        anak,
                        msfiled["mass"],
        ...
        ffn = "%s__%s.csv" % (
            anak,
            "-".join(self.fomnames[:3]),
        )  # name file by foms but onyl inlcude the 1st 3 to avoid long names
        self.multirunfiledict["fom_files"][ffn] = self.writefom_bare(
            destfolder,
            ffn,
            floatkeys=[],
            intfomkeys=["runint", "plate_id", self.fomnames[0]],
            strkeys=self.fomnames[1:],
        )
...
class Analysis__ECMS_Calibration(Analysis_Master_nointer):
    def __init__(self):
        ...
        self.dfltparams = {
            "masses": "2,15,26",
            "baseline_times_to_ave_wrt_valve_open_s": "300.,600.",
            "frac_cal_duration_to_use": "0.5,0.95",
            "instrument_QE_masses": "3.8,1.6,0.75",
            "modelling_basis__mM_torr": "3.96e+07,1.01e+08,2.57e+08",
    ...
    def perform(
        ...
        base_r0, base_r1 = [
            float(s.strip())
            for s in self.params["baseline_times_to_ave_wrt_valve_open_s"].split(",")
        ]
        cal_frac0, cal_frac1 = [
            float(s.strip()) for s in self.params["frac_cal_duration_to_use"].split(",")
        ]
        ...
        for filed in self.filedlist:
            ...
            concentration_mM = [
                float(s.strip()) for s in cald["concentration_mM"].split(",")
            ][masses.index(mass)]
            QE = [
                float(s.strip()) for s in self.params["instrument_QE_masses"].split(",")
            ][masses.index(mass)]
            ref_cal_factor = [
                float(s.strip())
                for s in self.params["modelling_basis__mM_torr"].split(",")
            ][masses.index(mass)]
            species = [s.strip() for s in cald["species"].split(",")][
                masses.index(mass)
            ]
            base_t0 = base_r0 * 1000.0 + cald["time_open_pervaporator_valve_ms"]
            base_t1 = base_r1 * 1000.0 + cald["time_open_pervaporator_valve_ms"]
            cal_t0 = (
                cald["time_switch_to_calibration_ms"]
                + 1000.0 * cald["duration_s"] * cal_frac0
            )
            cal_t1 = (
                cald["time_switch_to_calibration_ms"]
                + 1000.0 * cald["duration_s"] * cal_frac1
            )
            base_inds = np.where(
                (filed["ms_time_ms"] >= base_t0) & (filed["ms_time_ms"] <= base_t1)
            )
            cal_inds = np.where(
                (filed["ms_time_ms"] >= cal_t0) & (filed["ms_time_ms"] <= cal_t1)
            )
        ...
        filedesc, filestr = createcsvfilstr_bare(
            misccsvdlist, None, strfomkeys=["species", "mass"], return_file_desc=True
        )
...
class Analysis__ECMS_Fit_MS(Analysis_Master_FOM_Process):
    def __init__(self):
        ...
        self.dfltparams = {
            "ms_simulation_model": "1G_v2",
            "species": "H2,CH4,C2H4",
            "eche_techniques": "CV9",
            "max_per_species_FE": 1.1,
            "loss_fcn": "L2_3x_first_half_5x_positive",
            "select_ana": "ana__1",
            "cal_ana": "ana__2",
            "voltage_error_V": 0.01,
            "num_voltage_errors_to_abort": 30,
            "SGfilter_nptsoneside": 64,
            "SGfilter_order": 3,
            "ignore_FE_time_start_end_s": 8.0,
            "resistance_to_compensate_Ohm": 60.0,
            "max_current_valid_FE": -0.00003,
            "min_max_each_fit_param": "0.1,1.6;-1.4,-0.4",
        }
        self.params = copy.copy(self.dfltparams)
    ...
    def getapplicablefomfiles(self, anadict):  # called by getapplicablefilenames
        if not anadict is None and self.params["select_ana"] in anadict.keys():
            self.num_ana_considered, self.filedlist = stdgetapplicablefomfiles(
                anadict,
                params={
                    "select_ana": self.params["select_ana"],
                    "select_fom_keys": "num_MS_masses",
                },
            )
    ...
    def processnewparams(
        self, calcFOMDialogclass=None, anadict=None
    ):  # called after getapplicablefomfiles in getapplicablefilenames
        self.description = ""
        if anadict is None:
        ...
        try:
            ...
            self.filedlist = []
            return
        self.description = "Fit ECMS from %s calibrated by %s to the %s models" % (
            self.params["select_ana"],
            self.params["cal_ana"],
            self.params["ms_simulation_model"],
        )
    ...
    def perform(
        ...
        loss_fcn = loss_fcn_dict[self.params["loss_fcn"]]
        species = [s.strip() for s in self.params["species"].split(",")]
        sim_lib_dict = read_ecms_simulation_data(
            self.params["ms_simulation_model"], species
        )
        ...
        check_pars_out_of_range = lambda pars: True in [
            (v < minmax[0] or v > minmax[1])
            for minmax, v in zip(self.minmax_fitpars, pars)
            if isinstance(minmax, tuple) and len(minmax) == 2
        ]
        allow_fit_par_inds = [
            ind
            for ind, model_pars in enumerate(sim_lib_dict["model_params"])
            if not check_pars_out_of_range(model_pars)
        ]
        cal_p = os.path.join(destfolder, self.cal_filed["fn"])
        calfomd = readcsvdict(
            cal_p, self.cal_filed, returnheaderdict=False, includestrvals=True
        )
        ...
        self.initfiledicts(
            runfilekeys=["inter_rawlen_files", "inter_files"], runklist=runklist
        )
        self.multirunfiledict["misc_files"] = {}
        ...
        summ_plot_fom_keys = (
            ["Echem.charge.C"]
            + ["%s.charge.C" % sp for sp in species]
            + ["Other.charge.C"]
        )
        techd_fomdlist = {}
        for techcount, tech in enumerate(tech_set):  # per technique
            if (
                not tech in self.params["eche_techniques"]
            ):  # direct search in param string
                continue
            self.fomnames = [
                "max.FETotal"
            ]  # reset every technique in case keys are different
            self.strkeys_fomdlist = ["eche_technique", "fn_bestfit_eche"]
            techd_fomdlist[tech] = []
            pid_sid_datafomind__this_tech = sorted(
                [
                    (pid, sid, count)
                    for count, (pid, sid, te) in enumerate(
                        zip(datafomd["plate_id"], datafomd["sample_no"], tech_list)
                    )
                    if te == tech
                ]
            )
            datafominds = [tup[2] for tup in pid_sid_datafomind__this_tech]
            tech_summary_plot_data = dict(
                [
                    ("FE__%s" % sp, [[] for i in range(len(datafominds))])
                    for sp in species
                ],
                fomlistd=dict([(fomk, []) for fomk in summ_plot_fom_keys]),
            )
            ...
            for smpcount, datafomind in enumerate(datafominds):  # per sample,plate
                ...
                fomd = {}
                rawlend = {}
                for k in list(FOMKEYSREQUIREDBUTNEVERUSEDINPROCESSING) + [
                    "eche_technique"
                ]:
                ...
                (
                    ...
                    echem_i_smooth,
                    segment_index_arr,
                ) = eche_split_current_by_sweep_and_generate_intended_voltage(
                    tech, sim_lib_dict, echem_t, echem_i, smooth_paramd=self.params
                )
                vdiff = echem_v - sim_basis_v
                if (
                    np.isnan(sim_basis_v).sum() > 0
                ...
                echem_fe_test_inds = (
                    ...
                    & (
                        (echem_t[-1] - echem_t)
                        > self.params["ignore_FE_time_start_end_s"]
                    )
                    & (echem_i_smooth < self.params["max_current_valid_FE"])
                )
                ...
                rawlend["Ewe(compensatedVrhe)"] = (
                    sim_basis_v
                    - echem_i_smooth * self.params["resistance_to_compensate_Ohm"]
                )
                for spcount, sp in enumerate(species):
                    ...
                    inds = [
                        fd["keys"].index(k)
                        for k in (
                            "t(s)_MS,rawselectinds,MS(torr)_mass__%d" % cald["mass"]
                        ).split(",")
                    ]
                    ...
                    def get_sim_arrs_paramind_timewerteche(paramind, timewrteche):
                        ...
                        mod_t = mod_t[inds]
                        mod_c, mod_ip, mod_out = [
                            sim_lib_dict[sp][k][paramind][inds]
                            for k in ["conc_in_soltn(M)", "Ip(A)", "output(torr)"]
                        ]
                    ...
                    for model_pars_ind in allow_fit_par_inds:
                        ...
                        (
                            ...
                            mod_ip,
                            mod_out,
                        ) = get_sim_arrs_paramind_timewerteche(model_pars_ind, ms_t)
                        (
                            emod_t,
                            emod_c,
                            emod_ip,
                            emod_out,
                        ) = get_sim_arrs_paramind_timewerteche(model_pars_ind, echem_t)
                        max_fe_i1_is_1 = max(
                            (-emod_ip / echem_i_smooth)[echem_fe_test_inds]
                        )
                        ...
                        Ip_1 = (
                            0.0
                            if Ip_1 < 0.0
                            else (Ip_1_max if Ip_1 > Ip_1_max else Ip_1)
                        )
                        fitsummaryd = {}
                    ...
                    mod_t, mod_c, mod_ip, mod_out = get_sim_arrs_paramind_timewerteche(
                        allow_fit_par_inds[bestfitind], echem_t
                    )
                    ...
                    fomd["%s_bestfit.fitloss" % sp] = fitsummarydlist[bestfitind][
                        "fitloss"
                    ]
                    fomd["%s.maxFE" % sp] = max(
                        rawlend["FE__%s" % sp][echem_fe_test_inds]
                    )
                    fomd["%s.charge.C" % sp] = (
                        rawlend["Ip(A)__%s" % sp][:-1] * (echem_t[1:] - echem_t[:-1])
                    ).sum()
                    fomd["%s.Ip1.A" % sp] = Ip_1_bestfit
                    for parname, v in zip(
                        sim_lib_dict["metadata"]["parameter_names"],
                        sim_lib_dict["model_params"][allow_fit_par_inds[bestfitind]],
                    ):
                    ...
                    plt.subplots_adjust(
                        left=0.08, bottom=0.08, right=0.96, top=0.94, wspace=0.43
                    )
                    ax = ax2
                    ax.cla()
                    plt.sca(ax)
                    partups = [
                        tuple(arr)
                        for arr in sim_lib_dict["model_params"][allow_fit_par_inds, :2]
                    ]
                    partups_set = sorted(list(set(partups)))
                    plotdata = []
                    repvals = [
                        np.min([v for v, t in zip(l_fitloss, partups) if t == pt])
                        for pt in partups_set
                    ]
                    ...
                    for fitdcount, fitd in enumerate(fitsummarydlist):
                        st = (
                            {"lw": 1.5, "c": "r"}
                            if fitdcount == bestfitind
                            else {"lw": 0.7, "c": "grey"}
                        )
                        ax.plot(fitd["t(s)_MS_model"], fitd["Fit_MS(torr)"], "-", **st)
                    ax.plot(
                        fitd["t(s)_MS_model"], fitd["Adjusted_MS(torr)"], "ko", ms=6
                    )  # just use the last fitd since these arrays the same for every fitd
                    ...
                    self.multirunfiledict["misc_files"][
                        fnim0
                    ] = "ecms_fitsummary_png_file;"
                    plt.savefig(p, dpi=300)
                    p = os.path.join(destfolder, fnim1)
                    self.multirunfiledict["misc_files"][
                        fnim1
                    ] = "ecms_fitsummary_svg_file;"
                    plt.savefig(p)
                    # plt.show()
                    p = os.path.join(destfolder, fni)
                    fmtd = dict(
                    ...
                    kl = saveinterdata(p, interlend, savetxt=True, fmt=fmtd)
                    self.runfiledict["run__%d" % fomd["runint"]]["inter_files"][fni] = (
                        "%s;%s;%d;%d;%d"
                        % (
                            "ecms_interlen_file",
                            ",".join(kl),
                ...
                fomd["Echem.charge.C"] = (
                    echem_i_smooth[:-1] * (echem_t[1:] - echem_t[:-1])
                ).sum()
                ...
                plt.subplots_adjust(
                    left=0.11, bottom=0.08, right=0.96, top=0.94, wspace=0.28
                )
                for ax, s, st, loc in [
                    (ax1, "Ip(A)__", "Ismooth(A)", 4),
                    (ax2, "FE__", "FE_Total", 1),
                ]:
                    ax.cla()
                    plt.sca(ax)
                ...
                for segi in range(max(segment_index_arr) + 1):
                    ...
                    ax.plot(
                        rawlend["Ewe(compensatedVrhe)"][inds],
                        echem_i_smooth[inds],
                        "k" + ls,
                    )
                    axr = ax.twinx()
                    ...
                    for sp in species:
                        k = "FE__%s" % sp
                        tech_summary_plot_data[k][smpcount] += [
                            (rawlend["Ewe(compensatedVrhe)"][inds], rawlend[k][inds])
                        ]
            ...
            for spcount, sp in enumerate(species[:3]):
                ...
                k = "FE__%s" % sp
                for count, (col, seglist, smp) in enumerate(
                    zip(cols, tech_summary_plot_data[k], smps)
                ):
            ...
            for fomc, fomm, fomk in zip(
                ...
                ax.plot(
                    smprange,
                    tech_summary_plot_data["fomlistd"][fomk][:max_samples_to_plot],
                    "-",
                    marker=fomm,
                    label=fomk.partition(".")[0],
                    c=fomc,
                    ms=4,
                )
            ax.set_ylabel("Charge (C)")
            ax.set_xticks(smprange)
            ax.set_xticklabels(["%d" % smp for smp in smps], rotation=45.0)
            for ticklabel, tickcolor in zip(ax.get_xticklabels(), cols):
                ticklabel.set_color(tickcolor)
            plt.xlim(-0.5, len(smps) + 2.9)
            plt.legend(
                loc=1, fontsize="small", frameon=False, numpoints=1, handlelength=0
            )
            plt.subplots_adjust(
                left=0.11, bottom=0.11, right=0.96, top=0.97, wspace=0.25, hspace=0.12
            )
            ...
            self.writefom(
                destfolder,
                anak,
                anauserfomd=anauserfomd,
                strkeys_fomdlist=self.strkeys_fomdlist,
                fn="%s__tech__%s__%s.csv" % (anak, tech, "-".join(self.fomnames[:3])),
            )
