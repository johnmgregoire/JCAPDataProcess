...
def handlenan_savgol_filter(
    ...
    if len(nans) > 1 and len(nans) < len(d_arr):
        ...
        if len(naninds) > 1:
            d_arr[naninds] = numpy.array(
                [
                    numpy.nanmean(d_arr[max(0, nind - 3) : min(nind + 3, len(d_arr))])
                    for nind in naninds
                ]
            )
    try:
...
def BGgetapplicablefilenames(
    ...
    Afiledlist = [
        dict(
            ...
            ana=anak,
            fn=fnk,
            sample_no=int(tagandkeys.split(";")[4].strip()),
            nkeys=len(tagandkeys.split(";")[1].split(",")),
            num_header_lines=int(tagandkeys.split(";")[2]),
            Akeyind=tagandkeys.split(";")[1].split(",").index("abs_smth_refadj_scl"),
    ...
    if len(Afiledlist) == 0:
        return 0, []
    num_files_considered, filedlist = stdgetapplicablefilenames(
        expfiledict,
        usek,
        techk,
        typek,
    ...
    filedlist2 = [
        dict(d, Afiled=Afiledlist[Asmp.index(d["sample_no"])])
        for d in filedlist
    ...
    return (
        ...
        filedlist2,
    )  # inside of each dict in this list is a 'Afiled' with key 'fn'. That file in the analysis folder is an intermediate data arr whose column 'Akeyind' is the absorption array


def TRgetapplicablefilenames(
    expfiledict,
    usek,
    techk,
    typek,
    ...
    for k in list(refdict__filedlist.keys()):
        ...
        #        filedlist is
        if ref_run_selection.startswith(
            "run__"
        ):  # filter to only use refs from user-specified list
        ...
        if len(filedlist) == 0:
            if gui_mode_bool:
                print(
                    "NO REFERENCE DATA AVAILABLE FOR %s in %s"
                    % (
                        k,
    ...
    ntemp, Rfiledlist = stdgetapplicablefilenames(
        expfiledict,
        usek,
        "R_UVVIS",
        typek,
    ...
    Tfiledlist = [
        dict(Td, Rfiled=Rfiledlist[Rsmp.index(Td["fn"].partition("_")[0])])
        for Td in Tfiledlist
        if Td["fn"].partition("_")[0] in Rsmp
    ]
    return num_files_considered, Tfiledlist, refdict__filedlist


def DRgetapplicablefilenames(
    expfiledict,
    usek,
    techk,
    typek,
...
def stdcheckoutput(fomdlist, fomnames):
    # ?fomdlist
    nancount = [
        (not k in fomdlist) or numpy.isnan(d[k]) for d in fomdlist for k in fomnames
    ].count(True)
    return nancount, 1.0 * nancount / (len(fomdlist) * len(fomnames))
...
def refadjust_TR(
    ...
    if mini >= min_mthd_allowed and mini <= min_limit:
        x = (sdata[mini_idx] - (1.0 - 0.01) * sdark[mini_idx]) / (0.01 * sref[mini_idx])
        mthd_smth_refadj = (sdata - sdark) / (x * sref - sdark)
        min_rescaled = True
    maxi = numpy.nanmax(data)
    maxi_idx = numpy.nanargmax(mthd_smth)
    if maxi <= max_mthd_allowed and maxi >= max_limit:
        x = (sdata[maxi_idx] - (1.0 - 0.99) * sdark[maxi_idx]) / (0.99 * sref[maxi_idx])
        mthd_smth_refadj = (sdata - sdark) / (x * sref - sdark)
        max_rescaled = True
...
def binarray(data, bin_width=1):
    ...
    reddata = numpy.array(
        [
            numpy.mean(data[loc : min(loc + bin_width, numpy.size(data))])
...
def check_inrange(data, min_limit=0.0, max_limit=1.0):
    return numpy.min(data) > min_limit and numpy.max(data) < max_limit
...
class Analysis__TR_UVVIS(Analysis_Master_inter):
    def __init__(self):
        ...
        self.plotparams["plot__1"][
            "x_axis"
        ] = "hv"  # this is a single key from raw or inter data
        ...
        self.tauc_pow = dict(
            [("DA", 2), ("IA", 0.5), ("DF", 2.0 / 3.0), ("IF", 1.0 / 3.0)]
        )
    ...
    def processnewparams(self, calcFOMDialogclass=None):
        ...
        if numpy.array(
            ...
            idialog = messageDialog(
                self,
                "You enetered invalid parameters. They are being restored to defaults. Please try again.",
            )
            idialog.exec_()
        if self.params["reffilesmode"] not in ["time", "static"]:
            ...
            idialog = messageDialog(
                self,
                "Pls use static or time as options for reffilesmode, resetting to static. Please try again for time option.",
            )
            idialog.exec_()
        self.csvheaderdict = dict({}, csv_version="1", plot_parameters={})
        for idx, fom in enumerate(self.fomnames):
            self.csvheaderdict["plot_parameters"]["plot__" + str(idx + 1)] = dict(
                {},
                fom_name=fom,
                colormap="jet",
        ...
        for idx, qfom in enumerate(self.qualityfoms):
            self.qualityfomcsvheaderdict["plot_parameters"][
                "plot__" + str(idx + 1)
            ] = dict(
                {},
                fom_name=qfom,
    ...
    def getapplicablefilenames(
        ...
        (
            ...
            ref_run_selection=self.params["ref_run_selection"],
    ...
    def check_input(self, critfracapplicable=0.9):
        fracapplicable = 1.0 * len(self.filedlist) / self.num_files_considered
        return (
            fracapplicable > critfracapplicable,
            "%d files, %.2f of those available, do not meet requirements"
            % (len(self.filedlist) - self.num_files_considered, 1.0 - fracapplicable),
        )
    ...
    def check_output(self, critfracnan=0.9):
        ...
        return (
            fracnan > critfracnan,
            "%d samples, %.2f fraction of total samples have NaN in the absorption spectra in the wavelength range %.2f to %.2f"
            % (
                numnan,
                fracnan,
                self.params["chkoutput_wlrange"][0],
                self.params["chkoutput_wlrange"][1],
            ),
        )
    ...
    def setuprefdata(self, refkeymap, static_ref_fnd, anak="", destfolder=None):
        ...
        refd = (
            {}
        )  # refd will be a dictionary with 4 keys that makes a good start for the intermediate ref dictionary with raw-data-length arrays
        refd_all = {}
        for count, (rktup, rk) in enumerate(refkeymap):
            ...
            refdat3d = [
                filed["readfcn"](*filed["readfcn_args"], **filed["readfcn_kwargs"])[
                    :, ::-1
                ]
            ...
            if (
                not numpy.all(
                    numpy.array([len(refdat2d[0]) for refdat2d in refdat3d])
                    == len(refd["wl_fullrng"])
                )
            ) or not check_wl(
                numpy.array(
                    [refd["wl_fullrng"]] + [refdat2d[0] for refdat2d in refdat3d]
                )
            ):
                if self.debugmode:
            ...
            refd_all[rk] = numpy.float32(
                [refdat2d[1:].mean(axis=0) for refdat2d in refdat3d]
            )  # this mean is over the signal1,2,3 so that each refd_all[rk] has first index of ref filed so it is indexed the same as self.refdict__filedlist[rktup] (second index is data ooint)
        if self.params["reffilesmode"] == "static":
            ...
            for filed in self.filedlist:
                ...
                try:
                    ...
                    smp_time = smp_time_fcn(filed)
                for rktup, rk in refkeymap:
                    ref_time_arr = numpy.array(
                        [
                            smp_time_fcn(ref_filed)
                            for ref_filed in self.refdict__filedlist[rktup]
                        ]
                    )
                    refd[filed["sample_no"]][rk] = refd_all[rk][
                        numpy.argmin((ref_time_arr - smp_time) ** 2)
                    ]
        ...
        if not destfolder is None:
            for rktup, rk in refkeymap:
                ...
                for sig, fn in zip(
                    refd_all[rk],
                    [filed["fn"] for filed in self.refdict__filedlist[rktup]],
                ):
    ...
    def perform(
        ...
        anauserfomd={},
        expfiledict=None,
    ):  # TODO propogate , expfiledict=None
        self.initfiledicts(runfilekeys=["inter_rawlen_files", "inter_files"])
        self.multirunfiledict["misc_files"] = {}
        self.fomdlist = []
        refkeymap = [
            (("ref_dark", "T_UVVIS"), "Tdark"),
            (("ref_light", "T_UVVIS"), "Tlight"),
            (("ref_dark", "R_UVVIS"), "Rdark"),
            (("ref_light", "R_UVVIS"), "Rlight"),
        ]
        static_ref_fnd = dict(
            [
                ("Tdark", lambda x: numpy.min(x, axis=0)),
                ("Tlight", lambda x: numpy.max(x, axis=0)),
                ("Rdark", lambda x: numpy.min(x, axis=0)),
                ("Rlight", lambda x: numpy.max(x, axis=0)),
            ]
        )
        for filed in [
            ...
            filed["keyinds"] = [0] + filed["keyinds"][
                1
                + self.params["exclinitcols"] : len(filed["keyinds"])
                - self.params["exclfincols"]
            ]  # update the keyinds to keep zero index, which is the wl, and then select other columns
            if (
                len(filed["keyinds"]) == 1
            ):  # only wavelength left so need to abort analysis
                if self.debugmode:
                    raise ValueError(
                        "select ref columns resulted in no remaining Signals"
                    )
                self.writefom(
                    destfolder, anak, anauserfomd=anauserfomd, createdummyfomdlist=True
                )
                return
        closeziplist = self.prepare_filedlist(
            ...
            fnk="fn",
        )  # combine the filed, Rfiled, etc. together in 1 list so that any runzips are only opened once
        referror, refd_fn = self.setuprefdata(
            refkeymap, static_ref_fnd, anak=anak, destfolder=destfolder
        )
        if referror:
            self.writefom(
                destfolder, anak, anauserfomd=anauserfomd, createdummyfomdlist=True
        ...
        for filed in self.filedlist:
            ...
            Rfiled = filed["Rfiled"]
            Rfn = Rfiled["fn"]
            Tdataarr = filed["readfcn"](
                *filed["readfcn_args"], **filed["readfcn_kwargs"]
            )[:, ::-1]
            Rdataarr = Rfiled["readfcn"](
                *Rfiled["readfcn_args"], **Rfiled["readfcn_kwargs"]
            )[:, ::-1]
            #            print numpy.shape(Tdataarr),numpy.shape(Rdataarr)
            fomdict, rawlend, interlend = self.fomd_rawlend_interlend(
                Tdataarr, Rdataarr, refd_fn(filed["sample_no"])
            )
            if not numpy.isnan(
                filed["sample_no"]
        ...
        if (
            destfolder is None
        ):  # dont' do anything with quality items if output not being saved
            return
        ...
        qualitycsvfilstr = createcsvfilstr(
            self.fomdlist, [], intfomkeys=["runint", "plate_id"] + self.qualityfoms
        )  # , fn=fnf)
        ...
        totnumheadlines = writecsv_smpfomd(
            ...
            qualitycsvfilstr,
            headerdict=dict(
                {}, csv_version=self.qualityfomcsvheaderdict["csv_version"]
            ),
        )
        self.multirunfiledict["misc_files"][fnf] = "%s;%s;%d;%d" % (
            "csv_fom_file",
            ",".join(["sample_no", "runint", "plate_id"] + self.qualityfoms),
            totnumheadlines,
            len(self.fomdlist),
        )
    ...
    def fomd_rawlend_interlend(self, Tdataarr, Rdataarr, refd):
        if (
            Tdataarr.shape[1] != Rdataarr.shape[1]
            or Tdataarr.shape[1] != refd["Tdark"].shape[0]
        ):
            return [("testfom", numpy.nan)], {}, {}
        #        print Tdataarr[0][0:5],Rdataarr[0][0:5],refd['wl'][0][0:5]
        if not check_wl(
            numpy.array(numpy.s_[Tdataarr[0], Rdataarr[0], refd["wl_fullrng"]])
        ):
        ...
        if (
            ...
            raise ValueError(
                "Insufficient signals to remove %d initial signals and %d end signals"
                % (self.params["exclinitcols"], self.params["exclfincols"])
            )
        else:
...
class Analysis__DR_UVVIS(Analysis__TR_UVVIS):
    ...
    def perform(
        ...
        self.fomdlist = []
        refkeymap = [
            (("ref_dark", "DR_UVVIS"), "DRdark"),
            (("ref_light", "DR_UVVIS"), "DRlight"),
        ]
        static_ref_fnd = dict(
            [
                ("DRdark", lambda x: numpy.min(x, axis=0)),
                ("DRlight", lambda x: numpy.max(x, axis=0)),
            ]
        )
        for filed in [
            fd for rktup, rk in refkeymap for fd in self.refdict__filedlist[rktup]
        ...
        for filed in self.filedlist:
            fn = filed["fn"]
            DRdataarr = filed["readfcn"](
                *filed["readfcn_args"], **filed["readfcn_kwargs"]
            )[:, ::-1]
            #            print numpy.shape(DRdataar)
            fomdict, rawlend, interlend = self.fomd_rawlend_interlend(
                DRdataarr, refd_fn(filed["sample_no"])
            )
            if not numpy.isnan(
                filed["sample_no"]
            ):  # do not save the fom but can save inter data
        ...
        # TODO: if quality foms are integers, append them to the intfomkeys and pass [] as 2nd argument
        qualitycsvfilstr = createcsvfilstr(
            self.fomdlist, self.qualityfoms, intfomkeys=["runint", "plate_id"]
        )  # , fn=fnf)
        p = os.path.join(destfolder, fnf)
        totnumheadlines = writecsv_smpfomd(
            p, qualitycsvfilstr, headerdict=self.qualityfomcsvheaderdict
        )
        self.multirunfiledict["misc_files"][fnf] = "%s;%s;%d;%d" % (
            "csv_fom_file",
            ",".join(["sample_no", "runint", "plate_id"] + self.qualityfoms),
    ...
    def fomd_rawlend_interlend(self, DRdataarr, refd):
        ...
        if (
            ...
            inter_rawlend["DR_av-signal"] = DRdataarr[
                1
                + self.params["exclinitcols"] : DRdataarr.shape[0]
                - self.params["exclfincols"]
            ].mean(axis=0)
            inter_rawlend["DR_fullrng"] = (
                inter_rawlend["DR_av-signal"] - refd["DRdark"]
            ) / (refd["DRlight"] - refd["DRdark"])
            inter_rawlend["abs" + "_fullrng"] = (
                1.0 - inter_rawlend["DR_fullrng"]
            ) ** 2.0 / (2.0 * inter_rawlend["DR_fullrng"])
            inds = numpy.where(
                numpy.logical_and(
                    inter_rawlend["wl_fullrng"] > self.params["lower_wl"],
                    inter_rawlend["wl_fullrng"] < self.params["upper_wl"],
            ...
            (
                fomd["min_rescaled"],
                fomd["max_rescaled"],
                inter_selindd["DR" + "_smth" + "_refadj"],
            ) = refadjust(
                inter_selindd["DR" + "_smth"],
                self.params["min_mthd_allowed"],
                self.params["max_mthd_allowed"],
            )
            inter_selindd["abs" + "_smth_refadj"] = (
                1.0 - inter_selindd["DR" + "_smth_refadj"]
            ) ** 2.0 / (2.0 * inter_selindd["DR" + "_smth_refadj"])
            inter_selindd["abs_smth_refadj_scl"] = inter_selindd[
                "abs_smth_refadj"
            ] / numpy.nanmax(inter_selindd["abs_smth_refadj"])
            chkoutput_inds = numpy.where(
...
class Analysis__T_UVVIS(Analysis__TR_UVVIS):
    ...
    def perform(
        ...
        for filed in self.filedlist:
            ...
            #            print numpy.shape(DRdataar)
            fomdict, rawlend, interlend = self.fomd_rawlend_interlend(
                Tdataarr, refd_fn(filed["sample_no"])
            )
            if not numpy.isnan(
                filed["sample_no"]
            ):  # do not save the fom but can save inter data
...
class Analysis__BG(Analysis_Master_inter):
    def __init__(self):
        ...
        self.dfltparams = dict(
            [
                ...
                ("maxtol", 1e-03),
                # fitting params
                ("merge_linsegslopediff_percent", 4.0),
                ("max_merge_differentialTP", 10000.0),
                # params for merging linsegs
                ("DA_min_allowedslope", -1),
                ("IA_min_allowedslope", -0.5),
                ("DF_min_allowedslope", -0.75),
                ("IF_min_allowedslope", -0.5),
                ("min_finseglength", 0.1),
                # parameters before entering bg identification
                ("min_bgTP_diff", 0.2),
                ("min_bkgrdslope", -0.05),
                ("min_bgbkgrdslopediff", 0.4),
                ("min_bgTP_finseg_diff", 0.3),
                ("min_bgfinalseglength", 0.2),
                # params for bg and bkgrd
                ("analysis_types", "DA,IA"),
                ("maxbgspersmp", 2),
                ("chkoutput_types", "DA,IA"),
                ("abs_minallowedslope", -1.0e13),
                ("max_absolute_2ndderiv", 1.0e13),
                ("use_absderivs_forpeaks", False),
        ...
        self.requiredkeys = (
            []
        )  # required keys aren't needed for selecting applicable files because the requiremenets for inter_files will be sufficient. Only put requireded_keys that are need in the analysis and these required_keys are of the raw data not inter_data
        self.optionalkeys = []
        self.requiredparams = []
    ...
    def processnewparams(self, calcFOMDialogclass=None):
        ...
        self.fomnames = [
            ...
            xfom
            for bgtyp in self.params["analysis_types"]
            for xfom in [bgtyp + "_fit_minslope", bgtyp + "_tol", bgtyp + "_rel_rss"]
        ]
        ...
        self.fom_chkqualitynames = [
            bgtyp + "_bg_0" for bgtyp in self.params["chkoutput_types"]
        ]
        self.histfomnames = [
            bgtyp + "_fit_minslope" for bgtyp in self.params["analysis_types"]
        ]
        self.plotparams = dict({}, plot__1={"x_axis": "hv"})
        self.plotparams["plot__1"][
            "x_axis"
        ...
        for idx in range(len(self.params["analysis_types"])):
            self.csvheaderdict["plot_parameters"][
                ...
                {},
                fom_name=self.params["analysis_types"][idx] + "_" + "bg_0",
                colormap="jet_r",
                colormap_over_color="(0.5,0.5,0.5)",
                colormap_under_color="(0.,0.,0.)",
            )
            self.csvheaderdict["plot_parameters"][
                "plot__" + str(np_ana * idx + 2)
            ] = dict(
                {},
                fom_name=self.params["analysis_types"][idx] + "_" + "bg_repr",
                colormap="jet_r",
                colormap_over_color="(0.5,0.5,0.5)",
                colormap_under_color="(0.,0.,0.)",
            )
            self.csvheaderdict["plot_parameters"][
                "plot__" + str(np_ana * idx + 3)
            ] = dict(
                {},
                fom_name=self.params["analysis_types"][idx] + "_" + "bgcode_repr",
                colormap="jet_r",
                colormap_over_color="(0.5,0.5,0.5)",
                colormap_under_color="(0.,0.,0.)",
            )
            self.csvheaderdict["plot_parameters"][
                "plot__" + str(np_ana * idx + 4)
            ] = dict(
                {},
                fom_name=self.params["analysis_types"][idx] + "_" + "bgslope_repr",
                colormap="jet",
                colormap_over_color="(0.5,0.5,0.5)",
                colormap_under_color="(0.,0.,0.)",
            )
            self.csvheaderdict["plot_parameters"][
                "plot__" + str(np_ana * idx + 5)
            ] = dict(
                {},
                fom_name=self.params["analysis_types"][idx] + "_" + "bkgrdslope_repr",
                colormap="jet_r",
                colormap_over_color="(0.5,0.5,0.5)",
                colormap_under_color="(0.,0.,0.)",
            )
            self.csvheaderdict["plot_parameters"][
                "plot__" + str(np_ana * idx + 6)
            ] = dict(
                {},
                fom_name=self.params["analysis_types"][idx] + "_" + "rel_rss",
                colormap="jet",
                colormap_over_color="(0.5,0.5,0.5)",
                colormap_under_color="(0.,0.,0.)",
            )
        ...
        if self.params["calcbg_abscissa"]:
            for idx in range(len(self.params["analysis_types"])):
                ...
                extn = "_a"
                self.csvheaderdict["plot_parameters"][
                    "plot__" + str(nplots + np_ana * idx + 1)
                ] = dict(
                    {},
                    fom_name=self.params["analysis_types"][idx] + extn + "_" + "bg_0",
                    colormap="jet_r",
                    colormap_over_color="(0.5,0.5,0.5)",
                    colormap_under_color="(0.,0.,0.)",
                )
                self.csvheaderdict["plot_parameters"][
                    "plot__" + str(nplots + np_ana * idx + 2)
                ] = dict(
                    {},
                    fom_name=self.params["analysis_types"][idx]
                    + extn
                    + "_"
                    + "bg_repr",
                    colormap="jet_r",
                ...
                self.csvheaderdict["plot_parameters"][
                    "plot__" + str(nplots + np_ana * idx + 3)
                ] = dict(
                    {},
                    fom_name=self.params["analysis_types"][idx]
                    + extn
                ...
                self.csvheaderdict["plot_parameters"][
                    "plot__" + str(nplots + np_ana * idx + 4)
                ] = dict(
                    {},
                    fom_name=self.params["analysis_types"][idx]
                    + extn
    ...
    def getapplicablefilenames(
        ...
        self.num_files_considered, self.filedlist = BGgetapplicablefilenames(
            expfiledict,
            usek,
            techk,
            typek,
    ...
    def check_input(self, critfracapplicable=0.9):
        ...
        return (
            fracapplicable > critfracapplicable,
            "%d files, %.2f fraction of those available, do not meet requirements"
            % (len(self.filedlist) - self.num_files_considered, 1.0 - fracapplicable),
        )
    ...
    def check_output(self, critfracnan=0.5):
        ...
        for nm in self.fom_chkqualitynames:
            ...
            tfracnan += fracnan
            ostrl.append(
                "%d samples, %.2f fraction of total samples with no NaN in absorption have no %s band gaps; "
                % (numnan, fracnan, nm)
            )
        ...
        for idx, linfitd in enumerate(self.linfitdlist):
            ...
            if idx == 0:
                wstr += (
                    cdlmtr.join(
                        ["sample_no"]
                        + [k for k in sorted(linfitd.keys()) if k != "sample_no"]
                    )
                    + rdlmtr
                )
            ...
            wstr += cdlmtr.join(
                [
                    str(numpy.round(linfitd[k], decimals=4))
    ...
    def perform(
        ...
        for filed in self.filedlist:
            ...
            Adataarr = self.readdata(
                os.path.join(destfolder, Afn),
                Afiled["nkeys"],
                None,
                num_header_lines=Afiled["num_header_lines"],
                zipclass=None,
            )  # the inter_data cannot be zipped because it is from the active ana, inds=None will return all inds
            for k, v in zip(Afiled["keys"], Adataarr):
                rawlend[k] = v
            #            print rawlend.keys()
            fomdict, linfitd, selindd = self.fomd_rawlend_interlend(
                rawlend
            )  # use datadict as the argument or use the necessary keys like 'wl' amd 'abs_smth_scl'
            if not numpy.isnan(
                filed["sample_no"]
            ):  # do not save the fom but can save inter data
                fomdict = dict(
        ...
        if len(self.linfitdlist) > 0:
            ...
            self.multirunfiledict["misc_files"][fn_linfit] = (
                "csv_linfitparams" + "_file;"
            )
    ...
    def fomd_rawlend_interlend(self, rawlend):
        inter_selindd = {}
        abs2bg_inds = numpy.where(
            numpy.logical_and(
                rawlend["hv"] < 1239.8 / self.params["lower_wl"],
                rawlend["hv"] > 1239.8 / self.params["upper_wl"],
            )
        )[0]
        ...
        for typ in self.params["analysis_types"]:
            ...
            temparr = [
                v for k, v in list(inter_linfitd.items()) if typ + "_slopes" in k
            ]
