...
def ECHEPHOTO_checkcompletedanalysis_inter_filedlist(
    ...
    filedlist2 = [
        dict(filed, ana__inter_filed=interfns[0], ana__inter_rawlen_filed=interfns[1])
        for filed, interfns in zip(filedlist, interfnks_filedlist)
        if len(interfns) == 2
    ]  # 2 is 1 for inter_files and then for inter_rawlenfiles. if less than 2 then the analysis wasn't done or failed, if >2 then analysis done multiple times
    if (
...
class Analysis__Pphotomax(Analysis_Master_inter):
    ...
    def perform(
        ...
        self.fitconds = (
            True if self.params["function_type"].endswith("_lims") else False
        )
        self.fomdlist = []
        for filed in self.filedlist:
            ...
            try:
                ...
                (
                    ...
                    interlend,
                    miscfilestr,
                ) = self.fomtuplist_rawlend_interlend(datadict, filed)
            except:
                if self.debugmode:
                    raiseTEMP
