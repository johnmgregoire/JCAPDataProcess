...
def ECHEPHOTO_checkcompletedanalysis_files_by_sample(filedlist, expfiledict, anadict, requiredanalysis='Analysis__Iphoto', tech=None, gui_mode_bool=False):
    
    anak_ftklist=[(anak, [ftk for ftk in anav.keys() if 'files_run__' in ftk]) for anak, anav in anadict.iteritems()\
           if anak.startswith('ana__') and anav['name']==requiredanalysis and True in ['files_run__' in ftk for ftk in anav.keys()]]
    ...
    if len(tech)>0:
        ...
        anak_ftklist=[(anak,ftkl) for anak, ftkl in anak_ftklist if tech is None or anadict[anak]['technique'][:2]==tech[:2]]
    ...
    genfomdlist=lambda anad:[dict({}, fn=fnk, num_header_lines=int(tagandkeys.split(';')[2]), keys=tagandkeys.split(';')[1].split(',')) for fnk, tagandkeys in anad['files_multi_run']['fom_files'].iteritems()]
    interfnks_filedlist=[\
          [dict({}, anakeys=[anak, ftk, typek, fnk], anak=anak, fn=fnk, runk=ftk[6:], sample_no=filed['sample_no'], \
                                 nkeys=len(tagandkeys.split(';')[1].split(',')), num_header_lines=int(tagandkeys.split(';')[2]), \
                                 keys=tagandkeys.split(';')[1].split(','), fom_files_dlist=genfomdlist(anadict[anak])) \
    ...
    for dlist in interfnks_filedlist:
        for d in dlist:
            ...
            runk=d['runk']
            if 'user_run_foms' in expfiledict[runk].keys() and 'user_fom_led_wavelength' in expfiledict[runk]['user_run_foms'].keys():#user can override wavelength but presently no other params taken from here
                d['illumination_wavelength']=float(expfiledict[runk]['user_run_foms']['user_fom_led_wavelength'])
                ...
                if isinstance(togvallist, str) and ',' in togvallist:
                    ...
                    for k, v in expfiledict[runk]['parameters'].iteritems():
                        if not (k.startswith('illumination_') and ',' in v):
                            continue
                        d[k]=attemptnumericconversion_tryintfloat(v.split(',')[toglistind].strip())
                else:
                    for k, v in expfiledict[runk]['parameters'].iteritems():
                        if not k.startswith('illumination_') and isinstance(v, (int, float)):
                            continue
                        d[k]=v
            
    interfnks_filedlist=[[d for d in dlist if 'illumination_wavelength' in d.keys()] for dlist in interfnks_filedlist]
    ...
    filedlist2=[dict(filed, ana__inter_filedlist=interfiledlist) \
          for filed, interfiledlist in zip(filedlist, interfnks_filedlist) if len(interfiledlist)>0]
...
class Analysis__SpectralPhoto(Analysis_Master_inter):
    def __init__(self):
        ...
        self.requiredkeys=['t(s)']#, 't(s)_dark', 'Ewe(V)_dark', 'I(A)_dark', 't(s)_ill', 'Ewe(V)_ill', 'I(A)_ill', 'IllumBool']#these intermediate keys are not tested for explicitly, ontly through the custom getapplicablefilenames below
        self.optionalkeys=[]
        self.requiredparams=['illumination_wavelength', 'toggle_value_illumination', 'echem_params__']
        ...
        self.csvheaderdict['plot_parameters']['plot__1']=dict({}, fom_name='num_wavelengths', colormap='jet', colormap_over_color='(0.5,0.,0.)', colormap_under_color='(0.,0.,0.)')
    ...
    def getapplicablefilenames(self, expfiledict, usek, techk, typek, runklist=None, anadict=None):
        ...
        self.filedlist=ECHEPHOTO_checkcompletedanalysis_files_by_sample(self.filedlist, expfiledict, anadict, requiredanalysis=self.params['photo_analysis_ana_fcn'], tech=techk, gui_mode_bool=self.gui_mode_bool)
    ...
    def perform(self, destfolder, expdatfolder=None, writeinterdat=True, anak='', zipclass=None, anauserfomd={}, expfiledict=None):
        ...
        if len(mwparam)==0:
            ...
            genmw=lambda d:mwvals[d['toglistind']] if 'toglistind' in d.keys() else numpy.nan
        ...
        for filed in self.filedlist:
            ...
            for interd in interdlist:
                ...
                wlk=int(wl)
                photofomd_wl[wlk]={}#if ana__ data for multiple waqvelengths overwrite will happen here
                photofomd=photofomd_wl[wlk]

                for fomfiled in interd['fom_files_dlist']:
                    fomd=readcsvdict(os.path.join(destfolder, fomfiled['fn']), fomfiled, returnheaderdict=False, zipclass=None, includestrvals=False)
                    if not filed['sample_no'] in list(fomd['sample_no']):
                        continue
                    fomind=list(fomd['sample_no']).index(filed['sample_no'])#assumption that sample_no is in the fom csv fiel is from the fiel matching above that finds sample_no from the inter_files within this ana__
                    photofomd.update([(k, v[fomind]) for k, v in fomd.iteritems() if 'photo' in k])
                ...
                photofomd['sample_no']=filed['sample_no']#sample_no is needed for csv but does not count as a 
            ...
            fn='%s__%s-%d.csv' %(anak,'spectralfoms', filed['sample_no'])#name file by foms but onyl inlcude the 1st 3 to avoid long names
            csvfilstr=createcsvfilstr(wl_dlist, floatkeys, intfomkeys=intkeys)#, fn=fnf)

            allnames=['sample_no']+intkeys+floatkeys#this is the order in createcsvfilstr 
