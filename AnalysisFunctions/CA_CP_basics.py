...
referenceshiftfcn = lambda x, e0, redoxstr: (x - e0) * (
    (-1)
    ** (
        not redoxstr
        in ["FeCp2+/0", "Fe(CN)6-3/-4", "O2/H2O", "FeCp2+0", "Fe(CN)6-3-4", "O2H2O"]
    )
)
...
class Analysis__Imax(Analysis_Master_nointer):
    def __init__(self):
        ...
        self.dfltparams = {
            "seconds_to_skip_at_start": 0.0,
            "seconds_to_skip_at_end": 0.0,
        }
    ...
    def calc_min_or_max(self, dataarr, maxbool=True):
        t = dataarr[1]
        inds = numpy.where(
            (t >= self.params["seconds_to_skip_at_start"])
            & ((t.max() - t) >= self.params["seconds_to_skip_at_end"])
        )
...
class Analysis__Iave(Analysis_Master_nointer):
    def __init__(self):
        ...
        self.dfltparams = dict(
            [
                ("duration_s", 2.0),
                ("num_std_dev_outlier", 2.0),
                ("num_pts_outlier_window", 999999),
                ("from_end", True),
            ]
        )
        self.params = copy.copy(self.dfltparams)
    ...
    def avefcn(self, x, t):
        ...
        x = removeoutliers_meanstd(
            x,
            self.params["num_pts_outlier_window"] // 2,
            self.params["num_std_dev_outlier"],
        )
...
class Analysis__Etaave(
    Analysis_Master_inter, Analysis__Iave
):  # this order is improtant, i.e. get perform() from master and avefcn from Iave
    def __init__(self):
        self.analysis_fcn_version = (
            "2"  # changed FOM label from E.V_ave to Eta.V_ave on 20160212
        )
        self.dfltparams = dict(
            [
                ("duration_s", 2.0),
...
class Analysis__Iphoto(Analysis_Master_inter):
    ...
    def getapplicablefilenames(
        ...
        if self.params["illum_key"] == "t(s)":
            ...
            requiredparams += [
                self.echem_params_key
            ]  # ['toggle_dark_time_init', 'toggle_illum_time', 'toggle_illum_duty', 'toggle_illum_period']
    ...
    def photofcn(self, d, filed):
        ...
        err = calcdiff_ill_caller(
            d,
            interd,
            ikey=ikey,
            thresh=self.params["illum_threshold"],
            ykeys=[self.requiredkeys[0]],
            xkeys=list(self.requiredkeys[1:-1]),
            illfracrange=(
                self.params["frac_illum_segment_start"],
                self.params["frac_illum_segment_end"],
            ),
            darkfracrange=(
                self.params["frac_dark_segment_start"],
                self.params["frac_dark_segment_end"],
            ),
        )
        fomtuplist = []
...
class Analysis__Ephoto(Analysis__Iphoto):
    def __init__(self):
        ...
        self.plotparams["plot__2"][
            "series__1"
        ] = "Ewe(V)_dark,Ewe(V)_ill,Ewe(V)_illdiff"
        self.csvheaderdict = dict({}, csv_version="1", plot_parameters={})
        self.csvheaderdict["plot_parameters"]["plot__1"] = dict(
            {},
            fom_name=self.fomnames[0],
...
class Analysis__Etaphoto(Analysis__Iphoto):
    def __init__(self):
        ...
        self.plotparams["plot__2"][
            "series__1"
        ] = "Eta(V)_dark,Eta(V)_ill,Eta(V)_illdiff"
        self.csvheaderdict = dict({}, csv_version="1", plot_parameters={})
        self.csvheaderdict["plot_parameters"]["plot__1"] = dict(
            {},
            fom_name=self.fomnames[0],
    ...
    def fomtuplist_rawlend_interlend(self, dataarr, filed):
        ...
        fomtuplist = [
            (k, referenceshiftfcn(v, filed["reference_e0"], filed["redox_couple_type"]))
            for k, v in fomtuplist
        ]
        # rawlend just has IllumBool so add Eta(V)
        rawlend["Eta(V)"] = referenceshiftfcn(
            dataarr[0], filed["reference_e0"], filed["redox_couple_type"]
        )
        ...
        for oldk in keystochange:
            ...
            interlend[newk] = referenceshiftfcn(
                interlend[oldk], filed["reference_e0"], filed["redox_couple_type"]
            )
            del interlend[oldk]
...
class Analysis__Iphotothresh(Analysis__Iphoto):
    ...
    def fomtuplist_rawlend_interlend(self, dataarr, filed):
        ...
        nonnaninds = np.union1d(
            np.where(1 - np.isnan(ewe))[0], np.where(1 - np.isnan(iph))[0]
        )
        ts = ts[nonnaninds]
        ewe = ewe[nonnaninds]
        iph = iph[nonnaninds]
        ...
        if (  # combined loss find nearest
            ...
            ewe_thresh = (
                np.float(self.params["nearest_Erhe.V"]) + filed["reference_vrhe"]
            )
            ts_thresh = np.float(self.params["nearest_t.s"])
            mininds = np.argsort(
                (np.abs(ts - ts_thresh) / np.max(ts))
                + (np.abs(ewe - ewe_thresh) / ewe_range)
            )
            ...
            self.params["nearest_Erhe.V"] is not "" and self.params["nearest_t.s"] is ""
        ):
            ...
            self.params["nearest_Erhe.V"] is "" and self.params["nearest_t.s"] is not ""
        ):
            # print "closest t.s"
            ts_thresh = np.float(self.params["nearest_t.s"])
...
class Analysis__E_Ithresh(Analysis_Master_nointer):
    def __init__(self):
        ...
        self.dfltparams = dict(
            [
                ("i_thresh", 1.0e-5),
                ("num_consec_points", 20),
                ("above_bool", 1),
                ("skip_time_at_start_s", 0.0),
            ]
        )
        self.params = copy.copy(self.dfltparams)
