...
def get_element_list_for_list_of_pidstr(l_plate_idstr, union_bool=False):
    if union_bool:
        ...
        for pidstr in l_plate_idstr[1:]:
            els_to_return = els_to_return.intersection(
                set(getelements_plateidstr(pidstr))
            )
        els_to_return = sorted(list(els_to_return))
    return els_to_return


def append_udi_to_ana(
    l_anapath=None,
    l_anak_comps=None,
    l_anak_patterns=None,
    pattern_key="pattern_files",
    compkeys="AtFrac",
    q_key="q.nm_processed",
    intensity_key="intensity.counts_processed",
    pattern_fn_search_str="",
    union_el_list=False,
    xykeys_compcsv=None,
):
    ...
    if l_anapath is None:
        num_ana = userinputcaller(
            None, inputs=[("num_ana", int, "1")], title="Enter Number of ana to open"
        )
        if num_ana is None:
            return
        l_anapath = [
            selectexpanafile(
                None, exp=False, markstr="Select .ana/.pck to import, or .zip file"
            )
            for i in range(num_ana)
        ]
    ...
    if l_anak_comps is None or len(l_anak_comps) != len(l_anapath):
        ans = userinputcaller(
            None,
            inputs=[("ana_ints_for_comps", str, ",".join(["1"] * num_ana))],
    ...
    if l_anak_patterns is None or len(l_anak_patterns) != len(l_anapath):
        ans = userinputcaller(
            None,
            inputs=[("ana_ints_for_patterns", str, ",".join(["1"] * num_ana))],
    ...
    l_anadict_zipclass = [
        readana(p, stringvalues=False, erroruifcn=None, returnzipclass=True)
        for p in l_anapath
    ]
    get_file_path = (
        lambda p, fn, zipclass: fn
        if zipclass
        else (
            os.path.join(os.path.split(p)[0], fn)
            if ".ana" in p
            else os.path.join(p, fn)
        )
    )
    l_found_comp_keys = []
    l_comp_fomd = []
    ...
    for count, (anap, (anadict, zipclass), anak_comps, anak_patterns) in enumerate(
        zip(l_anapath, l_anadict_zipclass, l_anak_comps, l_anak_patterns)
    ):
        ...
        l_plate_idstr += [
            ...
            if isinstance(anadict[anak_comps]["plate_ids"], str)
            else ("%d" % anadict[anak_comps]["plate_ids"])
        ]
        ...
        for fn, filed in anadict[anak_comps]["files_multi_run"][
            ...
            if (len(found_comp_keys) > 0 and isinstance(compkeys, str)) or len(
                found_comp_keys
            ) == len(compkeys):
                fomd = readcsvdict(
                    get_file_path(anap, fn, zipclass),
                    filed,
        ...
        pidstr = (
            ...
            if isinstance(anadict[anak_patterns]["plate_ids"], str)
        ...
        for filesrunk, filesrund in anadict[anak_patterns].iteritems():
            ...
            for fn, filed in filesrund[pattern_key].iteritems():
                ...
                if (
                    q_key in filed["keys"]
                    and intensity_key in filed["keys"]
                    and filed["sample_no"] in fomd["sample_no"]
                ):
        ...
        if xykeys_compcsv is None:
            ...
            fomd["x"] = numpy.array(
                [pmdlist[smps.index(smp)]["x"] for smp in fomd["sample_no"]]
            )
            fomd["y"] = numpy.array(
                [pmdlist[smps.index(smp)]["y"] for smp in fomd["sample_no"]]
            )
        else:
    ...
    els_for_udi_temp = get_element_list_for_list_of_pidstr(
        l_plate_idstr, union_bool=union_el_list
    )
    els_for_udi = []
    ...
    for (
        count,
        (pidstr, elkey_byel, fomd, pattern_l_fn_fd, anap, (anadict, zipclass)),
    ) in enumerate(
        zip(
            l_plate_idstr,
            l_elkey_byel,
        ...
        inds = sorted(
            [
                fomdsmps.index(smp)
                for smp in patternsmps
                if not (pidstr, smp) in smps_added_so_far
            ]
        )
        inds = [
            i
            ...
            if not (
                True
                in [
                    numpy.isnan(fomd[elkey][i])
                    for elkey in elkey_byel
                    if not elkey is None
                ]
            )
        ]  # making sure none of the composition values is NaN. if elkey is None then that element isn't for this plate so don't penalize that
        ...
        for i in inds:
            cmp = numpy.array(
                [0.0 if elkey is None else fomd[elkey][i] for elkey in elkey_byel]
            )
            if cmp.sum() > 0.0:
                cmp /= cmp.sum()
            udi_dict["comps"] += [cmp]
    ...
    analysismasterclass.csvheaderdict["plot_parameters"]["plot__1"] = dict(
        {},
        fom_name="Intensity.max",
        colormap="jet",
        colormap_over_color="(0.5,0.,0.)",
        colormap_under_color="(0.,0.,0.)",
    )
    filedesc = analysismasterclass.writefom_bare(
        anafolder, csvfn, strkeys=[], floatkeys=None, intfomkeys=["runint", "plate_id"]
    )
    anadict[anak] = {}
    ...
    anadict[anak]["description"] = (
        "make udi with comps from %s and patterns from %s"
        % (",".join(l_anak_comps), ",".join(l_anak_patterns))
    )
    ...
    if len(auxanainds) > 0:
        ...
        for l_ind in auxanainds:
            rel_path_list += [
                get_relative_path_for_exp_or_ana_full_path(
                    os.path.split(l_anapath[l_ind])[0], exp=False
                )
            ]
    ...
    anadict[anak]["files_multi_run"]["misc_files"][udifn] = (
        "%s_udi_file" % anadict[anak]["technique"]
    )
...
def append_resampled_merged_patterns_to_ana(
    l_anapath=None,
    l_anak_patterns=None,
    l_pattern_fn_search_str=None,
    pattern_key="pattern_files",
    q_key="q.nm_processed",
    intensity_key="intensity.counts_processed",
    dq=None,
    q_log_space_coef=None,
    resamp_interp_order=1,
    pre_resamp_smooth_fcn=None,
    gradual_average_overlap_bool=True,
):
    if l_anapath is None:
        num_ana = userinputcaller(
            None, inputs=[("num_ana", int, "1")], title="Enter Number of ana to open"
    ...
    if l_pattern_fn_search_str is None or len(l_pattern_fn_search_str) != len(
        ...
        ans = userinputcaller(
            None,
            inputs=[
                ("pattern_fn_search_per_anakey", str, ",".join([".csv"] * num_ana))
            ],
    ...
    for (
        count,
        (anap, (anadict, zipclass), anak_patterns, pattern_fn_search_str),
    ) in enumerate(
        zip(l_anapath, l_anadict_zipclass, l_anak_patterns, l_pattern_fn_search_str)
    ):
    ...
    for smpcount, smp in enumerate(smps):
        ...
        for (
            ...
            if count == 0:
                lsmps_runint += [
                    runint
                ]  # might be merging multiple runs but take the first
        ...
        if smpcount == 0:
            ...
            if not dq is None:
                ...
                L = numpts_log_spacing_coef(
                    qmin, qmax, q_log_space_coef
                )  # will round down num points to be sure q_rasmp within bounds of qmin,qmax
                q_resamp = qmin * (q_log_space_coef ** (numpy.arange(L)))
            else:
                L = numpy.sum(
                    [len(q) for q in l_q]
                )  # preserve total num q values, which is oversampling if overlapped Q values
                q_resamp = numpy.linspace(qmin, qmax, L)
            #                print 'resampling required for merging at this time'
            #                return
            # assume all patterns same l_q
            qresampfcn = lambda l_z: numpy.array(
                [
                    interpolate.InterpolatedUnivariateSpline(
                        q, z, k=resamp_interp_order, ext="zeros"
                    )(q_resamp)
                    for q, z in zip(l_q, l_z)
                ]
            )  # functino resamps on whole array even if there are lots out of bounds. l_q is local var from smpcount==0 and is used for all later interps
            interp_mask_numpatterns_by_qresamp = (
                qresampfcn(l_q) != 0
            )  # interpolate q values (could interp zeros but q is avaialble in the right size and nonzero) with 0 fill to see where q_resamp is beyond bounds of each qarr in l_q. This mask is 1 where interp is valid
            numpatterns_contributing_to_each_qresamp_val = interp_mask_numpatterns_by_qresamp.sum(
                axis=0, dtype=numpy.int32
            )
            resampinds_zerofill = numpy.where(
                numpatterns_contributing_to_each_qresamp_val == 0
            )[0]
            resampinds_ave = numpy.where(
                numpatterns_contributing_to_each_qresamp_val > 0
            )[0]
            ...
            weights_2d[
                :, resampinds_zerofill
            ] = 0.0  # this has no consequence due to the zero fill in interp but makes sense to write. These inds have no input data from the patterns
            if (
                gradual_average_overlap_bool
                and 2 in numpatterns_contributing_to_each_qresamp_val
                and numpy.all(numpatterns_contributing_to_each_qresamp_val <= 2)
            ):
                startinds = (
                    numpy.where(
                        (numpatterns_contributing_to_each_qresamp_val[:-1] == 1)
                        & (numpatterns_contributing_to_each_qresamp_val[1:] == 2)
                    )[0]
                    + 1
                )
                stopinds = (
                ...
                if len(stopinds) > 0 and len(stopinds) == len(startinds):
                    for i, j in zip(startinds, stopinds):
                        activepatterns = numpy.where(
                            interp_mask_numpatterns_by_qresamp[:, i] == True
                        )[0]
                        ...
                        if interp_mask_numpatterns_by_qresamp[
                            activepatterns[1], i - 1
                        ]:  # if the data point before i is active in the 1st pattern, then this pattern is the one with lower Q so reverse their order
                            activepatterns = activepatterns[::-1]
                        newweights = numpy.linspace(0.0, 1.0, j - i)
                        weights_2d[activepatterns[0], i:j] = 1.0 - newweights
                ...
                for patternind in range(weights_2d.shape[0]):
                    weights_2d[patternind, resampinds_ave] = (
                        1.0
                        / numpatterns_contributing_to_each_qresamp_val[resampinds_ave]
                    )  # wieght each pattern to obtain mean
        ...
        area_reconstructed_inresampledpattern = cumtrapz(
            resamparr.sum(axis=0), x=q_resamp
        )[
            -1
        ]  # this is done on the sum of interpolated patterns without the weights since the weights aren't applied below. i.e. overlapped portions are double counted here and int he next line
        area_patterns = numpy.sum(
            [cumtrapz(Iarr, x=qarr)[-1] for qarr, Iarr in zip(l_q, l_I)]
        )  # these integrals cannot contain the weights_2d because they are nto resampled
        reconstruction_area_error = (
            area_reconstructed_inresampledpattern - area_patterns
        ) / area_patterns
    ...
    anadict[anak]["analysis_general_type"] = "process_fom"
    anadict[anak]["description"] = "".join(
        [
            "resample ",
            "" if len(l_anak_patterns) == 1 else "and merge",
            " patterns from ",
            ",".join(l_anak_patterns),
        ]
    )
    anadict[anak]["plate_ids"] = pidstr
    anadict[anak]["technique"] = anadict["analysis_type"]
    tempstr = (
        "none" if l_pattern_fn_search_str is None else ",".join(l_pattern_fn_search_str)
    )
    ...
    for smp, runint, reconstruction_area_error, I_resamp, newfn in zip(
        smps,
        lsmps_runint,
        lsmps_reconstruction_area_error,
        lsmps_I_resamp_flattened,
        ...
        anadict[anak][filesrunk][pattern_key][fn] = (
            "%s_csv_pattern_file;%s,%s;1;%d;%d"
            % (
                anadict[anak]["technique"],
                newq_key,
                newintensity_key,
                len(q_resamp),
                smp,
    ...
    analysismasterclass.csvheaderdict["plot_parameters"]["plot__1"] = dict(
        {},
        fom_name="Reconstruction_Area_Rel_Error",
        colormap="jet",
        colormap_over_color="(0.5,0.,0.)",
        colormap_under_color="(0.,0.,0.)",
    )
...
def percent_change_in_lattice_contsant(rho, num_shifted_patterns):
    return 100.0 * (rho ** (num_shifted_patterns - 1.0) - 1.0)
