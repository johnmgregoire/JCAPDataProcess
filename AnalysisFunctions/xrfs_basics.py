...
def getapplicablefilenames_specific_usetypetech(
    expfiledict,
    usek,
    techk,
    typek,
    ...
    if not (
        (specificuse is None or usek == specificuse)
        and (specifictech is None or techk == specifictech)
        and (specifictype is None or typek == specifictype)
    ):
        return 0, {}
...
def getapplicable_runs_paramsonly(expfiledict, usek, runklist=None, requiredparams=[]):
    ...
    runklist = [
        runk
        for runk in runklist
        if runk.startswith("run__") and (usek in expfiledict[runk]["run_use"])
    ]
    ...
    filedlist = [
        dict(
            ...
            run=runk,
            runparamd=expfiledict[runk]["parameters"],
...
class Analysis__XRFS_EDAX(Analysis_Master_nointer):
    ...
    def getapplicablefilenames(
        ...
        (
            ...
            self.filedlist,
        ) = getapplicablefilenames_specific_usetypetech(
            expfiledict,
            usek,
            techk,
            typek,
            ...
            specificuse=None,
            specifictech="XRFS",
            specifictype="batch_summary_files",
        )
        ...
        self.fomnames = [
            ...
            "StagZ",
            "StagR",
        ]  # to reset this after function used
    ...
    def perform(
        ...
        for filed in self.filedlist:
            ...
            filed["batch_summary"] = read_xrfs_batch_summary_csv(
                p,
                select_columns_headings__maindict=self.fomnames,
                include_inte_wt_at_subdicts=True,
                include_transitionslist_bool=True,
        ...
        for filed in self.filedlist:
            for tr in filed["batch_summary"]["transitionslist"]:  # to preserve order
                if not tr in alltransitions:
                    alltransitions += [tr]
        batchdictkey_appendstr = [
            (k, self.params[k.strip("%") + "_append"]) for k in ["Inte", "Wt%", "At%"]
        ]
        keymodfcn = lambda k, a: "%s.%s%s" % (k[:-1], k[-1:], a)
        trfoms = [
            keymodfcn(tr, s) for k, s in batchdictkey_appendstr for tr in alltransitions
        ]
...
class Analysis__PlatemapComps(Analysis_Master_nointer):
    def __init__(self):
        ...
        self.dfltparams = {
            ...
            "other_keys_to_include": "x,y,code",
        }
        self.params = copy.copy(self.dfltparams)
    ...
    def getapplicablefilenames(
        ...
        if True in [
            ...
            self.filedlist = []
            return self.filedlist
        self.num_files_considered, self.filedlist = getapplicable_runs_paramsonly(
            expfiledict, usek, runklist=None, requiredparams=self.requiredparams
        )
        return self.filedlist
    ...
    def perform(
        ...
        for filed in self.filedlist:
            try:
                ...
                (
                    errbool,
                    (cels_set_ordered, conc_el_chan),
                ) = get_multielementink_concentrationinfo(
                    filed["runparamd"], els, return_defaults_if_none=True
                ...
                if errbool:
                    raiseTemp
                pmkeys_to_include = [
                    k.strip() for k in self.params["other_keys_to_include"].split(",")
                ]
                pmpath = getplatemappath_plateid(
                    "", pmidstr=str(filed["map_id"]), return_pmidstr=False
                )
                ...
                newfomnames = [
                    ...
                    if not (lab in FOMKEYSREQUIREDBUTNEVERUSEDINPROCESSING)
                ]
                self.fomnames = [
                    k
                    for k in (newfomnames + pmkeys_to_include)
                    if k in platemapdlist[0]
                ]
            ...
            plotfomname = (
                ...
                if self.params["tot_conc_label"] in self.fomnames
                else self.fomnames[0]
            )
            self.csvheaderdict["plot_parameters"]["plot__1"] = dict(
                {},
                fom_name=plotfomname,
                colormap="jet",
                colormap_over_color="(0.5,0.,0.)",
                colormap_under_color="(0.,0.,0.)",
            )
            self.writefom(
                ...
                anauserfomd=anauserfomd,
                strkeys_fomdlist=self.strkeys_fomdlist,
                num_intfoms_at_start_of_fomdlist=num_intfoms_at_start_of_fomdlist,
            )  # sample_no, plate_id and runint are explicitly required in csv selection above and are assume to be present here
