...
os.chdir(r'K:\users\hte\Raman\40767\averaged_20180312_square')
with open('40767--1-001_avecirclespectra.txt', mode='r') as f:
    lines=f.readlines()

spectra_string=''.join([('\nI%d=' %(count+1))+','.join(l.strip().split(' ')[::-1]) for count, l in enumerate(lines)])

with open('40767--1-001_averingspectra.txt', mode='r') as f:
    lines=f.readlines()

spectra_string_substrate=''.join([('I%d=' %(count+1))+l.replace(' ', ',') for count, l in enumerate(lines)])

with open('40767--1-001_wdata.txt', mode='r') as f:
    lines=f.readlines()
q_string='Q='+','.join([l.strip() for count, l in enumerate(lines)][::-1])

with open('40767--1-001_sample_no.txt', mode='r') as f:
    lines=f.readlines()
...
with open('20180312_TRI_4076_MnFeNiCuCoZn_sample_averaged.udi', mode='w') as f:
    f.write(s_smps)

with open('20180312_TRI_4076_MnFeNiCuCoZn_substrate_averaged.udi', mode='w') as f:
    f.write(s_substrate)
