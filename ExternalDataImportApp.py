...
class extimportDialog(QDialog, Ui_ExternalImportDialog):
    def __init__(self, parent=None, title="", folderpath=None):
        ...
        for cb in [self.ExpSaveComboBox, self.AnaSaveComboBox]:
            for count, opt in enumerate(
                ["Do not save", "Save in TEMP as .run", "Save as .run", "Save as .done"]
            ):
                cb.insertItem(count, opt)
            cb.setCurrentIndex(3)
        self.ExpSaveComboBox.activated["QString"].connect(self.restrictanasave)
        for button, fcn in button_fcn:
            button.pressed.connect(fcn)
        self.CalcXLineEdit.editingFinished.connect(self.lookupsamples)
        self.CalcYLineEdit.editingFinished.connect(self.lookupsamples)
        # QObject.connect(self.RunSelectTreeWidget, SIGNAL('itemChanged(QTreeWidgetItem*, int)'), self.runselectionchanged)
        self.RcpTreeWidget.itemDoubleClicked[QTreeWidgetItem, int].connect(
            self.edittreeitem_readonly
        )
        self.ExpTreeWidget.itemDoubleClicked[QTreeWidgetItem, int].connect(
            self.edittreeitem_exp
        )
        self.AnaTreeWidget.itemDoubleClicked[QTreeWidgetItem, int].connect(
            self.edittreeitem_ana
        )
        self.RunSelectTreeWidget.itemDoubleClicked[QTreeWidgetItem, int].connect(
            self.edittreeitem
        )
        self.runtreeclass = treeclass_filedict(self.RunSelectTreeWidget)
        self.profiledefintionfcns = [
            self.xrds_profile_withq,
            self.xrds_profile,
            self.ssrl_profile_v1,
        ]
    ...
    def create_udi(self, opttionsearchstr_comps=None, opttionsearchstr_xrd=None):
        ...
        if not opttionsearchstr_xrd is None:
            ...
            xrdselectind = userselectcaller(
                self,
                options=xrdanaoptions,
                title="Select xrd data source for udi",
                cancelallowed=True,
            )
        ...
        udi_dict = {
            "ana_file_type": "pattern_files",
            "anak": anak_xrd,
            "q_key": qkey,
            "intensity_key": intkey,
            "pattern_source_analysis_name": analysis_name__patterns,
        }
        kl = [
            k
            for k in sort_dict_keys_by_counter(self.anadict, keystartswith="ana__")
        ...
        if len(anaoptions) == 0:
            ...
            # anaoptions+=['platemap']
            selectind = userselectcaller(
                self,
                options=anaoptions,
                title="Select composition source for udi",
                cancelallowed=True,
            )
        ...
        self.AnaTreeWidgetFcns.filltree(
            self.anadict, startkey="ana_version", laststartswith="ana__"
        )
    ...
    def copy_createudi(self, afd, anafolder):
        ...
        create_udi_anas(
            ...
            udi_dict,
            anadict=anadict_with_filed,
            anadict_comps=anadict_with_filed,
            anafolder=anafolder,
            anafolder_comps=anafolder,
        )
    ...
    def copy_createfomudi(self, afd, anafolder):
        ...
        os.path.join(anafolder, afd["anafn"])
        analysismasterclass.csvheaderdict = dict(
        ...
        self.anadict[afd["anak"]]["files_multi_run"][afd["type"]][
            afd["anafn"]
        ] = filedesc
    ...
    def copy_all_to_ana__ssrl(self, anatempfolder):
        ...
        for k, headline in zip(
            ["qcounts_subbcknd", "qcounts"],
            ["q.nm_processed,intensity.counts_processed", "q.nm,intensity.counts"],
        ):  # do qcounts last so this is the default for mainh5inds below
            q = g["xrd"][k].attrs["q"]
            temptups = sorted(
                [
        ...
        if not (len(mainh5inds) == 0 or len(templ) != 1):
            ...
            self.anadict[afd["anak"]]["files_multi_run"][afd["type"]][
                afd["anafn"]
            ] = ";".join([afd["fval"].rpartition(";")[0], "%d" % len(mainsmps)])
            lines = [afd["headline"]]
            fmstr = ",".join(fmtlist)
            ...
            for anak in sort_dict_keys_by_counter(self.anadict, keystartswith="ana__"):
                if self.anadict[anak]["name"] == "Analysis__SSRL_Integrate":
                    ...
                    self.anadict[anak]["parameters"] = {
                        "method": "legacy spline-based baseline detection"
                    }
        h5f.close()
        for count, afd in enumerate(self.ana_filedict_tocopy):
    ...
    def mod_smp_afd__xrds_withq(self, afd, anak, smpstr=None):
        ...
        if not ("q.nm" in afd["fval"]):
            afd["fval"] = afd["fval"].replace(
                "two_theta.deg",
                "q.nm_processed,two_theta.deg"
                if "_processed" in afd["fval"]
                else "q.nm,two_theta.deg",
            )
        afd["anak"] = anak
    ...
    def createfiledicts(self, rundoneext=".done"):
        ...
        self.expdict["description"] = "%d %s runs on plate_id %s" % (
            len(self.inds_rcpdlist),
            self.datatype,
            self.plate_idstr,
        )
                        ...
                        for afd in anafiledlist:
                            ...
                            for modafd in modafdlist:
                                ...
                                ana__d[antypek][modafd["anafn"]] = (
                                    modafd["fval"] + smp
                                )  # ana file val expected to have all other info included already
        ...
        self.RcpTreeWidgetFcns.filltree(
            self.all_rcp_dict,
            startkey="",
            laststartswith="rcp_file__",
            updatedwithtoplevelitems=True,
        )
        self.ExpTreeWidgetFcns.filltree(
            self.expdict, startkey="exp_version", laststartswith="run__"
        )
        self.AnaTreeWidgetFcns.filltree(
            self.anadict, startkey="ana_version", laststartswith="ana__"
        )
    ...
    def add_misc_to_ana(self, p=None, anak=None, anarunk=None, filltreebool=True):
        if anak is None or anarunk is None:
            ...
            ans = userinputcaller(
                ...
                inputs=[
                    ("ana__X  key", str, anak),
                    ("files_run__X  key", str, anarunk),
                ],
                title="Enter location in .ana",
                cancelallowed=True,
            )
            if ans is None:
                return
        ...
        if p is None:
            p = mygetopenfile(
                parent=self,
                xpath=self.import_folder,
                markstr="select file for import into ana",
            )
        if len(p) == 0:
            return
        fold, fn = os.path.split(
            p
        )  # would be prudent to check if this filename is already in the anak but hopefully user doesn't do this
        newfn = "%s_%s" % (anak, fn)
    ...
    def editparams(self, widget, item=None, column=0, readonly=False, d=None):
        ...
        warningbool = warningbool or True in [
            ws in parentstr or ws in k for ws in ["__", "parameters", "files_"]
        ]
        if warningbool:
    ...
    def importfolder(
        self, p=None, plate_idstr=None, fn_search_str_list_for_checkbool=None, **kwargs
    ):
        self.cleardata()
        self.profiledefintionfcns[
        ...
        find_fn_list = lambda filedlist: [
            True in [s in filed["fn"] for s in fn_search_str_list_for_checkbool]
            for filed in filedlist
        ]
        for count, rcpd in enumerate(self.maindatad["rcpdlist"]):
            mainitem = QTreeWidgetItem(["temprun__%d" % (count + 1)], 0)
            self.runtreeclass.fillmainitem_with_dlistvalues(
                ...
                if "ana_files" in fd.keys():
                    for an_name, auxfiledlist in fd["ana_files"].items():
                        ...
                        self.runtreeclass.fillmainitem_with_dlistvalues(
                            ...
                            v="fval",
                            checkbool=True
                            if fn_search_str_list_for_checkbool is None
                            else find_fn_list(auxfiledlist),
                        )
    ...
    def openplatemap(self, plate_idstr=None):
        if plate_idstr is None:
            pmpath = mygetopenfile(
                parent=self, xpath=PLATEMAPFOLDERS[0], markstr="select platemap"
            )
            if len(pmpath) == 0:
                return None
            self.pmidstr = None
        else:
            pmpath, self.pmidstr = getplatemappath_plateid(
                plate_idstr, return_pmidstr=True
            )
            self.platemapdlist = readsingleplatemaptxt(
                pmpath,
    ...
    def savefiles(self, overwrite=False, rundoneext=".done"):
        ...
        if os.path.isdir(rcpmainfolder):
            if overwrite:
                ...
                messageDialog(
                    self, "Aborting SAVE because %s folder exists" % rcpmainfolder
                ).exec_()
                return
        ...
        for runcount, ind_rcp in enumerate(self.inds_rcpdlist):
            ...
            if os.path.isdir(rcpfolder):
                messageDialog(
                    self, "Aborting SAVE because at least 1 rcp drop folder exists"
                ).exec_()
                return
        for runcount, ind_rcp in enumerate(self.inds_rcpdlist):
            rcpfiled = self.all_rcp_dict["rcp_file__%d" % (runcount + 1)]
            ...
            for fd in rcpd["file_dlist"]:
                shutil.copy(
                    os.path.join(fd["folderpath"], fd["fn"]),
                    os.path.join(rcpfolder, fd["fn"]),
                )
        ...
        if saveopt > 0:
            saveexpfiledict, dsavep = saveexp_txt_dat(
                self.expdict,
                saverawdat=False,
                experiment_type="temp" if saveopt == 1 else self.datatype,
                rundone=".done" if saveopt == 3 else ".run",
        ...
        if saveopt > 0:
            ...
            anafolder = saveana_tempfolder(
                anafilestr,
                anatempfolder,
                anadict=None,
                analysis_type="temp" if saveopt == 1 else self.datatype,
                rundone=".done" if saveopt == 3 else ".run",
            )
        ...
        for count, rcpd in enumerate(self.maindatad["rcpdlist"]):
            for fd in rcpd["file_dlist"]:
                if "xyarr" in fd.keys():
                    ...
                    fd["treeitem"].setText(
                        0, ": ".join([fd["fn"], fd["fval"]])
                    )  # the smpstr is kept track of in the tree widget so that it is user editable
    ...
    def plot(self, xyplotinfo):
        ...
        if len(xl) == 0:
            ...
            return
        circ = pylab.Circle((50, 47.3), radius=50.0, edgecolor="k", facecolor="none")
        ax.add_patch(circ)
        x0, x1 = 0, 100
        y0, y1 = 0, 97.3
        x0 = min(x0, min(xl))
    ...
    def plotwsetup(self):
        ...
        self.plotw_plate.fig.subplots_adjust(left=0.12, bottom=0.12)
...
class treeclass_filedict:
    ...
    def filltree(
        ...
        updatedwithtoplevelitems=False,
    ):
        self.treeWidget.clear()
    ...
    def fillmainitem_with_dlistvalues(
        self, mainitem, dlist, k="k", v="v", updatedwithitems=True, checkbool=None
    ):
    ...
    def createtxt(self, parentitem=None, indent="    "):
        ...
        if parentitem is None:
            itemlist = [
                self.treeWidget.topLevelItem(count)
                for count in range(int(self.treeWidget.topLevelItemCount()))
            ]
        else:
            itemlist = [
                parentitem.child(count) for count in range(int(parentitem.childCount()))
            ]
...
if __name__ == "__main__":
    ...
    class MainMenu(QMainWindow):
        def __init__(self, previousmm, execute=True, **kwargs):
            ...
            self.extimportui = extimportDialog(
                self, title="Create RCP/EXP/ANA for non-HTE instruments", **kwargs
            )
