...
#
ananames='20160802.211016.run,20160803.030404.run,20160803.034223.run,20160902.194122.run,20160803.042224.run,20160923.153723.run,20160912.125625.run,20160803.113046.run,20160923.151003.run,20160803.124020.run,20160803.050406.run,20160902.203104.run,20160906.161617.run,20160802.180002.run,20160912.133819.run,20160712.121351.run,20160802.184102.run,20160715.174915.run,20160923.160636.run,20160902.230552.run,20160906.161617.run,20160912.141405.run,20160923.161009.run'
...
for anacount, (anafold, compintstr) in enumerate(zip(ananames.split(','), compinstrs.split(','))):
    ...
    foldp=os.path.join(r'K:\processes\analysis\uvis', anafold)
    ...
    plotops=[\
          ...
          'stack1.5to2.9_;DA_bg_0;jet_r;(0.,0.,0.);(0.5,0.5,0.5);1.5;2.9',  \
          'stack1.5to2.9_;DA_bg_repr;jet_r;(0.,0.,0.);(0.5,0.5,0.5);1.5;2.9',  \
    ...
    for plotcount, optstr in enumerate(plotops):
        ...
        visdataui.compplotsizeLineEdit.setText('18' if compintstr=='5' else '100')
        ...
        savefigsdialog.doneCheckBox.setChecked(False)#plotcount==(len(plotops)-1))
        if 0:
            visdataui.show()
