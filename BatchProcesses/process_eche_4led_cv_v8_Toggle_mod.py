...
measurement_area_override = None  # None to use exp value
mineqeforplot = 1.e-3
...
expname = None  # r'L:\processes\experiment\eche\20171128.160211.copied-20171129083728283PST'#r'eche\20171115.163552.exp'#None#r'eche\20170727.133340.exp'#None#r'eche\20170727.173030'#None#r'eche\20170717.155030.run'#None#r'xrfs\20170518.122412'
    ...
    if i >= 12:
        ...
        if len(inds) > 0:
            ...
            if not numpy.all((visdataui.fomplotd['fom'] < vmin) | (visdataui.fomplotd['fom'] > vmax)):
                ...
                imagesidialog.widget_plow_dlist[2]['item'].setCheckState(0, Qt.Unchecked)  # xy
                if i == stdplotinds[
                    -1] and 'done' in anasaveextension:  # if need to convert to .done and skipped the fill factor plot, try this - not tested
            ...
            -1] and 'done' in anasaveextension:  # if need to convert to .done and skipped the fill factor plot, try this resave of last image- not tested
            imagesidialog = visdataui.savefigs(justreturndialog=True)
            imagesidialog.doneCheckBox.setChecked(Qt.Checked)
            imagesidialog.ExitRoutine()
            visdataui.importana(p=imagesidialog.newanapath)
