...
class MainMenu(QMainWindow):
    ...
    def extimportui_exec(self, show=True):
        if self.extimportui is None:
            self.extimportui = extimportDialog(
                self, title="Create RCP/EXP/ANA for non-HTE instruments"
            )
        if show:
    ...
    def combinefomui_exec(self):
        if self.combinefomui is None:
            self.combinefomui = combinefomDialog(
                self, title="Combine FOM from multiple files"
            )
    ...
    def visexpana(
        self, anafiledict=None, anafolder=None, experiment_path=None, show=True
    ):
