...
class stackplotDialog(QDialog, Ui_StackPlotDialog):
    def __init__(self, parent=None, title='', folderpath=None, GUIMODE=GUIMODE):
        ...
        self.AnaExpFomTreeWidgetFcns=treeclass_anaexpfom(self.AnaExpFomTreeWidget, None)
        ...
        QObject.connect(self.xplotchoiceComboBox,SIGNAL("activated(QString)"), self.fillxsearchstr)
    ...
    def importana(self, p=None, anafiledict=None, anafolder=None, anazipclass=None, ellabelsforquatplots='ask'):#if running batch set ellabelsforquatplots to something else
        ...
        readandformat_anafomfiles(self.anafolder, self.anafiledict, self.l_fomdlist, self.l_fomnames, self.l_csvheaderdict, templ_platemap4keys, self.AnaExpFomTreeWidgetFcns, anazipclass=self.anazipclass, anakl=self.sorted_ana_exp_keys())
...
#        self.clearexp()
    ...
    def setupfilterchoices(self):
        ...
        for count, fomdlist in enumerate(self.l_fomdlist):
            if len(fomdlist)==0 or not 'anaint' in fomdlist[0].keys():
                continue
            mainitem=QTreeWidgetItem(['%d:ana__%d' %(count, fomdlist[0]['anaint'])], 0)
    ...
    def filterandplotstackdata(self, plotbool=True):
        ...
        for l_count in range(int(self.SelectTreeWidget.topLevelItemCount())):
            ...
            checkedkeys=[str(mainitem.child(i).text(0)).strip() for i in range(mainitem.childCount()) if bool(mainitem.child(i).checkState(0))]
        ...
        avexindex=(numpy.arange(checkedarr2d.shape[1])[numpy.newaxis, :]*checkedarr2d).sum(axis=1)
    ...
    def stackplot(self):
        ...
        cols=[[myeval(s.strip()) for s in ls.split(',')] for ls in colsstrs if ls.count(',')==2]
        if len(cols)==0:
            if self.GUIMODE:
        ...
        for l, col in zip(self.stackplotd['stackkeys'], cols):
            legax.plot([], [], marker='s', ls='None', c=col, markersize=8, alpha=1., label=l)
        
        #legax.legend(loc=2, bbox_to_anchor=(1.1, .55), numpoints=1, markerscale=2, frameon=False, prop={'size':12})    
        legax.legend(loc=2, numpoints=1, markerscale=2, frameon=False, prop={'size':12})    
    ...
    def plot(self):#filed to plot from a  single file and must have key 'path' in addition to standard filed
        ...
        legax=self.plotw_xy.legendaxes
        for count, (ax, (xarr, yarr), xl, yl) in enumerate(zip([self.plotw_xy.axes, self.plotw_xy.twaxes], self.plotdata, [arrkeys[0], arrkeys[0]], [arrkeys[1], arrkeys[2]])):
            if len(xarr)==0:
                continue
    ...
    def plotwsetup(self):
        ...
        self.plotw_xy.fig.subplots_adjust(left=.15, bottom=.12, right=.65, top=.92)
        
        self.plotw_xy.legendaxes=self.plotw_xy.fig.add_axes([.8, 0, .2, 1])
    ...
    def savefigs(self, filenamesearchlist=None, justreturndialog=False, prependstr=''):
        ...
        idialog=saveimagesDialog(self, self.anafolder, self.fomplotd['fomname'], plateid_dict_list=[], code_dict_list=[], \
                                           histplow=None, xyplotw=xyplotw, x_y_righty=x_y_righty, repr_anaint_plots=self.repr_anaint_plots, \
                                           selectsamplebrowser=None, filenamesearchlist=filenamesearchlist)
...
if __name__ == "__main__":
    class MainMenu(QMainWindow):
        def __init__(self, previousmm, execute=True, **kwargs):
            ...
            self.stackui=stackplotDialog(self, title='Visualize Stack Plots from ANA data', **kwargs)
